...
CQGlobe_DrawPolygonCmd::CQGlobe_DrawPolygonCmd(CQGlobe_CommandDlg *pCmdDlg) : CQGlobe_DrawCmd(pCmdDlg)
{
	m_eCmdType = eQGlobe_CMD_DRAW_POLYGON;
...
CQGlobe_DrawPolygonCmd::~CQGlobe_DrawPolygonCmd(void)
{
	if(m_pOrgNode)
	{
		delete m_pOrgNode;
...
void CQGlobe_DrawPolygonCmd::Init(CGIS_Polygon *pNode, bool blNew)
{
	if(pNode == NULL)
		return;
...
void CQGlobe_DrawPolygonCmd::OnCancel()
{
	if(m_pNode == NULL)
	{
...
void CQGlobe_DrawPolygonCmd::OnEnd()
{
	qglobe_GetGISDoc()->SetSelectNode(NULL);
...
int	CQGlobe_DrawPolygonCmd::GetSelectTrackIndex(CursorState* mouseEvent)
{
...
bool CQGlobe_DrawPolygonCmd::OnMousePress(CursorState* mouseEvent)
{
	int nTrackId;
...
bool CQGlobe_DrawPolygonCmd::OnMouseMove(CursorState* mouseEvent)
{
	int nTrackId;
...
bool CQGlobe_DrawPolygonCmd::OnMouseRelease(CursorState* mouseEvent)
{
	CQGlobe_Location3D location;
