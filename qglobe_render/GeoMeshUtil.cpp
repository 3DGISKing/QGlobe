...
#include "../qglobe_terrainengine/CImage.h"
...
{
	double tilewidth=2*qglobe_PI/(1<<level);

	x=((longi+qglobe_PI)/tilewidth);
	y=((lat+qglobe_PI)/tilewidth);
}
...
void GeoMeshUtil::WriteTexture(QFile& file,SMeshBuffer* meshbuffer,int level)
{
	...
	{	
		...
		QString filename=QString::fromLatin1(imagename.c_str());
...
void GeoMeshUtil::GenerateTexture(SMeshBuffer * meshbuffer,int level)
{
	...
	QImage origim;
	origim.load(QString::fromLatin1(meshbuffer->Material.TextureLayer[0].Texture->getName().c_str()));
...
{
	double tileWidth = EARTH_RADIUS*qglobe_TwoPI / (1 << level);
...
void GeoMeshUtil::CalculateScreenRect(SGeoMesh* mesh,core::rect<s32>& screenrect)
{
	...
	********************************/
	...
	qglobe_MatrixTCStoSCS(&standCoord, &Transform );
...
void GeoMeshUtil::CalculateBoundingRect(SMesh* mesh,SGeoMesh::GeoMeshHeader header,core::rect<f32>& boundingrect)
{
	...
	********************************/
	...
	//�� �����˶� ��̡����� �伭���ֳ�.
	qglobe_RotateAroundZAxis(&standCoord,header.Azimuth*qglobe_DEGTORAD);
	...
	float minlongitude=qglobe_PI;
	float maxlongitude=-qglobe_PI;
	float minlatitude=qglobe_PI/2.0;
	float maxlatitude=-qglobe_PI/2.0;;

	CQGlobe_Location3D loc1;

	for(int i=0;i<8;i++)
	{
		p.set(edges[i]);
		qglobe_GetLocation(&p,&loc1);
