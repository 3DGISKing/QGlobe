// QGlobeDSGeoNameDBManager.cpp : implementation file

#include "QGlobeDSGeoNameDBManager.h"
...
#include "../qglobe_common/qglobe_version.h"

#include "../qglobe_gis/GIS_Polygon.h"
#include "../qglobe_gis/GIS_Area.h"
#include "../qglobe_gis/GIS_Path.h"
...
QGlobeDSGeoNameDBManager::QGlobeDSGeoNameDBManager()
{
#ifdef USE_MYSQL
...
bool QGlobeDSGeoNameDBManager::Initialize()
{
	return connect_mysql() && InitTableData();
}

void QGlobeDSGeoNameDBManager::Release()
{
	ReleaseTableData();
	connect_mysql();
}

bool QGlobeDSGeoNameDBManager::connect_mysql()
{
	if (!m_Database.open())
	{
		QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::connect_mysql.[error : %s]", m_Database.lastError().text().toUtf8().data());
		return false;
	}
...
void QGlobeDSGeoNameDBManager::disconnect_mysql()
{
	if (m_Database.isOpen())
		m_Database.close();
}

bool QGlobeDSGeoNameDBManager::InitTableData()
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
		{
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::InitTableData.[error : %s]", m_Database.lastError().text().toUtf8().data());
			return false;
		}
	}
	...
	if (nRowCount <= 0)
	{
		QGlobeDSLogOutput(E_LOG_ERROR, "DB type table record was empty at QGlobeDSGeoNameDBManager::InitTableData.");
		return false;
	}
	...
	for (int i = 0; i < m_nTableCount; i++)
	{
		...
		{
			qDebug("Program Exited in QGlobeDSGeoNameDBManager::InitTableData");
			qFatal("Error");
		}
...
int QGlobeDSGeoNameDBManager::GetRecordCount(IN char* a_ptTableName)
{
	...
	if (nRowCount <= 0)
	{
		QGlobeDSLogOutput(E_LOG_ERROR, "DB type table record was empty at QGlobeDSGeoNameDBManager::GetRecordCount.");
		return -1;
	}
...
void QGlobeDSGeoNameDBManager::ReleaseTableData()
{
	if (m_pTableList != NULL)
	{
		delete[] m_pTableList;
...
//////////////////////////////////////////////////////////////////////////
bool QGlobeDSGeoNameDBManager::GetGeoIdList(IN ST_REQUEST_SEARCH_IDLIST* a_pRequest, 
									   ...
									   OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
		{
			*a_pErrorCode = E_RESPONSE_ERROR_DATABASE_OPEN;
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::GetGeoNameRecordData.[error : %s]", m_Database.lastError().text().toUtf8().data());
			return false;
		}
	}
	...
	if (bSuccess)
	{
		...
		for (unsigned int i=0; i<nRowCount; i++)
		{
			...
			pRecord->sub_type = (E_QGlobe_SUBDATA_TYPE)oSqlQueryModel.record(i).value(0).toInt();
	...
	else
	{
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Query is invalid at QGlobeDSGeoNameDBManager::GetGeoNameRecordData.[error : %s]", oSqlQueryModel.lastError().text().toUtf8().data());
	}
...
bool QGlobeDSGeoNameDBManager::MakeQueryInSight(IN ST_REQUEST_SEARCH_IDLIST* a_pRequest, OUT QString& a_strQuery)
{
	QString strKeyword;
	...
	for (int i=0; i<m_nTableCount; i++)
	{
		...
		if (m_pTableList[i].type == E_QGlobe_SUBDATA_TYPE_NAME_COUNTRY
			|| m_pTableList[i].type == E_QGlobe_SUBDATA_TYPE_NAME_POPULAR)
		{
...
void QGlobeDSGeoNameDBManager::MakeQueryOutSight(IN ST_REQUEST_SEARCH_IDLIST* a_pRequest, OUT bool* a_pFinished, OUT int* a_pUnionTables , OUT QString& a_strQuery)
{
	int ntble_begin = 0;
	int ntble_end = 0;
...
bool QGlobeDSGeoNameDBManager::MakeQueryNameList(IN ST_REQUEST_SEARCH_DATA* a_pRequest, OUT int* a_pUnionTables, OUT QString& a_strQuery)
{
...
bool QGlobeDSGeoNameDBManager::GetTableInfoFromType(IN int a_nType, OUT int* a_ptbleNo)
{	
	for (int i=0; i<m_nTableCount; i++)
	{
...
bool QGlobeDSGeoNameDBManager::GetGeoNameList(IN ST_REQUEST_SEARCH_DATA* a_pRequest, 
										   ...
										   OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	...
	if (a_pRequest->count < 1){	 //exception
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Request ID count is zero at QGlobeDSGeoNameDBManager::GetGeoNameList");
		return false;
	}
	...
	{
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Invalid Table type at QGlobeDSGeoNameDBManager::GetGeoNameList");
		return false;
	}
	...
	if (bSuccess)
	{
		...
		for (unsigned int i = 0; i < oSqlQueryModel.rowCount(); i++)
		{
			...
			pRecord->type			= (E_QGlobe_SUBDATA_TYPE)oSqlQueryModel.record(i).value(1).toInt();
...
bool QGlobeDSGeoNameDBManager::GetGeoNameRecordData(IN ST_REQUEST_GEONAMESEARCH* a_pRequest, 
...
bool QGlobeDSGeoNameDBManager::GetGeoNameListInTile(IN E_QGlobe_SUBDATA_TYPE a_eSubDataType,
										...
										OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	if (!m_Database.isOpen())
	{
		...
		{
			*a_pErrorCode = E_RESPONSE_ERROR_DATABASE_OPEN;
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::GetGeoNameListInTile.[error : %s]", m_Database.lastError().text().toUtf8().data());
			return false;
		}
	}
	...
	if (bSuccess)
	{
		...
		if (pNameListData->count > GEONAME_MAX_COUNT_IN_TILE)
		{
			...
			QGlobeDSLogOutput(E_LOG_ERROR, "Name count was overflow. [type : %d, level : %d, x : %d, y : %d]", (int)a_eSubDataType, a_nLevel, a_nColumn, a_nRow);
		}
	...
	{
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Query is invalid at QGlobeDSGeoNameDBManager::GetGeoNameListInTile.[error : %s]", oSqlQueryModel.lastError().text().toUtf8().data());
	}
...
bool QGlobeDSGeoNameDBManager::AddGeoNameData(IN ST_ADDNAME_DATA* a_pCommand, 
										 ...
										 OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	if (!m_Database.isOpen())
	{
		...
		{
			*a_pErrorCode = E_RESPONSE_ERROR_DATABASE_OPEN;
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::AddGeoNameData.[error : %s]", m_Database.lastError().text().toUtf8().data());
			return false;
		}
	}
	...
	{
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Query is invalid at QGlobeDSGeoNameDBManager::AddGeoNameData.[error : %s]", oSqlQuery.lastError().text().toUtf8().data());
		return false;
	}
	...
	{
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Invalid Table type at QGlobeDSGeoNameDBManager::AddGeoNameData");
		return false;
	}
...
bool QGlobeDSGeoNameDBManager::UpdateGeoNameData(IN ST_UPDATENAME_DATA* a_pCommand, 
											OUT ST_RESPONSE_DBCOMMAND* a_pResponse, 
											OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	if (!m_Database.isOpen())
	{
		...
		{
			*a_pErrorCode = E_RESPONSE_ERROR_DATABASE_OPEN;
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::UpdateGeoNameData.[error : %s]", m_Database.lastError().text().toUtf8().data());
			return false;
		}
	}
	...
	{
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Query is invalid at QGlobeDSGeoNameDBManager::UpdateGeoNameData.[error : %s]", oSqlQuery.lastError().text().toUtf8().data());
		return false;
	}
	...
	{
		*a_pErrorCode = E_RESPONSE_ERROR_SQL_QUERY;
		QGlobeDSLogOutput(E_LOG_ERROR, "Query is invalid at QGlobeDSGeoNameDBManager::DeleteGeoNameData.[error : %s]", oSqlQuery.lastError().text().toUtf8().data());
		return false;
	}
...
bool QGlobeDSGeoNameDBManager::DeleteGeoNameData(IN ST_DELETENAME_DATA* a_pCommand, 
											OUT ST_RESPONSE_DBCOMMAND* a_pResponse, 
											OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	if (!m_Database.isOpen())
	{
		...
		{
			*a_pErrorCode = E_RESPONSE_ERROR_DATABASE_OPEN;
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::DeleteGeoNameData.[error : %s]", m_Database.lastError().text().toUtf8().data());
			return false;
		}
	}
...
/*
void QGlobeDSGeoNameDBManager::EditMainQueryString(IN ST_REQUEST_GEONAMESEARCH* a_pRequest, 
...
*/
bool QGlobeDSGeoNameDBManager::EditQueryStringByLevel(IN E_QGlobe_SUBDATA_TYPE a_eSubDataType,
												 IN unsigned int a_nLevel,
...
void QGlobeDSGeoNameDBManager::GetCoordinateByLevel(IN unsigned int a_nLevel, 
											   IN unsigned int a_nColumn, 
											   IN unsigned int a_nRow, 
...
bool QGlobeDSGeoNameDBManager::EditAddQueryString(IN ST_GEODATA_NAME* a_pstGeoNameData, OUT QString& a_strQuery)
{
 	int ntble_no;
...
bool QGlobeDSGeoNameDBManager::EditGetMaxIdQueryString(IN ST_GEODATA_NAME* a_pstGeoNameData, OUT QString& a_strQuery)
{
	int ntble_no;
...
bool QGlobeDSGeoNameDBManager::EditUpdateQueryString(IN ST_UPDATENAME_DATA* a_pstUpdateNameData, OUT QString& a_strQuery)
{
 	int ntble_no;
...
bool QGlobeDSGeoNameDBManager::EditDeleteQueryString(IN ST_DELETENAME_DATA* a_pstDelNameData, OUT QString& a_strQuery)
{
	int ntble_no;
...
void QGlobeDSGeoNameDBManager::EncodeKeyword(INOUT QString& a_strKeyword)
{
...
bool QGlobeDSGeoNameDBManager::AddGeoBuildingData(GeoBuildingProperty* p)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
		{
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::AddGeoBuildingData");
			return false;
		}
	}
...
bool QGlobeDSGeoNameDBManager::ExistGeoBuildingData(qint64 id)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
...
bool QGlobeDSGeoNameDBManager::UpdateGeoBuildingData(GeoBuildingProperty* p)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
...
bool QGlobeDSGeoNameDBManager::DeleteGeoBuildingData(quint64 id)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
...
bool QGlobeDSGeoNameDBManager::GetGeoBuildingData(quint64 id,GeoBuildingProperty *p)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
		{
			QGlobeDSLogOutput(E_LOG_ERROR, "Database engine open failed at QGlobeDSGeoNameDBManager::GetGeoBuildingData");
			return false;
		}
	}
...
bool QGlobeDSGeoNameDBManager::GetGeoPostBuildingData(QList<QString>& namelist,QList<int>& idlist)
{
	namelist.clear();
	idlist.clear();
...
bool QGlobeDSGeoNameDBManager::GetGeoBuildingData(QList<QString>& namelist,QList<int>& idlist,QString dong,int ban)
{
	namelist.clear();
	idlist.clear();
...
bool QGlobeDSGeoNameDBManager::GetGeoBuildingPosintion(quint64 id,float& longi,float& lat)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
...
CGIS_Polygon* QGlobeDSGeoNameDBManager::CreateFeature(E_QGlobe_SUBDATA_TYPE type,int id,CGIS_Polygon*& link)
{
	if (!m_Database.isOpen())
	{
		if (!m_Database.open())
	...
	if(type==E_QGlobe_SUBDATA_TYPE_NAME_POLUTANT)
	{
		pt.m_tX=record.field("accidentlong").value().toDouble()*qglobe_DEGTORAD;
		pt.m_tY=record.field("accidentlat").value().toDouble()*qglobe_DEGTORAD;
		...
		if(pt.m_tX!=0 && pt.m_tY!=0)
		{
			...
			pt.m_tX=record.field("longitude").value().toDouble()*qglobe_DEGTORAD;
			pt.m_tY=record.field("latitude").value().toDouble()*qglobe_DEGTORAD;
...
ST_DBTABLEINFO* QGlobeDSGeoNameDBManager::GetTableDef(E_QGlobe_SUBDATA_TYPE type)
{
	int index=0;
...
QSqlRecord QGlobeDSGeoNameDBManager::GetTableData(int type)
{
...
QSqlRecord QGlobeDSGeoNameDBManager::GetTableRecord(E_QGlobe_SUBDATA_TYPE type,int id)
{
	ST_DBTABLEINFO *tabledef=GetTableDef(type);
