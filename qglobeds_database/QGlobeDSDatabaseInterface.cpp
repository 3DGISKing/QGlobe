...
QGlobeDSFileManager* g_FileManager = NULL;
QGlobeDSGeoNameDBManager* g_DBManager = NULL;
...
bool InitializeGDSDatabase()
{
	...
	{
		g_FileManager = new QGlobeDSFileManager(30, 30);
		if (g_FileManager == NULL)
		{
			QGlobeDSLogOutput(E_LOG_ERROR, "new QGlobeDSFileManager failed at InitializeGDSDatabase.");
			return false;
		}

		if (!g_FileManager->Initialize())
		{
			QGlobeDSLogOutput(E_LOG_ERROR, "FileManager Initialize failed at InitializeGDSDatabase.");
	...
	if (g_DBManager == NULL)
	{
		g_DBManager = new QGlobeDSGeoNameDBManager();
		if (g_DBManager == NULL)
		{
				...
				OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	...
	E_QGlobe_MAINDATA_TYPE nMainDataType = a_pRequest->main_type;
	E_QGlobe_SUBDATA_TYPE nSubDataType = a_pRequest->sub_type;

	if (nMainDataType == E_QGlobe_MAINDATA_TYPE_DEM
		|| nMainDataType == E_QGlobe_MAINDATA_TYPE_IMG
		|| nMainDataType == E_QGlobe_MAINDATA_TYPE_SHAPE)
	{
			...
			a_pErrorCode))
		{
			*a_pBytes = sizeof(ST_RESPONSE_GEODATA) - sizeof(unsigned char)*4;
			QGlobeDSLogOutput(E_LOG_ERROR, "GetGeoDataFromBlock failed at GetGeoData. [type : %d, level : %d, x : %d, y : %d]", nMainDataType, nLevel, nColumn, nRow);
			return false;
		}
	}
	else  if(nMainDataType==E_QGlobe_MAINDATA_TYPE_3D || nMainDataType==E_QGlobe_MAINDATA_TYPE_RASTER)
	{
	...
	else if (nMainDataType == E_QGlobe_MAINDATA_TYPE_NAME)
	{
		if (g_DBManager == NULL)
		{
			*a_pBytes = sizeof(ST_RESPONSE_GEODATA) - sizeof(unsigned char)*4;
			*a_pErrorCode = E_RESPONSE_ERROR_NULLDBMANAGER;
			QGlobeDSLogOutput(E_LOG_ERROR, "DBManager is null at GetGeoData.");
			return false;
		}
											...
											a_pErrorCode))
		{
			*a_pBytes = sizeof(ST_RESPONSE_GEODATA) - sizeof(unsigned char)*4;
			...
			QGlobeDSLogOutput(E_LOG_ERROR, "DBManager::GetGeoNameList failed at GetGeoData. [level : %d, x : %d, y : %d]", nLevel, nColumn, nRow);
			return false;
		}
	}
	else
	{
		...
		*a_pErrorCode = E_RESPONSE_ERROR_UNKNOWNGEODATATYPE;
		QGlobeDSLogOutput(E_LOG_ERROR, "GeoData type is unknown at GetGeoData. [type : %d]", nMainDataType);
		return false;
	}
					...
					OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	if (g_DBManager == NULL)
	{
		*a_pErrorCode = E_RESPONSE_ERROR_NULLDBMANAGER;
		...
		QGlobeDSLogOutput(E_LOG_ERROR, "DBManager is null at GetGeoNameList.");
		return false;
	}
