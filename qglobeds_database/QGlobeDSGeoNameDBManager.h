...
public:
	...
	bool GetGeoNameListInTile(IN E_QGlobe_SUBDATA_TYPE a_eSubDataType,
	...
	CGIS_Polygon* CreateFeature(E_QGlobe_SUBDATA_TYPE type,int id,CGIS_Polygon*& p);
	ST_DBTABLEINFO* GetTableDef(E_QGlobe_SUBDATA_TYPE type);

	QSqlDatabase& GetDataBase(){return m_Database;};
	QSqlRecord GetTableData(int type);
	QSqlRecord GetTableRecord(E_QGlobe_SUBDATA_TYPE type,int id);
...
private:
	...
	bool EditQueryStringByLevel(IN E_QGlobe_SUBDATA_TYPE a_eSubDataType,
								IN unsigned int a_nLevel,
