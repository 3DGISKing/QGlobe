...
#include "../qglobe_common/qglobe_distributeconfig.h"
#endif
...
// Argument1		: IN E_QGlobe_MAINDATA_TYPE a_eMainDataType - data type
						 ...
						 OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	...
	if (!GetBlockFileName(a_eMainDataType, a_eSubDataType, a_nLevel, a_nColumn, a_nRow, tFileName))
	{
		*a_pErrorCode = E_RESPONSE_ERROR_FILENAME;
		QGlobeDSLogOutput(E_LOG_ERROR, "Block file name is unknown at GetGeoDataFromBlock! [type : %d]", (int)a_eMainDataType);
		return false;
	}

	unsigned int tHeaderSize = GetBlockFileHeaderSize(a_nLevel);
	QGlobeDSFILEHANDLE tFileHandle = INVALID_GDSFILEHANDLE;
	...
	if (!g_FileManager->OpenFileMap(tFileName, tHeaderSize, &tFileHandle, &bExistFile))
	{
		...
		*a_pErrorCode = E_RESPONSE_ERROR_FILE_OPEN;
		QGlobeDSLogOutput(E_LOG_ERROR, "OpenFileMap failed at GetGeoDataFromBlock! [file : %s]", tFileName.toUtf8().data());
		return false;
	}
	...
	if(!CalcTileOffset(tFileHandle, a_nLevel, a_nColumn, a_nRow, &tOffset, &tSize, a_pErrorCode))
	{
		...
		*a_pErrorCode = E_RESPONSE_ERROR_TILE_NOFOUND;
		QGlobeDSLogOutput(E_LOG_ERROR, "Tile is not found at GetGeoDataFromBlock! [type : %d, level : %d, x : %d, y : %d]", (int)a_eMainDataType, a_nLevel, a_nColumn, a_nRow);
		return false;
	}
	...
	if( !g_FileManager->ReadFileMap(tFileHandle, tOffset, tSize, a_pResponse->buffer) )
	{
		...
		*a_pErrorCode = E_RESPONSE_ERROR_FILE_READ;
		QGlobeDSLogOutput(E_LOG_ERROR, "Tile read was failed at GetGeoDataFromBlock! [type : %d, level : %d, x : %d, y : %d]", (int)a_eMainDataType, a_nLevel, a_nColumn, a_nRow);
		return false;
	}
					...
					OUT E_RESPONSE_ERROR_TYPE* a_pErrorCode)
{
	...
	if (tHeaderSize == 0)
	{
		*a_pErrorCode = E_RESPONSE_ERROR_BLOCKFILE_HEADERSIZE_ZERO;
		QGlobeDSLogOutput(E_LOG_ERROR, "Size of header of block file is zero at CalcTileOffset! [file : %s]", g_FileManager->GetFileName(a_mapHandle).toUtf8().data());
		return false;
	}
	...
	if (a_nLevel != ((ST_BLOCKFILE_HEADER*)tHeaderPtr)->level)
	{
		*a_pErrorCode = E_RESPONSE_ERROR_BLOCKFILE_LEVEL_DIFFERENT;
		QGlobeDSLogOutput(E_LOG_ERROR, "Level of block file is different with request level at CalcTileOffset! [request : %d, file : %d]", a_nLevel, ((ST_BLOCKFILE_HEADER*)tHeaderPtr)->level);
		return false;
	}
	...
	if (tIndex >= ((ST_BLOCKFILE_HEADER*)tHeaderPtr)->count)
	{
		*a_pErrorCode = E_RESPONSE_ERROR_BLOCKFILE_TILE_INDEX;
		QGlobeDSLogOutput(E_LOG_ERROR, "Index of Tile file is bigger at CalcTileOffset! [index : %d, count : %d]", tIndex, ((ST_BLOCKFILE_HEADER*)tHeaderPtr)->count);
		return false;
	}
	...
	*a_pSize = ((ST_BLOCKFILE_HEADER*)tHeaderPtr)->offset_list[tIndex].size;
	...
	{
		*a_pErrorCode = E_RESPONSE_ERROR_BLOCKFILE_HEADERSIZE_ZERO;
		QGlobeDSLogOutput(E_LOG_ERROR, "File size is smaller than offset invalid at CalcTileOffset! [file : %s]", g_FileManager->GetFileName(a_mapHandle).toUtf8().data());
		return false;
	}
