...
void Delauney_Triangulate(CQGlobe_LocationArr &Vertexs,TriangleArray &Triangles);
bool isInTriangle(Vector3d p, QGLOBE_MESH T)
{
...
int
intersect_RayTriangle(Vector3d rayOrg, Vector3d rayDir, QGLOBE_MESH T, Vector3d &destPt, double *distance)
{
...
double Area(CQGlobe_LocationArr &contour)
{
...
bool Snip(CQGlobe_LocationArr &contour,int u,int v,int w,int n,int *V)
{
	int p;
...
void SwapTest(TriangleArray &src, TriangleArray &result,CQGlobe_LocationArr &Vertexs, int index)
{
...
void CheckTriangles(CQGlobe_LocationArr &Vertexs,TriangleArray &Triangles)
{		
...
int isInVertexArray(CQGlobe_LocationArr &pArray, Vector3d pt, int index = 0)
{
	int n = 0;
...
void ValidatePoints(CQGlobe_LocationArr &ptArray)
{
	int n = ptArray.size();
	for (int i=0;i<n;i++)
	{		
...
bool isNearPoint(CQGlobe_LocationArr &pArray,Vector3d pa,Vector3d &pb)
{
	int k,n = pArray.size();
	for (int i = 0;i < n;i++)
	{
...
void CheckIntersection(CQGlobe_LocationArr &ptArray,CQGlobe_LocationArr &iArray)
{	
	iArray.set_used(0);
	int n = ptArray.size();
...
int getClosedPolygon(CQGlobe_LocationArr &pArray, CQGlobe_LocationArr &iArray, CQGlobe_LocationArr &result, int base = 0)
{
	int n = pArray.size();
	bool flag = 0;
	int i, j, index;
...
bool NormalTriangluate(CQGlobe_LocationArr &ptArray,TriangleArray &result)
{
...
void polygon2Polygons(CQGlobe_LocationArr &pArray,CQGlobe_LocationPolygonArr &polyArray)
{
	int n = (int)pArray.size();
	if ( n < 3 ) 
		return ;
		
	CQGlobe_LocationArr ptArray = pArray,iArray(n);
...
bool polygonTriangulate(CQGlobe_LocationArr &contour,Triangle3DArray &triArray)
{
	int n = (int)contour.size();
	...
	CQGlobe_LocationPolygonArr polyArray;
	polygon2Polygons(contour,polyArray);

	CQGlobe_LocationArr parray;	
...
bool IsValidTriangle(CQGlobe_LocationArr &Vertexs, Triangle &tri)
{
...
void CheckPointOrder2(CQGlobe_LocationArr &Vertexs,TriangleArray &Triangles)
{
	Triangle tri;	
	Edge edge, *e1, *e2;
