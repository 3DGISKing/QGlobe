...
#define _QGlobe_ARRAY_H_

#include "QGlobe_Heapsort.h"
#include "QGlobe_Allocator.h"
...
public:
	...
	//! Default constructor for empty qglobe_array.
	qglobe_array()
	...
	//! Constructs an qglobe_array and allocates an initial chunk of memory.
	/** \param start_count Amount of elements to pre-allocate. */
	qglobe_array(unsigned int start_count)
		: data(0), allocated(0), used(0),
			free_when_destroyed(true), is_sorted(true)
	{
	...
	//! Copy constructor
	qglobe_array(const qglobe_array<T>& other)
		: data(0)
	{
		*this = other;
	}
	...
	//! Reallocates the qglobe_array, make it bigger or smaller.
	/** \param new_size New size of qglobe_array. */
	...
	//! Adds an element at back of qglobe_array.
	/** If the qglobe_array is too small to add this new element it is made bigger.
	\param element: Element to add at the back of the qglobe_array. */
	void push_back(const T& element)
	{
		if (used + 1 > allocated)
		{
			// reallocate(used * 2 +1);
			// this doesn't work if the element is in the same qglobe_array. So
	...
	//! Adds an element at the front of the qglobe_array.
	/** If the qglobe_array is to small to add this new element, the qglobe_array is
	made bigger. Please note that this is slow, because the whole qglobe_array
	...
	//! Insert item into qglobe_array at specified position.
	...
	//! Clears the qglobe_array and deletes all allocated memory.
	void clear()
	{
		for (unsigned int i=0; i<used; ++i)
			allocator.destruct(&data[i]);
	...
	//! Sets pointer to new qglobe_array, using this as new workspace.
	/** \param newPointer: Pointer to new qglobe_array of elements.
	\param size: Size of the new qglobe_array. */
	...
	//! Sets if the qglobe_array should delete the memory it uses upon destruction.
	/** \param f If true, the qglobe_array frees the allocated memory in its
	...
	//! Sets the size of the qglobe_array and allocates new elements if necessary.
	...
	//! Assignment operator
	void operator=(const qglobe_array<T>& other)
	{
		if (data)
		{
			for (unsigned int i=0; i<used; ++i)
	...
	//! Equality operator
	bool operator == (const qglobe_array<T>& other) const
	{
		if (used != other.used)
			return false;
	...
	//! Gets a pointer to the qglobe_array.
	/** \return Pointer to the qglobe_array. */
	T* pointer()
	{
		return data;
	}
	...
	//! Gets a const pointer to the qglobe_array.
	/** \return Pointer to the qglobe_array. */
	...
	//! Get size of qglobe_array.
	/** \return Size of elements used in the qglobe_array. */
	...
	//! Check if qglobe_array is empty.
	/** \return True if the qglobe_array is empty false if not. */
	bool empty() const
	{
		return used == 0;
	}
	...
	//! Sorts the qglobe_array using heapsort.
	...
	/** The qglobe_array will be sorted before the binary search if it is not
	already sorted.
	...
	//! Performs a binary search for an element, returns -1 if not found.
	/** The qglobe_array must be sorted prior
	\param element Element to search for.
	...
	int binary_search(const T& element, int left, int right) const
	{
		...
		// " while((element != qglobe_array[m]) && left<=right);"
		// but we only want to use the '<' operator.
		// the same in next line, it is "(element == qglobe_array[m])"
	...
	//! Erases an element from the qglobe_array.
	...
	//! Erases some elements from the qglobe_array.
	/** May be slow, because all elements following after the erased
	element have to be copied.
	...
	//! Sets if the qglobe_array is sorted
