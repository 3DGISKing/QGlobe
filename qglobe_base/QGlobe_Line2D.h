#ifndef __QGLOBE_LINE_2D_H_INCLUDED__
#define __QGLOBE_LINE_2D_H_INCLUDED__
...
template <class T>
class CQGlobe_Line2D
{
	public:
		//! Default constructor for a_cLine going from (0,0) to (1,1).
		CQGlobe_Line2D() : m_cStart(0,0), m_cEnd(1,1) {}
		//! Constructor for a_cLine between the two points.
		CQGlobe_Line2D(T a_tXa, T a_tYa, T a_tXb, T a_tYb) : m_cStart(a_tXa, a_tYa), m_cEnd(a_tXb, a_tYb) {}
		//! Constructor for a_cLine between the two points given as vectors.
		CQGlobe_Line2D(const CQGlobe_Vector2D<T>& a_cStart, const CQGlobe_Vector2D<T>& a_cEnd) : m_cStart(a_cStart), m_cEnd(a_cEnd) {}
		//! Copy constructor.
		CQGlobe_Line2D(const CQGlobe_Line2D<T>& a_cOther) : m_cStart(a_cOther.m_cStart), m_cEnd(a_cOther.m_cEnd) {}

		// operators

		CQGlobe_Line2D<T> operator+(const CQGlobe_Vector2D<T>& a_cPoint) const { return CQGlobe_Line2D<T>(m_cStart + a_cPoint, m_cEnd + a_cPoint); }
		CQGlobe_Line2D<T>& operator+=(const CQGlobe_Vector2D<T>& a_cPoint) { m_cStart += a_cPoint; m_cEnd += a_cPoint; return *this; }

		CQGlobe_Line2D<T> operator-(const CQGlobe_Vector2D<T>& a_cPoint) const { return CQGlobe_Line2D<T>(m_cStart - a_cPoint, m_cEnd - a_cPoint); }
		CQGlobe_Line2D<T>& operator-=(const CQGlobe_Vector2D<T>& a_cPoint) { m_cStart -= a_cPoint; m_cEnd -= a_cPoint; return *this; }

		bool operator==(const CQGlobe_Line2D<T>& a_cOther) const
		...
		//! Set this a_cLine to new a_cLine going through the two points.
		void setLine(const CQGlobe_Vector2D<T>& nstart, const CQGlobe_Vector2D<T>& nend){m_cStart.set(nstart); m_cEnd.set(nend);}
		//! Set this a_cLine to new a_cLine given as parameter.
		void setLine(const CQGlobe_Line2D<T>& a_cLine){m_cStart.set(a_cLine.m_cStart); m_cEnd.set(a_cLine.m_cEnd);}

		//! Get length of a_cLine
		/** \return Length of the a_cLine. */
		QGLOBE_DOUBLE getLength() const { return m_cStart.getDistanceFrom(m_cEnd); }
		...
		CQGlobe_Vector2D<T> getMiddle() const
		{
		...
		CQGlobe_Vector2D<T> getVector() const { return CQGlobe_Vector2D<T>(m_cStart.X - m_cEnd.X, m_cStart.Y - m_cEnd.Y); }
		...
		bool intersectWith(const CQGlobe_Line2D<T>& a_cLine, CQGlobe_Vector2D<T>& a_cLineOut) const
		{
		...
		*/
		...
		bool intersectWith(const CQGlobe_Line2D<T>& a_cLine, CQGlobe_Vector2D<T>& a_cLineOut, double tolerance) const
			{
			// Uses the method given at:
			// http://local.wasp.uwa.edu.au/~pbourke/geometry/lineline2d/ 
			...
			if(qglobe_equals(commonDenominator, 0.0, tolerance))
				{ 
				// The lines are either coincident or parallel
				if(qglobe_equals(numeratorA, 0.0, tolerance) && qglobe_equals(numeratorB, 0.0, tolerance))
					{
					// Try and find a common endpoint
					if(a_cLine.m_cStart == m_cStart || a_cLine.m_cEnd == m_cStart)
						a_cLineOut = m_cStart;
		...
		CQGlobe_Vector2D<T> getUnitVector() const
		{
			T len = (T)(1.0 / getLength());
			return CQGlobe_Vector2D<T>((m_cEnd.X - m_cStart.X) * len, (m_cEnd.Y - m_cStart.Y) * len);
		}

		//! Get angle between this a_cLine and given a_cLine.
		/** \param a_cLine Other a_cLine for test.
		\return Angle in degrees. */
		QGLOBE_DOUBLE getAngleWith(const CQGlobe_Line2D<T>& a_cLine) const
		{
			CQGlobe_Vector2D<T> vect = getVector();
			CQGlobe_Vector2D<T> vect2 = a_cLine.getVector();
		...
		T getPointOrientation(const CQGlobe_Vector2D<T>& a_cPoint) const
		{
		...
		bool isPointOnLine(const CQGlobe_Vector2D<T>& a_cPoint) const
		{
		...
		bool isPointBetweenStartAndEnd(const CQGlobe_Vector2D<T>& a_cPoint) const
		{
			return a_cPoint.isBetweenPoints(m_cStart, m_cEnd);
		}

		//! Get the closest a_cPoint on this a_cLine to a a_cPoint
		CQGlobe_Vector2D<T> getClosestPoint(const CQGlobe_Vector2D<T>& a_cPoint) const
		{
			CQGlobe_Vector2D<T> c = a_cPoint - m_cStart;
			CQGlobe_Vector2D<T> v = m_cEnd - m_cStart;
			T d = (T)v.getLength();
			v /= d;
			T t = v.dotProduct(c);
		...
		CQGlobe_Vector2D<T> m_cStart;
		//! End a_cPoint of the a_cLine.
		CQGlobe_Vector2D<T> m_cEnd;
};

	//! Typedef for an double a_cLine.
	typedef CQGlobe_Line2D<double> CQGlobe_Line2DF;
	//! Typedef for an integer a_cLine.
	typedef CQGlobe_Line2D<QGLOBE_INT> CQGlobe_Line2DI;

#endif // __QGLOBE_LINE_2D_H_INCLUDED__
