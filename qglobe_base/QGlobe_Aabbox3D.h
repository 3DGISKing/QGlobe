#ifndef __QGLOBE_AABBOX_3D_H_INCLUDED__
#define __QGLOBE_AABBOX_3D_H_INCLUDED__
...
*/
template <class T>
class CQGlobe_Aabbox3D
{
	public:

		//! Default Constructor.
		CQGlobe_Aabbox3D(): MinEdge(-1,-1,-1), MaxEdge(1,1,1) {}
		//! Constructor with min edge and max edge.
		CQGlobe_Aabbox3D(const CQGlobe_Vector3D<T>& min, const CQGlobe_Vector3D<T>& max): MinEdge(min), MaxEdge(max) {}
		//! Constructor with only one point.
		CQGlobe_Aabbox3D(const CQGlobe_Vector3D<T>& init): MinEdge(init), MaxEdge(init) {}
		//! Constructor with min edge and max edge as single values, not vectors.
		CQGlobe_Aabbox3D(T minx, T miny, T minz, T maxx, T maxy, T maxz): MinEdge(minx, miny, minz), MaxEdge(maxx, maxy, maxz) {}

		// operators
		//! Equality operator
		/** \param other box to compare with.
		\return True if both boxes are equal, else false. */
		inline bool operator==(const CQGlobe_Aabbox3D<T>& other) const { return (MinEdge == other.MinEdge && other.MaxEdge == MaxEdge);}
		//! Inequality operator
		/** \param other box to compare with.
		...
		void addInternalPoint(const CQGlobe_Vector3D<T>& p)
		{
		...
		void addInternalBox(const CQGlobe_Aabbox3D<T>& b)
		{
		...
		void reset(const CQGlobe_Aabbox3D<T>& initValue)
		{
			*this = initValue;
		}
		...
		void reset(const CQGlobe_Vector3D<T>& initValue)
		{
			MaxEdge = initValue;
			MinEdge = initValue;
		}
		...
		/** \param p: Point to check.
		\return True if the point is within the box and false if not. */
		bool isPointTotalInside(const CQGlobe_Vector3D<T>& p) const
		{
			return (p.m_tX > MinEdge.m_tX && p.m_tX < MaxEdge.m_tX &&
				p.m_tY > MinEdge.m_tY && p.m_tY < MaxEdge.m_tY &&
				p.m_tZ > MinEdge.m_tZ && p.m_tZ < MaxEdge.m_tZ);
		...
		otherwise false. */
		bool intersectsWithBox(const CQGlobe_Aabbox3D<T>& other) const
		{
		...
		otherwise false. */
		bool isFullInside(const CQGlobe_Aabbox3D<T>& other) const
		{
		...
		bool intersectsWithLine(const CQGlobe_Line3D<T>& line) const
		{
		...
		\return True if there is an intersection, else false. */
		bool intersectsWithLine(const CQGlobe_Vector3D<T>& linemiddle,
					const CQGlobe_Vector3D<T>& linevect,
					T halflength) const
		{
			const CQGlobe_Vector3D<T> e = getExtent() * (T)0.5;
			const CQGlobe_Vector3D<T> t = getCenter() - linemiddle;
		...
		EIntersectionRelation3D classifyPlaneRelation(const CQGlobe_Plane3D<T>& plane) const
		{
			CQGlobe_Vector3D<T> nearPoint(MaxEdge);
			CQGlobe_Vector3D<T> farPoint(MinEdge);
		...
		CQGlobe_Vector3D<T> getCenter() const
		{
		...
		CQGlobe_Vector3D<T> getExtent() const
		{
		...
		void getEdges(CQGlobe_Vector3D<T> *edges) const
		{
			const CQGlobe_Vector3D<T> middle = getCenter();
			const CQGlobe_Vector3D<T> diag = middle - MaxEdge;
		...
		CQGlobe_Aabbox3D<T> getInterpolated(const CQGlobe_Aabbox3D<T>& other, double d) const
		{
			double inv = 1.0f - d;
			return CQGlobe_Aabbox3D<T>((other.MinEdge*inv) + (MinEdge*d),
		...
		T getVolume() const
		{
			const CQGlobe_Vector3D<T> e = getExtent();
		...
		//! The near edge
		CQGlobe_Vector3D<T> MinEdge;
		//! The far edge
		CQGlobe_Vector3D<T> MaxEdge;
};

	typedef CQGlobe_Aabbox3D<double> Aabbox3DF;
	//! Typedef for a double 3d bounding box.
	typedef CQGlobe_Aabbox3D<double> CQGlobe_Aabbox3DF;
	//! Typedef for an integer 3d bounding box.
	typedef CQGlobe_Aabbox3D<QGLOBE_INT> CQGlobe_Aabbox3DI;

#endif // __QGLOBE_AABBOX_3D_H_INCLUDED__
