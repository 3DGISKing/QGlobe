...
// B,L,H�Ṣ�� �鰢������˵�̩ ����
void qglobe_GetPosition(IN QGlobe_LOCATION *pLoc , OUT CQGlobe_Point3DF* pPosF)
{
...
// �鰢������Ṣ�� B,L,H��̩ ����
void qglobe_GetLocation(IN CQGlobe_Point3DF* pPosF, OUT QGlobe_LOCATION *pLoc)
{
	...
	{
		if(pPosF->m_tY < 0.0f)
			pLoc->m_dLongitude -= qglobe_PI;
		else if(pPosF->m_tY == 0.0f)
		{
			pLoc->m_dLongitude = pPosF->m_tX > 0.0f ? 0 : qglobe_PI;
		}
	}
	else
	{
...
// �鰢������Ṣ�� B,L,H��̩ ���� (˧̩ �ܺ�̮ �ĵ������� �޶���.)
void qglobe_GetLocation(IN CQGlobe_Point3DF* pPosF, OUT CQGlobe_Location3D *pLoc)
{
	double projXY = sqrt(pPosF->m_tX * pPosF->m_tX + pPosF->m_tY * pPosF->m_tY);
	double dist = pPosF->getLength();
	...
	if(pLoc->m_tX > 0)
	{
		...
		{
			pLoc->m_tX = pPosF->m_tX > 0.0f ? 0 : qglobe_PI;
		}
	}
	else
	{
...
void qglobe_RotateAroundVector(CQGlobe_Point3DF *pPt, double angle, CQGlobe_Vector3DF vt)
{
...
void qglobe_GetAngleBetweenTwoVector(CQGlobe_Vector3DF vt1, CQGlobe_Vector3DF vt2 , CQGlobe_Vector3DF upVt, double &angle)
{
	vt1.normalize();
	vt2.normalize();
	upVt.normalize();
	...
	CQGlobe_Vector3DF vt = vt1.crossProduct(vt2);
...
void qglobe_GetPerpendPt(CQGlobe_Point3DF* pResult, const CQGlobe_Point3DF* pPt , const CQGlobe_Point3DF* pOrg , const CQGlobe_Vector3DF* pVt)
{
	CQGlobe_Vector3DF vt = *pPt - *pOrg;
	CQGlobe_Vector3DF vt1 = *pVt;
	vt1.normalize();
	double t;
...
void qglobe_RotateAroundVector(QGlobe_Coord3D *pCoord, double angle, CQGlobe_Vector3DF vt)
{
	CQGlobe_Quaternion q;
	CQGlobe_Matrix4 mat;
	vt.normalize();
...
void qglobe_RotateAroundLine(CQGlobe_Point3DF *pPt, double angle, CQGlobe_Point3DF org , CQGlobe_Vector3DF vt)
{
	vt.normalize();
	CQGlobe_Quaternion q;
	CQGlobe_Matrix4 mat;
	q.fromAngleAxis(angle , vt);
	q.getMatrix(mat);
	CQGlobe_Vector3DF _vt;
	_vt = *pPt - org;
	mat.transformVect(_vt);
	*pPt = org + _vt;
}

void qglobe_RotateAroundLine(QGlobe_Coord3D *pCoord, double angle, CQGlobe_Point3DF org , CQGlobe_Vector3DF vt)
{
	vt.normalize();
	CQGlobe_Quaternion q;
	CQGlobe_Matrix4 mat;
...
void qglobe_GetQuaternion(IN QGlobe_Coord3D *pCoord, OUT CQGlobe_Quaternion* pQuaternion)
{
	CQGlobe_Matrix4 mat;
	qglobe_MatrixSCStoTCS(pCoord , &mat);
	*pQuaternion = mat;
}

void qglobe_SetQuaternion(CQGlobe_Quaternion* pQuaternion , QGlobe_Coord3D *pCoord)
{
	CQGlobe_Matrix4 mat;
...
void qglobe_MatrixSCStoTCS(IN QGlobe_Coord3D *pCoord, OUT CQGlobe_Matrix4 *pMat)
{
	CQGlobe_Vector3DF mx , my , mz;
...
void qglobe_MatrixTCStoSCS(QGlobe_Coord3D *pCoord, CQGlobe_Matrix4 *pMat)
{
	CQGlobe_Vector3DF mx , my , mz;
	CQGlobe_Vector3DF org;
	org = pCoord->m_org;
...
void qglobe_RotateAroundXAxis(QGlobe_Coord3D *pCoord, double angle)
{
	QGlobe_Coord3D tcoord = *pCoord;
...
void qglobe_RotateAroundYAxis(QGlobe_Coord3D *pCoord, double angle)
{
	QGlobe_Coord3D tcoord = *pCoord;
...
void qglobe_RotateAroundZAxis(QGlobe_Coord3D *pCoord, double angle)
{
	QGlobe_Coord3D tcoord = *pCoord;
...
void qglobe_TCStoSCS(QGlobe_Coord3D *pCoord, CQGlobe_Point3DF *pnt)
{
	CQGlobe_Point3DF ret;
...
void qglobe_VectorTCStoSCS(QGlobe_Coord3D *pCoord, CQGlobe_Vector3DF *pVtr)
{
	CQGlobe_Vector3DF ret;
...
void qglobe_SCStoTCS(QGlobe_Coord3D *pCoord, CQGlobe_Point3DF* pnt)
{
	CQGlobe_Vector3DF vtr = *pnt - pCoord->m_org;
	pnt->m_tX = vtr.dotProduct(pCoord->m_x);
	pnt->m_tY = vtr.dotProduct(pCoord->m_y);
	pnt->m_tZ = vtr.dotProduct(pCoord->m_z);
}

void qglobe_VectorSCStoTCS(QGlobe_Coord3D *pCoord, CQGlobe_Vector3DF *pVtr)
{
	CQGlobe_Vector3DF ret;
