...
bool GDSServiceController::start(const QStringList &arguments)
{
    if (!isInstalled())
        return false;
    if (isRunning())
...
QtServiceSysPrivate::QtServiceSysPrivate()
    : GDSUnixServerSocket(), ident(0), serviceFlags(0)
{
}
...
void QtServiceSysPrivate::slotReady()
{
	...
	if (cmd == QLatin1String("terminate")) {
            if (!(serviceFlags & GDSServiceBase::CannotBeStopped)) {
                GDSServiceBase::instance()->stop();
                QCoreApplication::instance()->quit();
                retValue = true;
            }
        } else if (cmd == QLatin1String("pause")) {
            if (serviceFlags & GDSServiceBase::CanBeSuspended) {
	    ...
	    cmd = cmd.mid(4);
            GDSServiceBase::instance()->processCommand(cmd.toInt());
            retValue = true;
	}
        QString retString;
        if (retValue)
...
void GDSServiceBase::logMessage(const QString &message, GDSServiceBase::MessageType type,
			    int, uint, const QByteArray &)
{
    ...
    int st;
    switch(type) {
        case GDSServiceBase::Error:
	    st = LOG_ERR;
	    break;
        case GDSServiceBase::Warning:
            st = LOG_WARNING;
	    break;
        default:
	    st = LOG_INFO;
