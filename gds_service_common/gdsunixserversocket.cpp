...
GDSUnixServerSocket::GDSUnixServerSocket(const QString &path, QObject *parent)
    : QTcpServer(parent)
{
    setPath(path);
}

GDSUnixServerSocket::GDSUnixServerSocket(QObject *parent)
    : QTcpServer(parent)
{
}

void GDSUnixServerSocket::setPath(const QString &path)
{
    path_.clear();
...
void GDSUnixServerSocket::close()
{
    QTcpServer::close();
    if (!path_.isEmpty()) {
