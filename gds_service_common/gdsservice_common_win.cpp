...
bool GDSServiceController::isInstalled() const
{
...
bool GDSServiceController::isRunning() const
{
    Q_D(const GDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
QString GDSServiceController::serviceFilePath() const
{
    Q_D(const GDSServiceController);
    QString result;
    if (!winServiceInit())
...
QString GDSServiceController::serviceDescription() const
{
    Q_D(const GDSServiceController);
    QString result;
    if (!winServiceInit())
...
GDSServiceController::StartupType GDSServiceController::startupType() const
{
    Q_D(const GDSServiceController);
...
bool GDSServiceController::uninstall()
{
...
bool GDSServiceController::start(const QStringList &args)
{
    Q_D(GDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
bool GDSServiceController::stop()
{
    Q_D(GDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
bool GDSServiceController::pause()
{
    Q_D(GDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
bool GDSServiceController::resume()
{
    Q_D(GDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
bool GDSServiceController::sendCommand(int code)
{
   Q_D(GDSServiceController);
   bool result = false;
   if (!winServiceInit())
...
void GDSServiceBase::logMessage(const QString &message, MessageType type,
...
class GDSServiceControllerHandler : public QObject
{
    Q_OBJECT
public:
    GDSServiceControllerHandler(QtServiceSysPrivate *sys);
...
public:
    ...
    void setServiceFlags(GDSServiceBase::ServiceFlags flags);
    DWORD serviceFlags(GDSServiceBase::ServiceFlags flags) const;
    ...
    GDSServiceControllerHandler *controllerHandler;

    void handleCustomEvent(QEvent *e);
};

GDSServiceControllerHandler::GDSServiceControllerHandler(QtServiceSysPrivate *sys)
    : QObject(), d_sys(sys)
{

}

void GDSServiceControllerHandler::customEvent(QEvent *e)
{
...
{
    if (!instance || !GDSServiceBase::instance())
        return;
    ...
    instance->serviceStatus = pRegisterServiceCtrlHandler((TCHAR*)GDSServiceBase::instance()->serviceName().utf16(), handler);
    ...
    handler(QTSERVICE_STARTUP); // Signal startup to the application -
                                // causes GDSServiceBase::start() to be called in the main thread
...
void QtServiceSysPrivate::handleCustomEvent(QEvent *e)
{
    ...
    case QTSERVICE_STARTUP: // Startup
        GDSServiceBase::instance()->start();
        break;
    case SERVICE_CONTROL_STOP:
        GDSServiceBase::instance()->stop();
        QCoreApplication::instance()->quit();
        break;
    case SERVICE_CONTROL_PAUSE:
        GDSServiceBase::instance()->pause();
        break;
    case SERVICE_CONTROL_CONTINUE:
        GDSServiceBase::instance()->resume();
        break;
    default:
	if (code >= 128 && code <= 255)
	    GDSServiceBase::instance()->processCommand(code - 128);
        break;
    }
...
void WINAPI QtServiceSysPrivate::handler( DWORD code )
{
    ...
    instance->mutex.lock();
    switch (code) {
    case QTSERVICE_STARTUP: // GDSService startup (called from WinMain when started)
...
void QtServiceSysPrivate::setServiceFlags(GDSServiceBase::ServiceFlags flags)
{
    if (!available())
        return;
...
DWORD QtServiceSysPrivate::serviceFlags(GDSServiceBase::ServiceFlags flags) const
{
    DWORD control = 0;
    if (flags & GDSServiceBase::CanBeSuspended)
        control |= SERVICE_ACCEPT_PAUSE_CONTINUE;
    if (!(flags & GDSServiceBase::CannotBeStopped))
    ...
    void run()
        {
            ...
            st[0].lpServiceName = (wchar_t*)GDSServiceBase::instance()->serviceName().utf16();
            ...
            if (!success) {
                if (GetLastError() == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
                    ...
                    GDSServiceBase::instance()->logMessage(QString("The Service failed to start [%1]").arg(qt_error_string(GetLastError())), GDSServiceBase::Error);
                }
...
bool GDSServiceBasePrivate::start()
{
    sysInit();
    if (!winServiceInit())
        return false;
...
bool GDSServiceBasePrivate::install(const QString &account, const QString &password)
{
    bool result = false;
    if (!winServiceInit())
        return result;
    ...
    if (hSCM) {
        QString acc = account;
        DWORD dwStartType = startupType == GDSServiceController::AutoStartup ? SERVICE_AUTO_START : SERVICE_DEMAND_START;
...
QString GDSServiceBasePrivate::filePath() const
{
...
bool GDSServiceBasePrivate::sysInit()
{
...
void GDSServiceBasePrivate::sysSetPath()
{

}

void GDSServiceBasePrivate::sysCleanup()
{
    if (sysd) {
        delete sysd;
        sysd = 0;
...
void GDSServiceBase::setServiceFlags(GDSServiceBase::ServiceFlags flags)
{
