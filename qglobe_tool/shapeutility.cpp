...
#include <QDir>
#include <stdio.h>
#include "../qglobe_gis/GIS_funcs.h"
#include <QRectF>
#include <QFileInfo>
#include <QMessageBox>
...
{	
	...
	double tileWidth = 360.0f / (1 << level);//���̼� ��̡�溷 ����̩ �����μ�
	...
	for (int nY = startY; nY < endY; nY++)
	{
		for (int nX = startX; nX < endX; nX++)
		{
			...
			CQGlobe_LocationPolygonArr oneTilePolyArr;
			QVector<int>   idArr;    
			oneTilePolyArr.clear();
			idArr.clear();
			QVector<CQGlobe_Rect2D> boundingArr;

			//���� �綮���� ������ 
			for (int i=0; i<pPolyData.size(); i++)
			{
				CQGlobe_LocationArr inPtArr;
				CQGlobe_LocationPolygonArr outLineArr;
				...
				if (isIntersecting(poly.polyHd.BXmin, poly.polyHd.BXmax, poly.polyHd.BYmin, poly.polyHd.BYmax, minX, maxX, minY, maxY))
				{
					...
					for(int k=0; k<poly.polyHd.nPointCount; k++)
					{
						...
						CQGlobe_Location3D pPt;
						pPt.set(pt.x, pt.y, 0);
						inPtArr.push_back(pPt);
					}

					//�� �綮��˾ ����̩ �ұ�4����˺�� ������ 
					qglobe_GetLineArrInRectangle(inPtArr, minX, maxX, minY, maxY, outLineArr);

					//������ �綮�ش�˾ ����� ����ױ�
					for (int nLine=0; nLine<outLineArr.size(); nLine++)
					{
						CQGlobe_LocationArr line = outLineArr[nLine];
						oneTilePolyArr.push_back(line);

						//������ ���� ɷ����̩ id�� ��� 

						idArr.push_back(i+1);//id�� 1���� �������ް� �����ٳ�.
					}
				}
			}
...
void GetBoundingRect(CQGlobe_LocationArr ptArr, CQGlobe_Rect2D *bound)
{
	double minX, maxX, minY, maxY;
...
bool WriteShapeFile(int x,int y,int level, CQGlobe_LocationPolygonArr polyarray, QString dstDir,QVector<int> idArr, 
					bool hasID, int maxFSize)
{
	...
	if(!tmp.exists(dirName))    //��θ� �����Ѵ�
		tmp.mkpath(dirName);


	QFile file;
	file.setFileName(filename);
	if(!file.open(QIODevice::WriteOnly)) 	return false; //���ο� ������ â���Ѵ�
	...
	out<<(int)polygonarray.size();                            //�Ķ���ͷ� ���� �����︸ ���� ���
	fileSize = fileSize + sizeof(int);
	CQGlobe_Location3D point;                                       //���� �Ķ���ͷ� ���� �����︸ ����
	...
	{
		CQGlobe_LocationArr &polygon = polygonarray[npoly];

			
		out<<(int)polygon.size();
		fileSize = fileSize + sizeof(int);
		//2012 6 12 added by ugi Shape����̩ �鼿 ����
		if (hasID)
			out<<(int)idArr[npoly];
		fileSize = fileSize + sizeof(int);
		//end
...
//���̼� ��̡�溷 ̩̩�� �ֲ� (���� ��˼ ���˶� ����� )���˾ �����ٳ�.
CQGlobe_LocationPolygonArr GetReducedPolygonArray(CQGlobe_LocationPolygonArr inArray, int level)
{
	CQGlobe_LocationPolygonArr res;
	double num_pix_tol;
	if (level < 7)
		num_pix_tol = 3.0f;
	else if (level < 9)
	...
	{
		CQGlobe_LocationArr &polygon = inArray[nPoly];
		CQGlobe_LocationArr line;
		CQGlobe_Location3D pPt, oldPt, upPt;
		CQGlobe_Location3D fVector, sVector;
		...
		for (int nPoint=0; nPoint<polygon.size()-1; nPoint++)
		{
			...
			CQGlobe_Location3D fVector = oldPt - pPt;        //Vector pointing from present point to former one.
			CQGlobe_Location3D sVector = upPt  - pPt;		   //Vector pointing from present point to next one.
...
bool ReadShapeFile(QString filename,CQGlobe_LocationPolygonArr& linearray)
{
	QFile shapefile(filename);
...
bool WriteShapeFile(int x,int y,int level, CQGlobe_LocationPolygonArr polygonarray, bool hasID)
{
	QString maintype="";
	...
	CQGlobe_LocationPolygonArr polygonarray_on_disk;
	...
	{
		CQGlobe_LocationArr &polygon = polygonarray_on_disk[npoly];
