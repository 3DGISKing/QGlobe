...
#include "../qglobe_libmain/building.h"	
...
int SmpBuildingProDlg::exportOneShapeTile(QList<OGRFeature*> & featurelist,QGlobe_TILE_INFO tileInfo)
{
	double tileWidth = qglobe_TwoPI / (1 << tileInfo.nLevel);//����̩ ��������

	CQGlobe_Rect2D tileBox; //����̩ �ұ�4����μ�
	tileBox.m_cMinPt.m_tX = tileWidth * tileInfo.nX - qglobe_PI;
	tileBox.m_cMinPt.m_tY = tileWidth * tileInfo.nY - qglobe_PI;
	...
	for(unsigned index = 0; index < featurelist.size(); index++) 
	{
		...
		if(!tileBox.isPointInside(x*qglobe_DEGTORAD,y*qglobe_DEGTORAD))
			continue;

		double area=(featureenvelop.MaxX-featureenvelop.MinX)*(featureenvelop.MaxY-featureenvelop.MinY)*qglobe_DEGTORAD*qglobe_DEGTORAD;
		...
		out<<featureenvelop.MinX*qglobe_DEGTORAD;
		out<<featureenvelop.MinY*qglobe_DEGTORAD;
		out<<featureenvelop.MaxX*qglobe_DEGTORAD;
		out<<featureenvelop.MaxY*qglobe_DEGTORAD;
		...
		if(geometry->getGeometryType()==wkbMultiPolygon25D || geometry->getGeometryType()==wkbMultiPolygon)
		{
			...
			{  
				...
				for(int j=0;j<ring->getNumPoints()-1;j++)
				{
					...
					out<<point.getX()*qglobe_DEGTORAD;
					out<<point.getY()*qglobe_DEGTORAD;
				}
...
void SmpBuildingProDlg::updateBuildingTable()
{
	for(int i=0;i<_featureList.size();i++)
	{
		...
		strQuery.sprintf(strQuery.toUtf8().data(),
			ID,
			envelope.MinX*qglobe_DEGTORAD,
			envelope.MaxX*qglobe_DEGTORAD,
			envelope.MinY*qglobe_DEGTORAD,
			envelope.MaxY*qglobe_DEGTORAD,
			name.toUtf8().data(),
...
{
	double tileWidth;
	QGlobe_TILE_INFO	startInfo, endInfo,tileInfo;
	...
	for(int level=THREED_MIN_LEVEL;level<=THREED_MAX_LEVEL;level++)
	{
		...
		tileWidth = qglobe_TwoPI / (1 << level);

		OGREnvelope Bound;   

		_layer->GetExtent(&Bound);

		startInfo.nX = (int)((Bound.MinX*qglobe_DEGTORAD + qglobe_PI) / tileWidth);
		startInfo.nY = (int)((Bound.MinY*qglobe_DEGTORAD + qglobe_PI) / tileWidth);
		endInfo.nX = (int)((Bound.MaxX*qglobe_DEGTORAD + qglobe_PI) / tileWidth);
		endInfo.nY = (int)((Bound.MaxY*qglobe_DEGTORAD + qglobe_PI) / tileWidth);
