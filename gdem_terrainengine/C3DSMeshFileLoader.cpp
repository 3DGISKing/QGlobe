...
{
	return new	C3DSMeshFileLoader(fs);
}


//! Constructor
C3DSMeshFileLoader::C3DSMeshFileLoader( io::IFileSystem* fs)
		...
		for (u32 i=0; i<Mesh->getMeshBufferCount(); ++i)
		{
			...
			else
			{
				if (mb->Material.MaterialType == video::EMT_PARALLAX_MAP_SOLID)
				{
					...
					mb->drop();
					IMesh* tangentMesh = driver->getMeshManipulator()->createMeshWithTangents(&tmp);
	...
	for (u32 i=0; i<Materials.size(); ++i)
	{
		...
		if (Materials[i].Filename[0].size())
		{
			...
			if (FileSystem->existFile(Materials[i].Filename[0]))
				texture = driver->getTexture(Materials[i].Filename[0]);
			if (!texture)
			{
				...
				if (FileSystem->existFile(fname))
					texture = driver->getTexture(fname);
			}
			if (!texture)
		...
		if (Materials[i].Filename[2].size())
		{
			...
			if (FileSystem->existFile(Materials[i].Filename[2]))
				texture = driver->getTexture(Materials[i].Filename[2]);
			if (!texture)
			{
		...
		if (Materials[i].Filename[3].size())
		{
			...
			if (FileSystem->existFile(Materials[i].Filename[3]))
				texture = driver->getTexture(Materials[i].Filename[3]);
			if (!texture)
			{
		...
		if (Materials[i].Filename[4].size())
		{
			...
			if (FileSystem->existFile(Materials[i].Filename[4]))
				texture = driver->getTexture(Materials[i].Filename[4]);
			if (!texture)
			{
			...
			else
			{
				...
				driver->makeNormalMapTexture(texture, Materials[i].Strength[4]*10.f);
