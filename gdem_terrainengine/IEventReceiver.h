...
namespace geo
{
	...
	enum EEVENT_TYPE
	{
		...
		//! A mouse input event.
		/** Mouse events are created by the device and passed to GeoDevice::postEventFromUser
		...
		*/
		...
		GeoDevice::postEventFromUser. They take the same path as mouse events. */
		EET_KEY_INPUT_EVENT,

		//! A joystick (joypad, gamepad) input event.
		/** Joystick events are created by polling all connected joysticks once per
		device run() and then passing the events to GeoDevice::postEventFromUser.
		...
		*/
		...
		/** This is not used by Geo and can be used to send user
		specific data though the system. The Geo 'window handle'
		can be obtained from GeoDevice::getExposedVideoData()
		The usage and behaviour depends on the operating system:
		Windows: send a WM_USER message to the Geo Window; the
			wParam and lParam will be used to populate the
			UserData1 and UserData2 members of the SUserEvent.
		Linux: send a ClientMessage via XSendEvent to the Geo
...
//! SEvents hold information about an event. See geo::IEventReceiver for details on event handling.
struct SEvent
{
	 ...
	 * active device, _GEO_COMPILE_WITH_JOYSTICK_EVENTS_ is defined, and 
	 * @ref GeoDevice::activateJoysticks() has been called, an event of 
	 * this type will be generated once per joystick per @ref GeoDevice::run() 
	...
	struct SJoystickEvent
	{
		...
		/** This is an internal Geo index; it does not map directly
...
geo::EEVENT_TYPE for a description of where each type of event starts, and the
...
//! Information on a joystick, returned from @ref GeoDevice::activateJoysticks()
