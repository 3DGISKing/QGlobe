...
COpenGLTexture::COpenGLTexture(IImage* origImage, const char* name, COpenGLDriver* driver, ECOLOR_FORMAT format)
	: ITexture(name), ColorFormat(format), Driver(driver), Image(0),
	TextureName(0), InternalFormat(GL_RGB), PixelFormat(GL_BGRA_EXT),
	...
	ReadOnlyLock(false), KeepImage(true)
{
	...
	// force set RGB in this module for save memory. RYM.
	ColorFormat = format;
...
//Texture의 유효성을 검사, Texture를 생성해주는 함수추가.
bool COpenGLTexture::validateTexture()
{
	if (!Image)
		return false;
...
void* COpenGLTexture::lock(bool readOnly)
{
	...
	if (!Image || IsRenderTarget)
	{
		...
		if (Driver->queryOpenGLFeature(COpenGLExtensionHandler::GEO_MESA_pack_invert))
	...
	StencilRenderBuffer(0), UseStencil(useStencil)
{
	...
	if (useStencil)
	{
		...
		if (Driver->queryOpenGLFeature(COpenGLExtensionHandler::GEO_EXT_packed_depth_stencil))
		{
