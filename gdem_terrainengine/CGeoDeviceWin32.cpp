...
namespace geo
{
	namespace video
	{
		#ifdef _GEO_COMPILE_WITH_DIRECT3D_8_
		...
		#ifdef _GEO_COMPILE_WITH_DIRECT3D_9_
		...
		IVideoDriver* createOpenGLDriver(const geo::SGeoCreationParameters& params, io::IFileSystem* io);
		#endif
	}
} // end namespace geo
...
{
	HWND hWnd;
	geo::CGeoDeviceWin32* irrDev;
};

geo::core::list<SEnvMapper> EnvMap;

SEnvMapper* getEnvMapperFromHWnd(HWND hWnd)
{
	geo::core::list<SEnvMapper>::Iterator it = EnvMap.begin();
...
geo::CGeoDeviceWin32* getDeviceFromHWnd(HWND hWnd)
{
	geo::core::list<SEnvMapper>::Iterator it = EnvMap.begin();
	for (; it!= EnvMap.end(); ++it)
		if ((*it).hWnd == hWnd)
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	geo::CGeoDeviceWin32* dev = 0;
	geo::SEvent event;

	static geo::s32 ClickCount=0;
	...
	case WM_MOUSEWHEEL:
		...
		event.MouseInput.Wheel = ((geo::f32)((short)HIWORD(wParam))) / (geo::f32)WHEEL_DELTA;
		event.MouseInput.Event = geo::EMIE_MOUSE_WHEEL;
	...
	case WM_LBUTTONDOWN:
		...
		SetCapture(hWnd);
		event.EventType = geo::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = geo::EMIE_LMOUSE_PRESSED_DOWN;
	...
	case WM_LBUTTONUP:
		...
		event.EventType = geo::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = geo::EMIE_LMOUSE_LEFT_UP;
		event.MouseInput.X = (short)LOWORD(lParam);
		event.MouseInput.Y = (short)HIWORD(lParam);
		dev = getDeviceFromHWnd(hWnd);
		if (dev)
	...
	case WM_RBUTTONDOWN:
		...
		SetCapture(hWnd);
		event.EventType = geo::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = geo::EMIE_RMOUSE_PRESSED_DOWN;
		event.MouseInput.X = (short)LOWORD(lParam);
		event.MouseInput.Y = (short)HIWORD(lParam);
		dev = getDeviceFromHWnd(hWnd);
		if (dev)
	...
	case WM_RBUTTONUP:
		...
		event.EventType = geo::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = geo::EMIE_RMOUSE_LEFT_UP;
		event.MouseInput.X = (short)LOWORD(lParam);
		event.MouseInput.Y = (short)HIWORD(lParam);
		dev = getDeviceFromHWnd(hWnd);
		if (dev)
	...
	case WM_MBUTTONDOWN:
		...
		SetCapture(hWnd);
		event.EventType = geo::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = geo::EMIE_MMOUSE_PRESSED_DOWN;
		event.MouseInput.X = (short)LOWORD(lParam);
		event.MouseInput.Y = (short)HIWORD(lParam);
		dev = getDeviceFromHWnd(hWnd);
		if (dev)
	...
	case WM_MBUTTONUP:
		...
		event.EventType = geo::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = geo::EMIE_MMOUSE_LEFT_UP;
		event.MouseInput.X = (short)LOWORD(lParam);
		event.MouseInput.Y = (short)HIWORD(lParam);
		dev = getDeviceFromHWnd(hWnd);
		if (dev)
	...
	case WM_MOUSEMOVE:
		event.EventType = geo::EET_MOUSE_INPUT_EVENT;
		event.MouseInput.Event = geo::EMIE_MOUSE_MOVED;
		event.MouseInput.X = (short)LOWORD(lParam);
		event.MouseInput.Y = (short)HIWORD(lParam);
		dev = getDeviceFromHWnd(hWnd);
	...
	case WM_KEYUP:
		{
			...
			event.EventType = geo::EET_KEY_INPUT_EVENT;
			event.KeyInput.Key = (geo::EKEY_CODE)wParam;
	...
	case WM_USER:
		event.EventType = geo::EET_USER_EVENT;
		event.UserEvent.UserData1 = (geo::s32)wParam;
		event.UserEvent.UserData2 = (geo::s32)lParam;
		dev = getDeviceFromHWnd(hWnd);
...
//! constructor
CGeoDeviceWin32::CGeoDeviceWin32(const SGeoCreationParameters& params)
: CGeoDeviceStub(params), HWnd(0), ChangedToFullScreen(false),
	IsNonNTWindows(false), Resized(false),
	ExternalWindow(false), Win32CursorControl(0)
{
	#ifdef _DEBUG
	setDebugName("CGeoDeviceWin32");
	#endif
	...
	{
		const c8* ClassName = "CGeoDeviceWin32";
...
//! destructor
CGeoDeviceWin32::~CGeoDeviceWin32()
{
...
//! create the driver
void CGeoDeviceWin32::createDriver()
{
	...
	case video::EDT_DIRECT3D8:
		...
		#endif // _GEO_COMPILE_WITH_DIRECT3D_8_
	...
	case video::EDT_DIRECT3D9:
		...
		#endif // _GEO_COMPILE_WITH_DIRECT3D_9_
...
void CGeoDeviceWin32::sleep(u32 timeMs, bool pauseTimer)
{
...
void CGeoDeviceWin32::resizeIfNecessary()
{
	if (!Resized)
		return;
	...
	else
	{
		...
		getVideoDriver()->OnResize(geo::core::dimension2d<geo::s32>(r.right, r.bottom));
	}
...
void CGeoDeviceWin32::setWindowCaption(const wchar_t* text)
{
	DWORD dwResult;
	if (IsNonNTWindows)
	{
...
bool CGeoDeviceWin32::present(video::IImage* image, void* windowId, core::rect<s32>* src)
{
	HWND hwnd = HWnd;
	if ( windowId )
...
void CGeoDeviceWin32::closeDevice()
{
	MSG msg;
...
bool CGeoDeviceWin32::isWindowActive() const
{
	_GEO_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
...
bool CGeoDeviceWin32::isWindowFocused() const
{
...
bool CGeoDeviceWin32::isWindowMinimized() const
{
	WINDOWPLACEMENT plc;
...
bool CGeoDeviceWin32::switchToFullScreen(s32 width, s32 height, s32 bits)
{
	DEVMODE dm;
...
CGeoDeviceWin32::CCursorControl* CGeoDeviceWin32::getWin32CursorControl()
{
...
//! by the gfx adapter.
video::IVideoModeList* CGeoDeviceWin32::getVideoModeList()
{
...
void CGeoDeviceWin32::getWindowsVersion(core::stringc& out)
{
...
void CGeoDeviceWin32::OnResized()
{
	Resized = true;
}

//! Sets if the window should be resizeable in windowed mode.
void CGeoDeviceWin32::setResizeAble(bool resize)
{
...
bool CGeoDeviceWin32::activateJoysticks(core::array<SJoystickInfo> & joystickInfo)
{
...
void CGeoDeviceWin32::pollJoysticks()
{
#if defined _GEO_COMPILE_WITH_JOYSTICK_EVENTS_
	...
	for(joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
	{
		if(JOYERR_NOERROR == joyGetPosEx(ActiveJoysticks[joystick].Index, &info))
		{
			...
			event.EventType = geo::EET_JOYSTICK_INPUT_EVENT;
...
{
	CGeoDeviceWin32* dev = new CGeoDeviceWin32(parameters);
