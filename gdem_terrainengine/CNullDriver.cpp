...
//kimyi : Texture Duplicate Delete
//#define GEO_NO_USE_DEFAULT
	...
	PrimitivesDrawn(0), TextureCreationFlags(0), AllowZWriteOnTransparent(false)
{
...
#ifdef _GEO_COMPILE_WITH_BMP_WRITER_
...
#endif
#ifdef _GEO_COMPILE_WITH_TGA_WRITER_
	...
	//2013 2 28
	MeshLoader.push_back(scene::create3DSMeshLoader(FileSystem));
...
// check validate texture
void CNullDriver::validateTextures()
{
	for (u32 i=0; i<Textures.size(); ++i)
		Textures[i].Surface->validateTexture();
}
...
void CNullDriver::removeTexture(ITexture* texture)
{
	/********************************************************
	2013 3 6 ugi
	ËË ¿¸´ÅÀË¶ìË¼ deadlockº¬ÀôËæ Æê¼è±Â ÂÚºã ËØ²÷ Ì¡ÂéÂÙ ¹¢¹¤ËË·àµá ³Þ»¤ ¶ìÊ­²ÛÊ¾³Þ.

	example

	QMutex a,b;
	a.lock();
	b.lock();   //ÊÞ±¨º· deadlockËæ ´ÉÊÌ°£³Þ.

	Ìº¶· ËË ¹¢¹¤Ë¼ ÊÖ³õ°ÃË¾ ¹ÖÀË¶£»ôµê¶¦ texture basedËæ Ì©ÂÙ ¸Ò¸âËºµá ·Æº¡Â×µØ°Ö »¤´ªÂ×³Þ°¡
	¸ÇÊ­²ÄÊÝÊÝ³Þ. ËËµÍÂÙ ¸Ò¸âË¾ °éÂïÂ×»ô·² ÊÞµÍ±¶Ì© È¸¶Á´Å°¡ Á¢º÷½î·»µâËæ ¼±°ýÂ×·àµá lock¶¦ °¾ÊÌ¼Ô Â­Êý°¡
	½£±¨´óÊî³õ°ÃËËÊî³Þ.
	**********************************************************/
	//mutex.lock();
	if (!texture)
		return;
#ifdef GEO_NO_USE_DEFAULT
	...
	for (u32 i=0; i<Textures.size(); ++i)
	{
		if (Textures[i].Surface == texture)
		{
			...
			/***************************
			2013 2 27
			¼èË§µØ²÷ Á¢º÷½î¶¦ ½ÜÊ­ ¼èË© ³ÞËÁËæ²÷ ¼á»¤ À³Á¬ÂÙ³Þ.
			ËË ¿¸´Å°¡ ÊÖËº·² ¸Æ°û°¡ ±ýÊòºã ËØ³Þ
			****************************/
			return;
		}
	}
...
//! loads a Texture
ITexture* CNullDriver::getTexture(void* imgBuffer, geo::s32 buffSize, const geo::s8* imgType, const geo::s8* textureName)
{
	ITexture* texture = 0;

#ifdef GEO_NO_USE_DEFAULT
		texture = loadTextureFromFile(imgBuffer, buffSize, imgType, textureName);
#else
	if (imgBuffer)
	{
                texture = findTexture((geo::c8* )textureName);
	...
	if (!texture)
                os::Printer::log("Could not load texture", (geo::c8*) textureName, ELL_WARNING);
#endif
...
video::ITexture* CNullDriver::loadTextureFromFile(void* imgBuffer, geo::s32 buffSize, const geo::s8* imgType, const geo::s8* textureName)
{
	ITexture* texture = 0;
	IImage* image = createImageFromFile(imgBuffer, buffSize, imgType, textureName);

	if (image)
	{
		// create texture from surface
                texture = createDeviceRGBTexture(image, (char* )textureName);
                os::Printer::log("Loaded texture", (geo::c8*)textureName);
		image->drop();
	}
...
//! adds a surface, not loaded or created by the Geo Engine
...
//! Draws a 3d line.
void CNullDriver::draw3DPoint(const core::vector3df& point, SColor color)
{
}
...
//! Creates a software image from a file.
IImage* CNullDriver::createImageFromFile(void* imgBuffer, geo::s32 buffSize, const geo::s8* imgType, const geo::s8* textureName)
{
	if (!imgBuffer)
		return 0;
	...
	for (i=0; i<SurfaceLoader.size(); ++i)
	{
                if (SurfaceLoader[i]->isALoadableFileExtension((geo::c8*)imgType))
		{
			// reset file position which might have changed due to previous loadImage calls
			image = SurfaceLoader[i]->loadImage(imgBuffer, buffSize);
			if (image)
				return image;
		}
	}
...
scene::IAnimatedMesh* CNullDriver::createMeshFromFile(const char* filename)
	{
	if (!filename)
		return 0;
...
//2013 2 28  ugi
scene::IAnimatedMesh* CNullDriver::createMeshFromFile(io::IReadFile* file)
{
	if (!file)
		return 0;
	...
	for (i=0; i<MeshLoader.size(); ++i)
	{
		if (MeshLoader[i]->isALoadableFileExtension(file->getFileName()))
		{
			// reset file position which might have changed due to previous loadmesh calls
			MeshLoader[i]->setDriver(this);
			file->seek(0);
			mesh = MeshLoader[i]->createMesh(file);
			if (mesh)
				return mesh;
		}
	}
