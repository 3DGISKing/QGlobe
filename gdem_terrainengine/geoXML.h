...
// For conditions of distribution and use, see copyright notice in geoEngine.h and/or irrXML.h

#ifndef __GEO_XML_H_INCLUDED__
#define __GEO_XML_H_INCLUDED__
...
/** \mainpage irrXML 1.2 API documentation
 ...
 \section intro Introduction
  ...
  irrXML provides forward-only, read-only
	...
	used with the Geo Engine, it directly reads from compressed .zip files.
	- Just like the Geo Engine for which it was originally created, it is extremely easy
	to use.
	...
	<config>
		...
		<messageText caption="Geo Engine Mesh Viewer">
		Welcome to the Mesh Viewer of the &quot;Geo Engine&quot;.
		</messageText>
	</config>
	\endcode
	...
	\code
	#include <irrXML.h>
	using namespace geo; // irrXML is located in the namespace geo::io
	using namespace io;
	...
	void main()
	{
		...
		GeoXMLReader* xml = createGeoXMLReader("config.xml");
	...
	<A HREF="http://irrlicht.sourceforge.net" >Geo Engine</A>. But because
	the parser has become very useful with the latest release, people asked for a
	separate version of it, to be able to use it in non Geo projects. With
...
namespace io
{
	...
	//! Enumeration for all xml nodes which are parsed by GeoXMLReader
	enum EXML_NODE
	{
	...
	things than just files. The Geo engine is using this for example to
	...
	your implementation when calling createGeoXMLReader(),
	createGeoXMLReaderUTF16() or createGeoXMLReaderUTF32() */
	...
	//! Empty class to be used as parent class for GeoXMLReader.
	...
	The Geo Engine for example needs IReferenceCounted as base class for every object to
	...
	createGeoXMLReader(), createGeoXMLReaderUTF16() and createGeoXMLReaderUTF32().
	If using the parser from the Geo Engine, please use IFileSystem::createXMLReader()
	instead.
	...
	*/
	...
	class IGeoXMLReader : public super_class
	{
	public:

		//! Destructor
		virtual ~IGeoXMLReader() {}
		...
		GeoXMLReaderUTF8() and UTF32 when it has been created using
		GeoXMLReaderUTF32. It should not be necessary to call this
	...
	in this format.
	Create an instance of this with createGeoXMLReader();
	See IGeoXMLReader for description on how to use it. */
	typedef IGeoXMLReader<char, IXMLBase> GeoXMLReader;
	...
	in this format.
	Create an instance of this with createGeoXMLReaderUTF16();
	See IGeoXMLReader for description on how to use it. */
	typedef IGeoXMLReader<char16, IXMLBase> GeoXMLReaderUTF16;
	...
	in this format.
	Create an instance of this with createGeoXMLReaderUTF32();
	See IGeoXMLReader for description on how to use it. */
	typedef IGeoXMLReader<char32, IXMLBase> GeoXMLReaderUTF32;
	...
	If you are using the Geo Engine, it is better not to use this function but
	IFileSystem::createXMLReaderUTF8() instead.
	\param filename: Name of file to be opened.
	\return Returns a pointer to the created xml parser. This pointer should be
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReader* GEOCALLCONV createGeoXMLReader(const char* filename);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReader* GEOCALLCONV createGeoXMLReader(FILE* file);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReader* GEOCALLCONV createGeoXMLReader(IFileReadCallBack* callback);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReaderUTF16* GEOCALLCONV createGeoXMLReaderUTF16(const char* filename);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReaderUTF16* GEOCALLCONV createGeoXMLReaderUTF16(FILE* file);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReaderUTF16* GEOCALLCONV createGeoXMLReaderUTF16(IFileReadCallBack* callback);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReaderUTF32* GEOCALLCONV createGeoXMLReaderUTF32(const char* filename);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReaderUTF32* GEOCALLCONV createGeoXMLReaderUTF32(FILE* file);
	...
	deleted using 'delete' after no longer needed. Returns 0 if an error occured
	and the file could not be opened. */
	GEOENGINE_API GeoXMLReaderUTF32* GEOCALLCONV createGeoXMLReaderUTF32(IFileReadCallBack* callback);


	/*! \file irrxml.h
	\brief Header file of the irrXML, the Geo XML parser.

	This file includes everything needed for using irrXML,
	the XML parser of the Geo Engine. To use irrXML,
