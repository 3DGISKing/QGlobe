...
namespace scene
{
	IMeshLoader* create3DSMeshLoader(io::IFileSystem* fs);
} 
namespace io
{
	class IWriteFile;
...
namespace video
{
	...
	public:
		...
		//! loads a Texture from buffer
		virtual ITexture* getTexture(void* imgBuffer, geo::s32 buffSize, const geo::s8* imgType, const geo::s8* textureName);
		...
		//! Draws a 3d Point.
		virtual void draw3DPoint(const core::vector3df& point,
		...
		//! Draws a 3d Polygon.
		virtual void draw3DPolyline(const core::array<core::vector3df>& vertexArray, 
			bool closed = false, SColor color = SColor(255,255,255,255)) {}
		...
		//! Creates a software image from a buffer.
		virtual IImage* createImageFromFile(void* imgBuffer, geo::s32 buffSize, const geo::s8* imgType, const geo::s8* textureName);
		...
		//2013 2 28 added by ugi
		virtual scene::IAnimatedMesh* createMeshFromFile(const char* filename);
		virtual scene::IAnimatedMesh* createMeshFromFile(io::IReadFile* file);
		//end
	...
	protected:
		...
		//! loads textur from buffer into the surface
		video::ITexture* loadTextureFromFile(void* imgBuffer, geo::s32 buffSize, const geo::s8* imgType, const geo::s8* textureName);
		...
		virtual video::ITexture* createDeviceRGBTexture(IImage* surface, const char* name) {return NULL;};
		...
		struct SDummyTexture : public ITexture
		{
			...
			virtual const IImage* getImage() const {return 0;}
		...
		//2013 2 16 added by ugi
		core::array<scene::IMeshLoader*> MeshLoader;
		//end
