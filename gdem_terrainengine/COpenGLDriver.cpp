...
COpenGLDriver::COpenGLDriver(const geo::SGeoCreationParameters& params,
		io::IFileSystem* io)
...
bool COpenGLDriver::initDriver(geo::SGeoCreationParameters params)
{
	...
	if (AntiAlias)
	{
		...
		const c8* ClassName = "GLCGeoDeviceWin32";
...
//! Windows constructor and init code
COpenGLDriver::COpenGLDriver(const SGeoCreationParameters& params,
							 io::IFileSystem* io, CGeoDeviceQT *device)
							 : CNullDriver(io, params.WindowSize), COpenGLExtensionHandler(),
							 CurrentRenderMode(ERM_NONE), ResetRenderStates(true), Transformation3DChanged(true),
							 AntiAlias(params.AntiAlias), RenderTargetTexture(0), LastSetLight(-1),
...
//// MacOSX CONSTRUCTOR
//// -----------------------------------------------------------------------
//#ifdef _GEO_USE_OSX_DEVICE_
////! Windows constructor and init code
//COpenGLDriver::COpenGLDriver(const SGeoCreationParameters& params,
//		io::IFileSystem* io, CGeoDeviceMacOSX *device)
//: CNullDriver(io, params.WindowSize), COpenGLExtensionHandler(),
//	CurrentRenderMode(ERM_NONE), ResetRenderStates(true), Transformation3DChanged(true),
//	AntiAlias(params.AntiAlias), RenderTargetTexture(0), LastSetLight(-1),
//	CurrentRendertargetSize(0,0), ColorFormat(ECF_R8G8B8), _device(device)
	...
	RenderTargetTexture(0), LastSetLight(-1), CurrentRendertargetSize(0,0), ColorFormat(ECF_R8G8B8)
{
	...
	setDebugName("COpenGLDriver");
	#endif
        ExposedData.OpenGLLinux.X11Context = (void*)  glXGetCurrentContext();
        ExposedData.OpenGLLinux.X11Display = (void*) glXGetCurrentDisplay();
	ExposedData.OpenGLLinux.X11Window = (unsigned long)params.WindowId;
        g_Drawable  = (XID ) glXGetCurrentDrawable();
...
//	glEnable(GL_COLOR_MATERIAL);
	...
	if (FeatureAvailable[GEO_EXT_separate_specular_color])
		glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL, GL_SEPARATE_SPECULAR_COLOR);
#endif
	glLightModeli(GL_LIGHT_MODEL_LOCAL_VIEWER, 1);

// This is a fast replacement for NORMALIZE_NORMALS
//	if ((Version>101) || FeatureAvailable[GEO_EXT_rescale_normal])
...
void COpenGLDriver::createMaterialRenderers()
{
	...
	addAndDropMaterialRenderer(new COpenGLMaterialRenderer_OVERLAY_ALPHA_TEXTURE(this));
...
#ifdef _GEO_USE_QT_DEVICE_
	return true;
#elif defined(_GEO_USE_WINDOWS_DEVICE_)
	return SwapBuffers(HDc) == TRUE;
#elif defined(_GEO_USE_LINUX_DEVICE_)
        glXSwapBuffers((Display*)ExposedData.OpenGLLinux.X11Display, g_Drawable);
	return true;
//kimyi : Use QT
...
bool COpenGLDriver::updateVertexHardwareBuffer(SHWBufferLink_opengl *HWBuffer)
{
	...
	if (!FeatureAvailable[GEO_ARB_vertex_buffer_object])
		return false;
		...
		E_VERTEX_TYPE vType, scene::E_PRIMITIVE_TYPE pType, E_INDEX_TYPE iType)
{
	...
	switch (pType)
	{
		...
		case scene::EPT_POINT_SPRITES:
		{
			...
			if (pType==scene::EPT_POINT_SPRITES && FeatureAvailable[GEO_ARB_point_sprite])
...
video::ITexture* COpenGLDriver::createDeviceRGBTexture(IImage* surface, const char* name)
{
	return new COpenGLTexture(surface, name, this, ECF_R8G8B8);
}
	...
	for (u32 u=0; u<MaxTextureUnits; ++u)
	{
		...
		switch (material.TextureLayer[u].TextureWrap)
		{
			...
			case ETC_CLAMP_TO_EDGE:
				...
				if (FeatureAvailable[GEO_SGIS_texture_edge_clamp])
			...
			case ETC_CLAMP_TO_BORDER:
				...
				if (FeatureAvailable[GEO_ARB_texture_border_clamp])
					mode=GL_CLAMP_TO_BORDER_ARB;
				else
#endif
#ifdef GL_SGIS_texture_border_clamp
				if (FeatureAvailable[GEO_SGIS_texture_border_clamp])
...
#endif
#ifdef GL_ARB_texture_border_clamp
				if (FeatureAvailable[GEO_ARB_texture_mirrored_repeat])
					mode=GL_MIRRORED_REPEAT_ARB;
				else
#endif
#ifdef GL_IBM_texture_mirrored_repeat
				if (FeatureAvailable[GEO_IBM_texture_mirrored_repeat])
...
void COpenGLDriver::drawStencilShadowVolume(const core::vector3df* triangles, s32 count, bool zfail)
{
	...
	if (FeatureAvailable[GEO_EXT_stencil_two_side])
	{
		glEnable(GL_STENCIL_TEST_TWO_SIDE_EXT);
#ifdef GL_NV_depth_clamp
		if (FeatureAvailable[GEO_NV_depth_clamp])
		...
		if (!zfail)
		{
			...
			if (FeatureAvailable[GEO_EXT_stencil_wrap])
...
#endif
	if (FeatureAvailable[GEO_ATI_separate_stencil])
	{
		glDisable(GL_CULL_FACE);
		if (!zfail)
		{
			...
			f32 end, f32 density, bool pixelFog, bool rangeFog)
{
	...
	if (FeatureAvailable[GEO_EXT_fog_coord])
...
//! Draws a 3d Point.
void COpenGLDriver::draw3DPoint(const core::vector3df& point, SColor color)
{
	setRenderStates3DMode();
...
//! Draws a 3d Polygon.
void COpenGLDriver::draw3DPolyline(const core::array<core::vector3df>& vertexArray, 
								   bool closed, SColor color)
{
	setRenderStates3DMode();
	...
	glColor4ub(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha());
	for(s32 i=0 ; i<(s32)vertexArray.size() ; i++)
	{
...
IImage* COpenGLDriver::createScreenShot()
{
	...
	if (FeatureAvailable[GEO_MESA_pack_invert])
...
void COpenGLDriver::saveRenderState()
{
#if 0
//	glEnable(GL_DEPTH_TEST);
	//glFlush();
	glGetIntegerv( GL_VIEWPORT, renderState.viewport );
	int size = renderState.viewport[2] * renderState.viewport[3];
	if (size > renderState.maxZBufferSize)
	{
		renderState.maxZBufferSize = size;
		if (renderState.zbuffer)
			free(renderState.zbuffer);
		renderState.zbuffer = NULL;
		renderState.zbuffer = (GLfloat*)malloc(size * sizeof(GLfloat));
		if (renderState.zbuffer == NULL)
			return;
	}
	glReadBuffer(GL_BACK);
	glReadPixels( 0, 0, renderState.viewport[2], renderState.viewport[3], GL_DEPTH_COMPONENT, GL_FLOAT, renderState.zbuffer );
	//glReadPixels( 0, 0, 10, 10, GL_DEPTH_COMPONENT, GL_FLOAT, renderState.zbuffer );
	glGetDoublev( GL_MODELVIEW_MATRIX, renderState.modelviewMatrix );
	glGetDoublev( GL_PROJECTION_MATRIX, renderState.projectionMatrix );
//	glDisable(GL_DEPTH_TEST);
#endif
}

void COpenGLDriver::screenToWorld(int xScreen, int yScreen, double* posWorld)
{
#if 0
	if (renderState.zbuffer == NULL)
		return;
	if (xScreen < 0 || xScreen > renderState.viewport[2] || yScreen < 0 || yScreen > renderState.viewport[3])
		return;
	GLfloat winX, winY, winZ;
	GLdouble posX, posY, posZ;
	winX = xScreen;
	winY = renderState.viewport[3] - yScreen;
	winZ = renderState.zbuffer[ int(winY) * renderState.viewport[2] + xScreen];

	gluUnProject( winX, winY, winZ, renderState.modelviewMatrix, renderState.projectionMatrix, 
				renderState.viewport, &posX, &posY, &posZ);
...
// QT VERSION
// -----------------------------------
#if defined(_GEO_USE_QT_DEVICE_)
...
// -----------------------------------
#ifdef _GEO_USE_WINDOWS_DEVICE_
IVideoDriver* createOpenGLDriver(const geo::SGeoCreationParameters& params,
	io::IFileSystem* io)
{
#ifdef _GEO_COMPILE_WITH_OPENGL_
...
//// MACOSX VERSION
//// -----------------------------------
//#if defined(_GEO_USE_OSX_DEVICE_)
//IVideoDriver* createOpenGLDriver(const SGeoCreationParameters& params,
//		io::IFileSystem* io, CGeoDeviceMacOSX *device)
//{
//#ifdef _GEO_COMPILE_WITH_OPENGL_
...
//#endif //  _GEO_COMPILE_WITH_OPENGL_
//}
//#endif // _GEO_USE_OSX_DEVICE_

// -----------------------------------
// X11/SDL VERSION
// -----------------------------------
#if defined(_GEO_USE_LINUX_DEVICE_) || defined(_GEO_USE_SDL_DEVICE_)
IVideoDriver* createOpenGLDriver(const SGeoCreationParameters& params,
		io::IFileSystem* io)
{
#ifdef _GEO_COMPILE_WITH_OPENGL_
