...
#ifndef __GEO_I_VIDEO_DRIVER_H_INCLUDED__
#define __GEO_I_VIDEO_DRIVER_H_INCLUDED__
...
namespace video
{
	...
	the Geo Engine: All rendering and texture manipulation is done with
	this interface. You are able to use the Geo Engine by only
	invoking methods of this interface if you like to, although the
	geo::scene::ISceneManager interface provides a lot of powerful classes
	...
	*/
	...
	public:
		...
		//2013 2 16 added by ugi
		virtual scene::IAnimatedMesh* createMeshFromFile(const char* filename)=0;
		virtual scene::IAnimatedMesh* createMeshFromFile(io::IReadFile* file)=0;
		//end
		...
		*/
		...
		virtual ITexture* getTexture(void* imgBuffer, geo::s32 buffSize, const geo::s8* imgType, const geo::s8* textureName) = 0;
		...
		virtual void draw3DPolyline(const core::array<core::vector3df>& vertexArray, 
			bool closed = false, SColor color = SColor(255,255,255,255)) = 0;
		...
		/** This is useful if the Geo Engine should be able to load
		...
		/** This is useful if the Geo Engine should be able to
		...
		*/
		...
		// check texture validate for multithread (RYM)
		virtual void validateTextures() {};

		virtual void saveRenderState() {};
		virtual void screenToWorld(int xScreen, int yScreen, double* posWorld) {};
