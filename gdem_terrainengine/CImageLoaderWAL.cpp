...
bool CImageLoaderWAL::isALoadableFileFormat(geo::io::IReadFile* file) const
{
	return (false); // no recognition possible, use a proper file ending
}


IImage* CImageLoaderWAL::loadImage(geo::io::IReadFile* file) const
{
	...
	// I wrote an 8 to 32 converter, but this works with released Geo code.
...
{
	return new geo::video::CImageLoaderWAL();
}
