...
const char ID_STR_UNKNOWN_NAME_STRING[] = u8"[�̸�����]";
...
static bool GetPlaceSubType(const kmldom::FeaturePtr feature, E_QGlobe_SUBDATA_TYPE* subtype)
{
	if (feature == NULL)
		return false;	// return NO
	...
	if (feature->has_id())
	{
		if (feature->get_id().compare(ID_STR_DEFAULT_STARTING_LOCATION) != 0)
		{
			*subtype = (E_QGlobe_SUBDATA_TYPE)(atoi(feature->get_id().c_str()));
...
//--------------------------------------------------- Set Subtype
static int SetPlaceSubType(const kmldom::FeaturePtr feature, E_QGlobe_SUBDATA_TYPE subtype)
{
	if (feature == NULL)
		return -1;	// return error (Null pointer)
