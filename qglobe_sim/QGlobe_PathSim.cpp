...
CQGlobe_PathSim::CQGlobe_PathSim(CQGlobe_SimMgr* a_pSimMgr) : CQGlobe_Simulator(a_pSimMgr)
{
	m_dStartMovetime = 0;
...
CQGlobe_PathSim::~CQGlobe_PathSim()
{
	if(m_pGISPath) 
		m_pGISPath = NULL;
...
void	
CQGlobe_PathSim::InitSimulation()
{	
	
	//m_dStarttime = qglobe_GetTickCount();
	//m_eStatus == SIM_PATH_START;

}

bool	
CQGlobe_PathSim::AdvanceSimulation(IN double a_dt)
{
	if ( m_ended )
		return true;
...
void	
CQGlobe_PathSim::GetSimulationStatus(OUT QGlobe_SIM_STATUS* a_pSimStatus)
{
...
void
CQGlobe_PathSim::SetGISPath(IN CGIS_Path* a_pGISPath , OUT QGlobe_CAMERA_INFO* a_pCameraInfo)
{
	...
	CQGlobe_Vector3DF vPosition;
	QGlobe_LOCATION   sLatLong;
	...
	CQGlobe_Vector3DF vSpeed;
	CQGlobe_Vector3DF vN1 , vN2 ;
	CQGlobe_Vector3DF vPos ;
	CQGlobe_Vector3DF vTemp ;
	double dtime = 0;
	...
	{		
		...
		vPos =  m_pArrTargetPos[i] + (vTemp.normalize())*(m_dCamerDistance*cos(m_dSlopeAngle*qglobe_DEGTORAD)) 
			+ vN2*(m_dCamerDistance*sin(m_dSlopeAngle*qglobe_DEGTORAD));		
	...
	qglobe_GetCoordInfo(&coord, &cameraInfo, 0);
...
void 
CQGlobe_PathSim::ControlPlay(IN QGlobe_SIM_CONTROL a_type, double a_dtime)
{
...
void
CQGlobe_PathSim::SetSimulationStatus(IN QGlobe_SIM_STATUS* a_pSimStatus)
{
...
bool
CQGlobe_PathSim::StartAnimation()
{
	if(!m_pGISPath) 
		return false;
...
bool	
CQGlobe_PathSim::MoveStartPosition()
{
	CQGlobe_Vector3DF vLookAt;
	CQGlobe_Vector3DF vSpeed;
	...
	CQGlobe_Point3DF  CameraPosition(m_pArrCameraPos[0].m_tX,m_pArrCameraPos[0].m_tY,m_pArrCameraPos[0].m_tZ);
...
bool	
CQGlobe_PathSim::MoveEndPosition()
{
	CQGlobe_Vector3DF vLookAt;
	CQGlobe_Vector3DF vSpeed;
	...
	CQGlobe_Point3DF  CameraPosition(m_pArrCameraPos[Index].m_tX,m_pArrCameraPos[Index].m_tY,m_pArrCameraPos[Index].m_tZ);
...
#define		QGLOBE_MIN_PATH_DIVISION_ANGLE		0.03 // ~PAI / 30

inline void ConvertVector3D( IN CQGlobe_Location3D *pLoc ,OUT CQGlobe_Point3DF* pPosF)
{
...
void 
CQGlobe_PathSim::GetDrawSegPoints(CQGlobe_VectorArr* pVetorArr, QGlobe_LOCATION* pStart, QGlobe_LOCATION* pEnd, double nDiv)
{
	double	dHeight = 0;
...
void CQGlobe_PathSim::ResetDivisionPoints()
{
	if(!m_pGISPath)
		return;
	...
	for(i = 1; i < allCount; i++)
	{
		...
		if(dist < QGLOBE_MIN_PATH_DIVISION_ANGLE)
		{
		...
		// division
		nDiv = (int)(dist / QGLOBE_MIN_PATH_DIVISION_ANGLE + 1);
...
bool
CQGlobe_PathSim::GetSpeedVector(IN int a_nIndex, INOUT CQGlobe_Vector3DF* a_pvSpeed)
{
	int nCount = m_pArrTargetPos.size();
	...
	{
		CQGlobe_Vector3DF vTempNext = m_pArrTargetPos[a_nIndex+1] - m_pArrTargetPos[a_nIndex];
...
{		
	...
	CQGlobe_Vector3DF  vTargetP0 = m_pArrTargetPos[idx];
	CQGlobe_Vector3DF  vTargetP1 = m_pArrTargetPos[idx + 1];

	CQGlobe_Vector3DF  vCameraP0 = m_pArrCameraPos[idx];
	CQGlobe_Vector3DF  vCameraP1 = m_pArrCameraPos[idx + 1];
	...
	if(m_dSlopeAngle < 1.0)
	{
		...
		//CQGlobe_Vector3DF vSpeed  = vCameraP0 * h1 + vCameraP1 * h2 + V0 * h3 + V1 * h4;

		CQGlobe_Vector3DF vSpeed  = V0 * (1.0 - u) + V1 * u;


		//change camera position
		CQGlobe_Vector3DF vLookAt = vTargetPosition - vCameraPosition;

		CQGlobe_Vector3DF UpVector = vLookAt.crossProduct(vSpeed.crossProduct(vLookAt));

		if(m_pCamera)
		{
			CQGlobe_Point3DF  CameraPosition(vCameraPosition.m_tX,vCameraPosition.m_tY,vCameraPosition.m_tZ);
	...
	else
	{
		...
		CQGlobe_Vector3DF vLookAtCenter = (-vCameraPosition);

		if(m_pCamera)
		{
			CQGlobe_Vector3DF UpVector = vLookAt.crossProduct(vLookAt.crossProduct(vLookAtCenter));
			CQGlobe_Point3DF  CameraPosition(vCameraPosition.m_tX,vCameraPosition.m_tY,vCameraPosition.m_tZ);
			m_pCamera->LookAt(&CameraPosition,&vTargetPosition,&UpVector);
		}
...
void CQGlobe_PathSim::OnKeyPressEvent(KeyState keyState)
{
