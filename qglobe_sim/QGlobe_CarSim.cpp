...
CQGlobe_CarSim::CQGlobe_CarSim(CQGlobe_SimMgr* a_pSimMgr) : CQGlobe_Simulator(a_pSimMgr)
{
	m_pCar = new CQGlobe_Car();	
	qglobe_TileListSelector->SetLRUUsage(true);
}

CQGlobe_CarSim::~CQGlobe_CarSim()
{
}

void	CQGlobe_CarSim::InitSimulation()
{
...
bool	CQGlobe_CarSim::AdvanceSimulation(IN double a_dt)
{
...
void CQGlobe_CarSim::EndSimulation()
{
	if (m_pCar)
	{
		delete m_pCar;
	}	
	qglobe_TileListSelector->SetLRUUsage(false);

}

void CQGlobe_CarSim::GetSimulationStatus(OUT QGlobe_SIM_STATUS* a_pSimStatus)
{
...
void CQGlobe_CarSim::OnKeyPressEvent(KeyState keyState)
{
	QGlobe_CAR_MOTION carMotion = CAR_MOTION_NONE;
	QGlobe_CAR_STEER carSteer = CAR_STEER_MID;
