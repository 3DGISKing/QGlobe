...
FS_Factor
CQGlobe_FlightSim::ComputeRotationFactor( void )
{
...
void
CQGlobe_FlightSim::RotateAircraft( FS_Millisecond a_Milliseconds )
{
...
void
CQGlobe_FlightSim::ComuteTotalForce()
{
	...
	qglobe_VectorSCStoTCS( &coord, &thrustForce );
	qglobe_VectorSCStoTCS( &coord, &dragForce );
	qglobe_VectorSCStoTCS( &coord, &liftForce );
	qglobe_VectorSCStoTCS( &coord, &weightForce );
...
void
CQGlobe_FlightSim::ApplyForce( FS_Millisecond a_Milliseconds )
{
	...
	qglobe_TCStoSCS( &coord, &accelAngle );
	...
	//if ( angleMag > 0.1 * FS_DEG2RAD )
	{
		...
		qglobe_RotateAroundVector( &coord, angleMag, angle );
	}
	...
	angleV		= angle;
	qglobe_VectorSCStoTCS( &coord, &forceV );
	qglobe_VectorSCStoTCS( &coord, &accelV );
	qglobe_VectorSCStoTCS( &coord, &velocityV );
	qglobe_VectorSCStoTCS( &coord, &angleV );
...
bool
CQGlobe_FlightSim::HasVelocity( void )
{
	return FS_Speed2KPH( m_Speed ) > FS_SPEED_MINIMAL;
}

FS_Degree
CQGlobe_FlightSim::ComputeAngleOfAttack( bool a_hasVelocity )
{
	if ( a_hasVelocity )
	{
...
void
CQGlobe_FlightSim::HandleCollision()
{
