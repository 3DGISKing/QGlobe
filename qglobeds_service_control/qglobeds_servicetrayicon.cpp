...
#if USE_DIALOG
QGlobeDSServiceTrayIcon::QGlobeDSServiceTrayIcon(QApplication *qapp, CtrlDialog *qdlg)
#else
QGlobeDSServiceTrayIcon::QGlobeDSServiceTrayIcon(QApplication *qapp)
#endif
{
	myQapp=qapp;
#if USE_DIALOG
	...
	setMyIcon(QString("/qglobeserver.png"));
...
QGlobeDSServiceTrayIcon::~QGlobeDSServiceTrayIcon(void)
{
}

void QGlobeDSServiceTrayIcon::initMenu(QMenu *_menu)
{
	if (_menu)
	{
		QAction* act = NULL;
...
QString QGlobeDSServiceTrayIcon::GetResource(const QString& fileName)
{
	QString workDir;
...
void QGlobeDSServiceTrayIcon::setMyIcon(const QString fileName)
{
...
void QGlobeDSServiceTrayIcon::trayActivated(QSystemTrayIcon::ActivationReason reason)
{
	switch(reason)
	{
...
void QGlobeDSServiceTrayIcon::menuOption(QAction * action){
...
void QGlobeDSServiceTrayIcon::serviceQuery(const QString& serviceName, const E_SVC_QUERY_TYPE queryType)
{
	if (!serviceName.isEmpty())
	{
		QGlobeDSServiceController gdsServiceController(serviceName);
...
void QGlobeDSServiceTrayIcon::updateUIServiceStatus(const QString& serviceName, const E_SVC_QUERY_TYPE queryType)
{
	if (!serviceName.isEmpty())
	{
		QGlobeDSServiceController gdsServiceController(serviceName);
