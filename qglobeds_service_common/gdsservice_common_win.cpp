...
bool QGlobeDSServiceController::isInstalled() const
{
...
bool QGlobeDSServiceController::isRunning() const
{
    Q_D(const QGlobeDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
QString QGlobeDSServiceController::serviceFilePath() const
{
    Q_D(const QGlobeDSServiceController);
    QString result;
    if (!winServiceInit())
...
QString QGlobeDSServiceController::serviceDescription() const
{
    Q_D(const QGlobeDSServiceController);
    QString result;
    if (!winServiceInit())
...
QGlobeDSServiceController::StartupType QGlobeDSServiceController::startupType() const
{
    Q_D(const QGlobeDSServiceController);
...
bool QGlobeDSServiceController::uninstall()
{
    ...
    if (hSCM) {
        // Try to open the service
        SC_HANDLE hService = pOpenService(hSCM, (wchar_t *)d->serviceName.utf16(), SERVICE_ALL_ACCESS);	// DELETE
        if (hService) {
...
bool QGlobeDSServiceController::start(const QStringList &args)
{
    Q_D(QGlobeDSServiceController);
    bool result = false;
    if (!winServiceInit())
    ...
    // Open the Service Control Manager
    SC_HANDLE hSCM = pOpenSCManager(0, 0, SC_MANAGER_ALL_ACCESS); // SC_MANAGER_CONNECT
    if (hSCM) {
        // Try to open the service
        SC_HANDLE hService = pOpenService(hSCM, (wchar_t *)d->serviceName.utf16(), SERVICE_ALL_ACCESS);	// SERVICE_START
        if (hService) {
...
bool QGlobeDSServiceController::stop()
{
    Q_D(QGlobeDSServiceController);
    bool result = false;
    if (!winServiceInit())
    ...
    SC_HANDLE hSCM = pOpenSCManager(0, 0, SC_MANAGER_ALL_ACCESS);	// SC_MANAGER_CONNECT
    if (hSCM) {
        SC_HANDLE hService = pOpenService(hSCM, (wchar_t *)d->serviceName.utf16(), SERVICE_ALL_ACCESS); // SERVICE_STOP|SERVICE_QUERY_STATUS
        if (hService) {
...
bool QGlobeDSServiceController::pause()
{
    Q_D(QGlobeDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
bool QGlobeDSServiceController::resume()
{
    Q_D(QGlobeDSServiceController);
    bool result = false;
    if (!winServiceInit())
...
bool QGlobeDSServiceController::sendCommand(int code)
{
   Q_D(QGlobeDSServiceController);
   bool result = false;
   if (!winServiceInit())
...
void QGlobeDSServiceBase::logMessage(const QString &message, MessageType type,
...
class QGlobeDSServiceControllerHandler : public QObject
{
    Q_OBJECT
public:
    QGlobeDSServiceControllerHandler(QtServiceSysPrivate *sys);
...
public:
    ...
    void setServiceFlags(QGlobeDSServiceBase::ServiceFlags flags);
    DWORD serviceFlags(QGlobeDSServiceBase::ServiceFlags flags) const;
    ...
    QGlobeDSServiceControllerHandler *controllerHandler;

    void handleCustomEvent(QEvent *e);
};

QGlobeDSServiceControllerHandler::QGlobeDSServiceControllerHandler(QtServiceSysPrivate *sys)
    : QObject(), d_sys(sys)
{

}

void QGlobeDSServiceControllerHandler::customEvent(QEvent *e)
{
...
{
    if (!instance || !QGlobeDSServiceBase::instance())
        return;
    ...
    instance->serviceStatus = pRegisterServiceCtrlHandler((TCHAR*)QGlobeDSServiceBase::instance()->serviceName().utf16(), handler);
    ...
    handler(QTSERVICE_STARTUP); // Signal startup to the application -
                                // causes QGlobeDSServiceBase::start() to be called in the main thread
...
void QtServiceSysPrivate::handleCustomEvent(QEvent *e)
{
    ...
    case QTSERVICE_STARTUP: // Startup
        QGlobeDSServiceBase::instance()->start();
        break;
    case SERVICE_CONTROL_STOP:
        QGlobeDSServiceBase::instance()->stop();
        QCoreApplication::instance()->quit();
        break;
    case SERVICE_CONTROL_PAUSE:
        QGlobeDSServiceBase::instance()->pause();
        break;
    case SERVICE_CONTROL_CONTINUE:
        QGlobeDSServiceBase::instance()->resume();
        break;
    default:
	if (code >= 128 && code <= 255)
	    QGlobeDSServiceBase::instance()->processCommand(code - 128);
        break;
    }
...
void WINAPI QtServiceSysPrivate::handler( DWORD code )
{
    ...
    instance->mutex.lock();
    switch (code) {
    case QTSERVICE_STARTUP: // QGlobeDSService startup (called from WinMain when started)
...
void QtServiceSysPrivate::setServiceFlags(QGlobeDSServiceBase::ServiceFlags flags)
{
    if (!available())
        return;
...
DWORD QtServiceSysPrivate::serviceFlags(QGlobeDSServiceBase::ServiceFlags flags) const
{
    DWORD control = 0;
    if (flags & QGlobeDSServiceBase::CanBeSuspended)
        control |= SERVICE_ACCEPT_PAUSE_CONTINUE;
    if (!(flags & QGlobeDSServiceBase::CannotBeStopped))
        control |= SERVICE_ACCEPT_STOP;
    return control;
}

#include "gdsservice_common_win.moc"
    ...
    void run()
        {
            ...
            st[0].lpServiceName = (wchar_t*)QGlobeDSServiceBase::instance()->serviceName().utf16();
            ...
            if (!success) {
                if (GetLastError() == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
                    ...
                    QGlobeDSServiceBase::instance()->logMessage(QString("The Service failed to start [%1]").arg(qt_error_string(GetLastError())), QGlobeDSServiceBase::Error);
                }
...
*/
class QGlobeDSServiceNativeEventFilter : public QAbstractNativeEventFilter
{
public:
    bool nativeEventFilter(const QByteArray &eventType, void *message, long *result) override;
};

bool QGlobeDSServiceNativeEventFilter::nativeEventFilter(const QByteArray &eventType, void *message, long *result)
{
...
bool QGlobeDSServiceBasePrivate::start()
{
    sysInit();
    if (!winServiceInit())
        return false;
    ...
    if (!app)
        return false;
    static QGlobeDSServiceNativeEventFilter nativeEventFilter;
    app->installNativeEventFilter(&nativeEventFilter);

    sys->controllerHandler = new QGlobeDSServiceControllerHandler(sys);
...
bool QGlobeDSServiceBasePrivate::install(const QString &account, const QString &password)
{
    bool result = false;
    if (!winServiceInit())
        return result;
    ...
    if (hSCM) {
        QString acc = account;
        DWORD dwStartType = startupType == QGlobeDSServiceController::AutoStartup ? SERVICE_AUTO_START : SERVICE_DEMAND_START;
...
QString QGlobeDSServiceBasePrivate::filePath() const
{
...
bool QGlobeDSServiceBasePrivate::sysInit()
{
...
void QGlobeDSServiceBasePrivate::sysSetPath()
{

}

void QGlobeDSServiceBasePrivate::sysCleanup()
{
    if (sysd) {
        delete sysd;
        sysd = 0;
...
void QGlobeDSServiceBase::setServiceFlags(QGlobeDSServiceBase::ServiceFlags flags)
{
