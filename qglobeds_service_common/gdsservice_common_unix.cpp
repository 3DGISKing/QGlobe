...
bool QGlobeDSServiceController::start(const QStringList &arguments)
{
    if (!isInstalled())
        return false;
    if (isRunning())
...
QtServiceSysPrivate::QtServiceSysPrivate()
    : QGlobeDSUnixServerSocket(), ident(0), serviceFlags(0)
{
}
...
void QtServiceSysPrivate::slotReady()
{
	...
	if (cmd == QLatin1String("terminate")) {
            if (!(serviceFlags & QGlobeDSServiceBase::CannotBeStopped)) {
                QGlobeDSServiceBase::instance()->stop();
                QCoreApplication::instance()->quit();
                retValue = true;
            }
        } else if (cmd == QLatin1String("pause")) {
            if (serviceFlags & QGlobeDSServiceBase::CanBeSuspended) {
	    ...
	    cmd = cmd.mid(4);
            QGlobeDSServiceBase::instance()->processCommand(cmd.toInt());
            retValue = true;
	}
        QString retString;
        if (retValue)
...
//#include "gdsservice_common_unix.moc"
...
void QGlobeDSServiceBase::logMessage(const QString &message, QGlobeDSServiceBase::MessageType type,
			    int, uint, const QByteArray &)
{
    ...
    int st;
    switch(type) {
        case QGlobeDSServiceBase::Error:
	    st = LOG_ERR;
	    break;
        case QGlobeDSServiceBase::Warning:
            st = LOG_WARNING;
	    break;
        default:
	    st = LOG_INFO;
