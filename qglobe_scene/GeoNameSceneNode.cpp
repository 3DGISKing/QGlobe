...
#define QGLOBE_CREATE_FONT_SIZE	20
...
namespace geo
{
	...
	namespace scene
	{
		...
		{
			m_pKorFont	= new CGeoFont(QGLOBE_CREATE_FONT_SIZE, szKorFileName);
			m_pEngFont	= new CGeoFont(QGLOBE_CREATE_FONT_SIZE + 4, szEngFileName);

			m_fFontScale = 1.0f / QGLOBE_CREATE_FONT_SIZE;
			m_fontflag = flag;
		}
		...
		void CGeoNameSceneNode::InitTextures()
		{
			...
			{
				if(layer->GetMainType()==E_QGlobe_MAINDATA_TYPE_NAME)
				{
...
#define BLINKSTEP 5
		...
		{
			CQGlobe_TextNodeArr nameList;
			CQGlobe_TextMgr	*pTextMgr;
			...
			pTextMgr = qglobe_GetDataMgr()->m_pTextMgr;
			...
			{
				QGlobe_TextCell &nameCell = nameList[i].m_data;
				...
				if(nameCell.datatype == E_QGlobe_SUBDATA_TYPE_NAME_MOUNTAIN && nameCell.focusScale > 0.1)
					strText = QString("%1(%2m)").arg(strText).arg((int)nameCell.height);

				if(nameCell.datatype != E_QGlobe_SUBDATA_TYPE_NONE && nameCell.focusScale > 0.05)
				{
					CGIS_Node* node=qglobe_GetGISNode(nameCell.datatype,nameCell.key);
