...
QGlobe_MESH_TILE_ARR g_qglobe_MeshTileArr;

#define QGLOBE_MIN_RATE_DISP_LEVEL	8

unsigned char bc[QGLOBE_MAX_DEM_PT_COUNT][QGLOBE_MAX_DEM_PT_COUNT];
...
TreeListSelector	*qglobe_TileListSelector = NULL;	
...
{			
	...
	pVBuf = pVertexBuf;		
	LRUHead = 0;
	qglobe_CameraGetLocation(qglobe_GetDataMgr(),&m_CameraLocation);
}
...
void initBC(QGlobe_MESH_TILE *pMeshTile)
{	
	initBC(pMeshTile->nDivision);
	int step = QGLOBE_MAX_DEM_DIVISION/pMeshTile->nDivision, index = 0;
	int sp1 = 0, sp2 = sp1+QGLOBE_MAX_DEM_DIVISION, sp3 = QGLOBE_MAX_DEM_DIVISION3-step, sp4 = QGLOBE_MAX_DEM_DIVISION4-step;
...
QGlobe_POINT3D tmpPts[QGLOBE_MAX_DEM_PT_COUNT * QGLOBE_MAX_DEM_PT_COUNT];  // for overflow of call stack size

void qglobe_SetRenderParamForTexture(QGLOBE_RENDER_GEO_PARAM *pParam,QGlobe_MESH_TILE* pMeshTile,QGlobe_POINT3D *pBuf)
{
	int vertexindex = 0;
	...
	{		
		...
		for(int col = 0; col <= pMeshTile->nDivision; col++) //from left to right
		{
			...
			else
			{
				...
				pParam->pVertexBuf[vertexindex].TCoords.X = (float)(0.5 + dLongi / qglobe_PI / 2);
				pParam->pVertexBuf[vertexindex].TCoords.Y = (float)(0.5 - dLati / qglobe_PI);
			}
...
void qglobe_SetRenderParamForHeight(QGLOBE_RENDER_GEO_PARAM *pParam,QGlobe_MESH_TILE* pMeshTile,QGlobe_POINT3D *pBuf)
{
	int vertexindex = 0;
	...
	bool blheight;
	float height=0 ;
	float maxHeight = QGlobe_MAX_HEIGHT;
	float minHeight = 10;
...
void qglobe_SetRenderParamForRateAngle(QGLOBE_RENDER_GEO_PARAM *pParam,QGlobe_MESH_TILE* pMeshTile,QGlobe_POINT3D *pBuf)
{
	int vertexindex = 0;
	...
	float angle = 0; //0 ~ 90
	float maxAngle = QGlobe_GIS_MAX_ANGLE;
	...
	{		
		...
		for(int col = 0; col <= pMeshTile->nDivision; col++) //from left to right
		{
			...
			{
				angle = 0; //0 ~ 90
				if(blNormalInfo && g_qglobe_MeshTileArr.nLevel >= QGLOBE_MIN_RATE_DISP_LEVEL)
...
void qglobe_SetRenderParamForRatePlan(QGLOBE_RENDER_GEO_PARAM *pParam,QGlobe_MESH_TILE* pMeshTile,QGlobe_POINT3D *pBuf)
{
	int vertexindex = 0;
...
int	qglobe_GetRenderCallBackTerrain(QGLOBE_RENDER_GEO_PARAM *pParam)
{	
	Q_ASSERT(pParam!=NULL);

	if(pParam->nIndex >= g_qglobe_MeshTileArr.nCount)
		return 0;

	QGlobe_POINT3D *pBuf;
	
	QGlobe_MESH_TILE *pMeshTile;
			
	pMeshTile = &g_qglobe_MeshTileArr.pData[pParam->nIndex];
	...
	{		
		...
		for(int row = 0; row <= pMeshTile->nDivision; row++) //from bottom to top
		{
			...
			for(int col = 0; col <= pMeshTile->nDivision; col++) //from left to right 
			{				
				qglobe_get3DWorldPoint(dBaseLongitude, dBaseLatitude, tmpPts[nVertex++], 0, pMeshTile->pDemTile, true);
	...
	switch(qglobe_GetDataMgr()->m_sOption.layerTextureMode)
	{
		case LTM_TEXTURE:
		case LTM_RASTER:
			qglobe_SetRenderParamForTexture(pParam,pMeshTile,pBuf);
			break;
		case LTM_HEIGHT:
			qglobe_SetRenderParamForHeight(pParam,pMeshTile,pBuf);
			break;
		case LTM_RATEANGLE:
			qglobe_SetRenderParamForRateAngle(pParam,pMeshTile,pBuf);
			break;
		case LTM_RATEPLAN:
			qglobe_SetRenderParamForRatePlan(pParam,pMeshTile,pBuf);
	}
	...
	int step = QGLOBE_MAX_DEM_DIVISION/pMeshTile->nDivision;
	int sp1 = 0, sp2 = QGLOBE_MAX_DEM_DIVISION, sp3 = QGLOBE_MAX_DEM_DIVISION3-step, sp4 = QGLOBE_MAX_DEM_DIVISION4-step;
...
void qglobe_MatchNeighbor(QGlobe_MESH_TILE *pMeshTile,QGlobe_MESH_TILE *pOther,int type)
{
	int ox,oy,nx,ny,mx,my;	
	QGlobe_POINT3D *p1, *p2;
	unsigned char *m1, *m2, *n1, *n2;
	int step1 = QGLOBE_MAX_DEM_DIVISION/pMeshTile->nSubs;
	int step2 = QGLOBE_MAX_DEM_DIVISION/pOther->nSubs;
	...
	switch(type)
	{
		case 0:		//bottom-top : pMeshTile�� �����, pOther�� �Ʒ��� ��������
			p1 = &pMeshTile->mBP[nx]; p2 = &pOther->mBP[QGLOBE_MAX_DEM_DIVISION3-mx];
			m1 = &pMeshTile->mBC[nx]; m2 = &pOther->mBC[QGLOBE_MAX_DEM_DIVISION3-mx];
			n1 = &pMeshTile->mBU[nx]; n2 = &pOther->mBU[QGLOBE_MAX_DEM_DIVISION3-mx];							
			break;
		case 1:		//left-right : pMeshTile�� ���ʿ�, pOther�� �����ʿ� ����
			p1 = &pMeshTile->mBP[QGLOBE_MAX_DEM_DIVISION+ny]; p2 = &pOther->mBP[QGLOBE_MAX_DEM_DIVISION4-my];
			m1 = &pMeshTile->mBC[QGLOBE_MAX_DEM_DIVISION+ny]; m2 = &pOther->mBC[QGLOBE_MAX_DEM_DIVISION4-my];
			n1 = &pMeshTile->mBU[QGLOBE_MAX_DEM_DIVISION+ny]; n2 = &pOther->mBU[QGLOBE_MAX_DEM_DIVISION4-my];
			break;	
		default:
			break;
	}
	...
	for (i = 0;i < QGLOBE_MAX_DEM_PT_COUNT;i++)
	{
		if (i<=step1) {	n1[i] = 1;}
		if (i<=step2) { n2[-i] = 1;}
	}

	if (pMeshTile->mBU[0] && !pMeshTile->mBU[QGLOBE_MAX_DEM_DIVISION4])
	{
		pMeshTile->mBU[QGLOBE_MAX_DEM_DIVISION4] = 1;
		pMeshTile->mBP[QGLOBE_MAX_DEM_DIVISION4] = pMeshTile->mBP[0];
		pMeshTile->mBC[QGLOBE_MAX_DEM_DIVISION4] = pMeshTile->mBC[0];
	}

	if (!pMeshTile->mBU[0] && pMeshTile->mBU[QGLOBE_MAX_DEM_DIVISION4])
	{
		pMeshTile->mBU[0] = 1;
		pMeshTile->mBP[0] = pMeshTile->mBP[QGLOBE_MAX_DEM_DIVISION4];
		pMeshTile->mBC[0] = pMeshTile->mBC[QGLOBE_MAX_DEM_DIVISION4];
	}

	if (pOther->mBU[0] && !pOther->mBU[QGLOBE_MAX_DEM_DIVISION4])
	{
		pOther->mBU[QGLOBE_MAX_DEM_DIVISION4] = 1;
		pOther->mBP[QGLOBE_MAX_DEM_DIVISION4] = pOther->mBP[0];
		pOther->mBC[QGLOBE_MAX_DEM_DIVISION4] = pOther->mBC[0];
	}

	if (!pOther->mBU[0] && pOther->mBU[QGLOBE_MAX_DEM_DIVISION4])
	{
		pOther->mBU[0] = 1;
		pOther->mBP[0] = pOther->mBP[QGLOBE_MAX_DEM_DIVISION4];
		pOther->mBC[0] = pOther->mBC[QGLOBE_MAX_DEM_DIVISION4];
	}
}

void qglobe_MakeBounds(QGlobe_MESH_TILE* pMeshTile)
{		
	if (pMeshTile->nDemDivision >= 1 && pMeshTile->buf_dem)
	{	
		int row = QGLOBE_MAX_DEM_PT_COUNT*pMeshTile->nDivision+1;  
		int rcol = pMeshTile->nDivision, lcol = QGLOBE_MAX_DEM_PT_COUNT;
		int step = QGLOBE_MAX_DEM_DIVISION/pMeshTile->nDivision;
		int sp1 = 0, sp2 = QGLOBE_MAX_DEM_DIVISION, sp3 = QGLOBE_MAX_DEM_DIVISION3-step, sp4 = QGLOBE_MAX_DEM_DIVISION4-step;
		...
		{			
			...
			sp3-=step;
			sp4-=step;	
			rcol += QGLOBE_MAX_DEM_PT_COUNT;			
			lcol += QGLOBE_MAX_DEM_PT_COUNT;
		}
		pMeshTile->mBP[QGLOBE_MAX_DEM_DIVISION4] = pMeshTile->mBP[0];
	}
}

void qglobe_MakeExtraBounds(QGlobe_MESH_TILE* pMeshTile)
{	
	// calcu edges
	double tileWidth = qglobe_TwoPI / (1 << pMeshTile->nLevel);
	double cellWidth = tileWidth / pMeshTile->nDivision;
	...
	{		
		if (!pMeshTile->mBU[sp1])	//bottom
			qglobe_get3DWorldPoint(longi, dBaseLatitude, pMeshTile->mBP[sp1], 0, NULL, true);			

		if (!pMeshTile->mBU[sp2])	//right
			qglobe_get3DWorldPoint(rightlongi, lati, pMeshTile->mBP[sp2], 0, NULL, true);				

		longi+=cellWidth;
		lati +=cellWidth;
		if (!pMeshTile->mBU[sp3])	//top
			qglobe_get3DWorldPoint(longi, toplati, pMeshTile->mBP[sp3], 0, NULL, true);			

		if (!pMeshTile->mBU[sp4])	//left	
			qglobe_get3DWorldPoint(dBaseLongitude, lati, pMeshTile->mBP[sp4], 0, NULL, true);		
...
void qglobe_GetPyramidDivisionTiles(QGlobe_MESH_TILE_ARR *pArr)
{
	const QGlobe_LOGIC_TILE_ARR *pBaseArr = qglobe_GetRenderTiles();
	QGlobe_DEM_TILE *pDemTile;
	int i,j, nX, nY, nDelta;
	int nMaxX,base;
	double width;	
	QGlobe_MESH_TILE  *pMeshTile, *pOther;
	const QGlobe_LOGIC_TILE *pLogicTile;

	memset(pArr, 0, sizeof(QGlobe_MESH_TILE_ARR));
	...
	for(i= 0; i < pBaseArr->nCount; i++)
	{
		...
		if (pDemTile)
		{
			...
			{
				pMeshTile->nDemDivision = pMeshTile->nDivision = QGLOBE_MAX_DEM_DIVISION/(1<<nDelta);		
				nX = pMeshTile->nX - (pDemTile->m_sInfo.nX << nDelta);
				nY = pMeshTile->nY - (pDemTile->m_sInfo.nY << nDelta);
				base = QGLOBE_MAX_DEM_PT_COUNT * pMeshTile->nDivision * nY + pMeshTile->nDivision * nX;
				pMeshTile->buf_dem	  = qglobe_GetDataMgr()->m_pTerrainPtCache->GetPts(pDemTile) + base;				
				pMeshTile->buf_height = pDemTile->m_pData + base;
				if(qglobe_GetDataMgr()->IsNecessaryDirInfo())
					pMeshTile->buf_dirInfo = qglobe_GetDataMgr()->m_pTerrainDirCache->GetInfos(pDemTile) + base;	
				qglobe_MakeBounds(pMeshTile);
			}
			else
			{
				qglobe_MakeExtraBounds(pMeshTile);
				pMeshTile->mBP[QGLOBE_MAX_DEM_DIVISION4] = pMeshTile->mBP[0];
			}
		}
		else
		...
		if(qglobe_GetDataMgr()->m_sOption.layerTextureMode==LTM_TEXTURE && pLogicTile->pImgTile != NULL)
		{
			pMeshTile->nTextureID = qglobe_GetTextureIDCache()->GetTextureID(pLogicTile->pImgTile);
		...
		if(qglobe_GetDataMgr()->m_sOption.layerTextureMode==LTM_RASTER && pLogicTile->prasterTile != NULL)
		{
			pMeshTile->nTextureID = qglobe_GetRasterTextureIDCache()->GetTextureID(pLogicTile->prasterTile);
		...
		pMeshTile->nOverlayTextureID = qglobe_GetGISTextureIDCache()->GetTextureID(pLogicTile->nLevel,pLogicTile->nX,pLogicTile->nY);
	}
	...
	{				
		...
		for (j = i+1;j < pArr->nCount;j++)
		{
			...
			if (pOther->nSubs > QGLOBE_MAX_DEM_DIVISION || pMeshTile->nSubs > QGLOBE_MAX_DEM_DIVISION)
				continue;
				...
				(pMeshTile->nSubX >= pOther->nSubX) && (pMeshTile->nSubX+pMeshTile->nSubs <= pOther->nSubX+pOther->nSubs))
			{
				...
				if (pMeshTile->nSubY == pOther->nSubY+pOther->nSubs || pOther->nSubY == pMeshTile->nSubY+pMeshTile->nSubs){	
					...
					if (pMeshTile->nSubY == pOther->nSubY+pOther->nSubs)
						qglobe_MatchNeighbor(pMeshTile,pOther,0);										
					else if (pOther->nSubY == pMeshTile->nSubY+pMeshTile->nSubs)
						qglobe_MatchNeighbor(pOther, pMeshTile,0);		
				...
				(pMeshTile->nSubY >= pOther->nSubY) && (pMeshTile->nSubY+pMeshTile->nSubs <= pOther->nSubY+pOther->nSubs))
			{
				...
				if (pMeshTile->nSubX == (pOther->nSubX+pOther->nSubs)%nMaxX || pOther->nSubX == (pMeshTile->nSubX+pMeshTile->nSubs)%nMaxX){						
					...
					if (pMeshTile->nSubX == (pOther->nSubX+pOther->nSubs)%nMaxX )
						qglobe_MatchNeighbor(pOther,pMeshTile,1);																							
					else if (pOther->nSubX == (pMeshTile->nSubX+pMeshTile->nSubs)%nMaxX)
						qglobe_MatchNeighbor(pMeshTile,pOther,1);
				}	
			}			
		}		
	}
