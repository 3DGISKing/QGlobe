...
void RenderView::OnResizeGL(int width, int height )
{
	...
	qglobe_GetDataMgr()->m_pTextMgr->Init(width, height);
}

void RenderView::OnPaint(QGlobe_RENDER_INFO *renderInfo)
{
	if(_RenderSceneMgr == 0)
		return;
...
void RenderView::UpdateCameraInfo(QGlobe_Coord3D& coord, QGlobe_ViewFrustum& viewFrustum)
{
	if(_RenderSceneMgr)
	{
