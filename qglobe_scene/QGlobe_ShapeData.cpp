...
#include "../qglobe_base/QGlobe_Line2D.h"
//#include "../qglobe_base/QGlobe_Line3D.h"
//end addition
...
void qglobe_AddVertexToPolygon(double longitude, double latitude, double offsetRadius, LineSegment* polygonData, QGlobe_DEM_TILE* pDemTile)
{
	QGlobe_POINT3D pt;
	qglobe_get3DBodyPoint(longitude, latitude, pt, offsetRadius, pDemTile, true);
	...
	qglobe_GetFPSTester()->m_totalLineCount++;
}
...
inline void GetExactPtsInLogicTile(const QGlobe_LOGIC_TILE *ptile, CQGlobe_Line2DF* line, 
								   geo::core::array<geo::core::vector3df> *ptArr)
{
	QGlobe_DEM_TILE * pDem;
	pDem = ptile->pDemTile;
	...
	fstInside = (prevX >= 0 && prevX < QGLOBE_MAX_DEM_PT_COUNT && prevY >= 0 && prevY < QGLOBE_MAX_DEM_PT_COUNT);
	sndInside = (curX >= 0 && curX < QGLOBE_MAX_DEM_PT_COUNT && curY >= 0 && curY < QGLOBE_MAX_DEM_PT_COUNT);

	int startX = qMin(prevX, curX); startX = qMax(startX+1, 0);
	int startY = qMin(prevY, curY); startY = qMax(startY+1, 0);
	int endX = qMax(prevX, curX); endX = qMin(endX, QGLOBE_MAX_DEM_DIVISION);
	int endY = qMax(prevY, curY); endY = qMin(endY, QGLOBE_MAX_DEM_DIVISION);

	QList<CROSSINGPOINT> inPtArr;
	CQGlobe_Vector2DF startPt, endPt, crossPt;
	CQGlobe_Line2DF tstLine;
	...
	if (qglobe_GetDataMgr()) rate = qglobe_GetDataMgr()->m_sOption.dem_detail_rate;
	...
	for (iX = startX; iX < (endX+1); iX++)
	{
		...
		{		
			...
			h1 = pDem->m_pData[iY * QGLOBE_MAX_DEM_PT_COUNT + iX];
			h2 = pDem->m_pData[(iY + 1)*QGLOBE_MAX_DEM_PT_COUNT + iX];
	...
	for (iY = startY; iY < (endY+1); iY++)
	{
		...
		if (line->intersectWith(tstLine, crossPt, INTERSECTING_TOLERANCE))
		{
			...
			h1 = pDem->m_pData[iY * QGLOBE_MAX_DEM_PT_COUNT + iX];
			h2 = pDem->m_pData[iY * QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
	...
	int startTri, endTri;
	startTri = qMin(prevY - prevX,curY - curX); startTri = qMax(startTri, -QGLOBE_MAX_DEM_DIVISION);
	endTri   = qMax(prevY - prevX,curY - curX); endTri   = qMin(endTri, QGLOBE_MAX_DEM_DIVISION);
	...
	for (int i=startTri; i< endTri+2; i++)
	{
		...
		if (line->intersectWith(tstLine, crossPt, INTERSECTING_TOLERANCE))
		{
			...
			h1 = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX];
			h2 = pDem->m_pData[(iY+1)*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
	...
	endTri   = qMax(prevX + prevY, curX + curY); endTri   = qMin(endTri, 2*QGLOBE_MAX_DEM_DIVISION);
	...
	for (int i=startTri; i<endTri+2; i++)
	{
		...
		if (line->intersectWith(tstLine, crossPt, INTERSECTING_TOLERANCE))
		{
			...
			h1 = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
			h2 = pDem->m_pData[(iY + 1)*QGLOBE_MAX_DEM_PT_COUNT + iX];
	...
	if (fstInside)
	{
		...
		if ((iX % 2 == 0 && iY % 2 == 0) || (iX % 2 != 0 && iY %2 != 0))
		{
			if (dX > dY)
			{
				...
				height[0] = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX];
				height[1] = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
				height[2] = pDem->m_pData[(iY+1)*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
			}
			else
			{
				...
				height[0] = pDem->m_pData[(iY+1)*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
				height[1] = pDem->m_pData[(iY+1)*QGLOBE_MAX_DEM_PT_COUNT + iX];
				height[2] = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX];
			}
		}
		else
		{
			if ((dX + dY) < pDem->gridWidth)
			{
				...
				height[0] = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
				height[1] = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX];
				height[2] = pDem->m_pData[(iY+1)*QGLOBE_MAX_DEM_PT_COUNT + iX];
			}
			else
			{
				...
				height[0] = pDem->m_pData[(iY+1)*QGLOBE_MAX_DEM_PT_COUNT + iX];
				height[1] = pDem->m_pData[(iY+1)*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
				height[2] = pDem->m_pData[iY*QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
			}
		}
	...
	{
		qglobe_get3DBodyPoint(inPtArr.at(i).pt.m_tX, inPtArr.at(i).pt.m_tY, pos, inPtArr.at(i).pt.m_tZ);
		ptArr->push_back(geo::core::vector3df(pos.m_X, -pos.m_Y, pos.m_Z));
	}
}
//*/
//end addition by RSH
void	convert3d(const QGlobe_LOGIC_TILE *ptile, LineSegment* linesegment)
{   	
	...
	double latestnonheight=0.0;      //���� �װ��� ������ 0�� ʭ�� ���˰�
	...
	{
		CQGlobe_Vector2DF prevPt, curPt;
		CQGlobe_Line2DF line;
	...
	{		
		...
		{
		   height=qglobe_getHeight(linesegment->pts[i].X, linesegment->pts[i].Y);
		...
		for(unsigned int i=0;i<linesegment->pts.size();i++)
		{
			 qglobe_get3DBodyPoint(linesegment->pts[i].X, linesegment->pts[i].Y, pt, linesegment->pts[i].Z+offsetheight);
	...
	arrdivpt.push_back(linesegment->pts[0]);  //˧�� �󱨼�˾ �Ĳ���.
	double dist;
	vector3df tmp;
	...
	for(unsigned int i=1;i<linesegment->pts.size();i++)
	{
		dist=getDistance(linesegment->pts[i-1].X,linesegment->pts[i-1].Y,
			             linesegment->pts[i].X,linesegment->pts[i].Y);//������Ĵ��̩ ������ �˺��ٳ�.
		dist = abs(dist);
	...
	{
		qglobe_get3DBodyPoint(arrdivpt[i].X, arrdivpt[i].Y, pt, arrdivpt[i].Z+offsetheight);
...
void get2dVertexLinesegmentArray(const QGlobe_LOGIC_TILE *ptile, PolyLinesegment* linesegment_arr)
{
	QGlobe_POINT2D *vertexbuf;
	const QGlobe_SHAPE_TILE* pShapeTile;
	LineSegment* be_writing_linesegment; 
	QGlobe_POLYGON *current_source_linesegment;
	...
	for(int i = 0; i < ptile->nShapeTileCnt; i++)
	{
		...
		for(int j = 0; j < pShapeTile->m_nPolygonCount; j++)
		{
			...
			current_source_linesegment = (QGlobe_POLYGON*)current_shapetile_pos;

			vertexbuf = (QGlobe_POINT2D*)((int*)current_source_linesegment + 1); // count;
			...
			for(unsigned int k = 0; k < current_source_linesegment->m_nCount; k++)
			{
				...
				//	{
				//		be_writing_linesegment->pts.push_back(vector3df(longitude+qglobe_PI,qglobe_PI-latitude,0))	;
				...
				//	be_writing_linesegment->pts.push_back(vector3df(preLongitude+qglobe_PI, qglobe_PI-preLatitude,0));
			...
			current_shapetile_pos += 4/*count*/ + sizeof(QGlobe_POINT2D) * current_source_linesegment->m_nCount;
		}
	}
...
void	get3dVertexLinesegmentArray(const QGlobe_LOGIC_TILE *ptile, PolyLinesegment* linesegment_arr)
{
	QGlobe_POINT2D *vertexbuf;
	const QGlobe_SHAPE_TILE* pShapeTile;
	LineSegment* be_writing_linesegment; 
	...
	CQGlobe_Vector2DF crossPt;
	bool isOutPt = false;
	CQGlobe_Line2DF bndLines[4], line;
	bndLines[0].setLine(CQGlobe_Vector2DF(ptile->minLongitude, ptile->minLatitude), CQGlobe_Vector2DF(ptile->minLongitude, ptile->maxLatitude));
	bndLines[1].setLine(CQGlobe_Vector2DF(ptile->minLongitude, ptile->minLatitude), CQGlobe_Vector2DF(ptile->maxLongitude, ptile->minLatitude));
	bndLines[2].setLine(CQGlobe_Vector2DF(ptile->maxLongitude, ptile->minLatitude), CQGlobe_Vector2DF(ptile->maxLongitude, ptile->maxLatitude));
	bndLines[3].setLine(CQGlobe_Vector2DF(ptile->minLongitude, ptile->maxLatitude), CQGlobe_Vector2DF(ptile->maxLongitude, ptile->maxLatitude));
   	...
   	for(int i = 0; i < ptile->nShapeTileCnt; i++)
	{
		...
		for(int j = 0; j < pShapeTile->m_nPolygonCount; j++)
		{
			...
			vertexbuf = (QGlobe_POINT2D*)((int*)current_source_linesegment + 2 + 8/*added by RSH 2013.3.3 for bound. */); // count,id, bound;
			be_writing_linesegment->type = pShapeTile->m_sInfo.eSubType;

			if(polyMaxX < ptile->minLongitude || polyMinX > ptile->maxLongitude
							|| polyMaxY < ptile->minLatitude || polyMinY > ptile->maxLatitude)
			{
				current_shapetile_pos += 4/*count*/+4/*id*/ + 32/*bounding box: 4 * double*/ + sizeof(QGlobe_POINT2D) * current_source_linesegment->m_nCount;
				continue;
			}	
			//end addition by RSH
			isOutPt = false;
			for(unsigned int k = 0; k < current_source_linesegment->m_nCount; k++)
			{
				...
				{	
					...
					line.setLine(CQGlobe_Vector2DF(vertexbuf[k-1].m_X, vertexbuf[k-1].m_Y), CQGlobe_Vector2DF(vertexbuf[k].m_X, vertexbuf[k].m_Y));
					...
					{
						CQGlobe_Line2DF line;
						line.setLine(CQGlobe_Vector2DF(preLongitude, preLatitude), CQGlobe_Vector2DF(longitude, latitude));
					...
					else if (k != current_source_linesegment->m_nCount-1)	//���켰�� ʭ������
					{
						// ������˾ �ֵ��ٳ�.
...
//̺��̩ ����
int	qglobe_GetGeoShapeInfo(int* pIndex, PolyLinesegment* pGeoShapeArray)
{
	int nTileIndex;	
	const QGlobe_LOGIC_TILE *pTile;	
	const QGlobe_LOGIC_TILE_ARR *p_shape_tile_list;
	double baseOffset;
	QGlobe_POINT2D *pVertex;

	p_shape_tile_list = qglobe_GetRenderTiles();
	...
	qglobe_CameraGetLocation(qglobe_GetDataMgr(), &location);
	...
	// get offset
	baseOffset = qglobe_GetDataMgr()->m_pCamera->m_location.m_dDist / 50.0;
	...
	for(int i = 0; i < pTile->nShapeTileCnt; i++)
	{
		...
		const QGlobe_SHAPE_TILE* pShapeTile = pTile->sArrShapeTile[i];
		if(pShapeTile == NULL)
			continue;
		...
		if(0)
		{
			...
			polyIndex++;
			double tileWidth = qglobe_TwoPI / (1 << pShapeTile->m_sInfo.nLevel);

			double minLongitude = tileWidth * pShapeTile->m_sInfo.nX - qglobe_PI;
			double minLatitude	= tileWidth * pShapeTile->m_sInfo.nY - qglobe_PI;
			qglobe_AddVertexToPolygon(minLongitude, minLatitude, baseOffset, polygonData, NULL);
			qglobe_AddVertexToPolygon(minLongitude + tileWidth, minLatitude, baseOffset, polygonData, NULL);
			qglobe_AddVertexToPolygon(minLongitude + tileWidth, minLatitude + tileWidth, baseOffset, polygonData, NULL);
			qglobe_AddVertexToPolygon(minLongitude, minLatitude + tileWidth, baseOffset + 100, polygonData, NULL);
		...
		{		
			...
			for(int j = 0; j < pShapeTile->m_nPolygonCount; j++)
			{
				...
				polygonData = &pGeoShapeArray->polyList[polyIndex];
				QGlobe_POLYGON *pPolygon = (QGlobe_POLYGON*)pData;
				
				pVertex = (QGlobe_POINT2D*)((int*)pPolygon + 1); // count;
				...
				for(unsigned int k = 0; k < pPolygon->m_nCount; k++)
				{
					...
					if (0)
					{
						...
						minLon = 76.122619 * qglobe_DEGTORAD;
						maxLon = 77.736738 * qglobe_DEGTORAD;
						minLat = 31.598381 * qglobe_DEGTORAD;
						maxLat = 33.118941 * qglobe_DEGTORAD;
					...
					{						
						...
						{
							qglobe_AddVertexToPolygon(longitude, latitude, baseOffset, polygonData, NULL);
					...
					{
						qglobe_AddVertexToPolygon(preLongitude, preLatitude, baseOffset, polygonData, NULL);
					}

					qglobe_AddVertexToPolygon(longitude, latitude, baseOffset, polygonData, pTile->pDemTile);
					preLongitude = longitude;
					preLatitude = latitude;
				}
				...
				pData += 4/*count*/ + sizeof(QGlobe_POINT2D) * pPolygon->m_nCount;
			}
