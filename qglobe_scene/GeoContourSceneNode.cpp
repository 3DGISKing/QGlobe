...
extern int	qglobe_GetGeoContourInfo(int* pIndex, CONTOUR_POLYGON_LIST* pGeoContourArray);
...
namespace geo
{
	namespace scene
	{
		...
		{
			if(!(qglobe_GetDataMgr()->m_sOption.blContour)) 	
				return ;
			...
			while(1)
			{
				...
				nCount = qglobe_GetGeoContourInfo(&nTileIndex, &polyArray);
