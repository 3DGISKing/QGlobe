...
extern void qglobe_MakeRenderGeoNameData();
extern void qglobe_PreparePlacemarkData();
extern void qglobe_PrepareLinkLenAreaName();//for custom

QGlobe_SceneManager* g_SceneManager=NULL;
...
QGlobe_SceneManager::QGlobe_SceneManager(void)
{
	g_SceneManager=this;
	m_pDevice = NULL;
	m_pDriver = NULL;
	...
	// create texturemgr
	m_pTextureMgr			= new CQGlobe_TextureMgr;
	m_pGisTextureMgr		= new CGIS_TextureMgr;

	// create texture id cache
	m_pTextureIdCache		= new CQGlobe_TextureIDCache;
...
void QGlobe_SceneManager::Initialize()
{
	qglobe_GetDataMgr()->SetTextureIDCache(m_pTextureIdCache);
}

bool QGlobe_SceneManager::registerRenderEngine(QOpenGLWidget* widget)
{
	...
	qglobe_GetTextureMgr()->SetVideoDriver(m_pDriver);
	qglobe_GetGISTextureMgr()->SetVideoDriver(m_pDriver);

	qglobe_GetRasterTextureMgr()->SetVideoDriver(m_pDriver);
	
	return true;
}

void QGlobe_SceneManager::unregisterRenderEngine()
{
	if(m_pDevice)
	{
...
//Terrain, Shape, SkyBox�� SceneGraph�� �����Ѵ�.
bool QGlobe_SceneManager::prepareSceneGraph()
{
	if(!m_pSceneMgr)
		return false;
	...
	if (m_pGeoTerrainSceneNode)
	{
		...
		QString workDir = qglobe_GetWorkPath();
...
void QGlobe_SceneManager::setupEnv()
{
...
void QGlobe_SceneManager::setCameraPos(geo::core::vector3df& absPos, geo::core::vector3df& pos, geo::core::vector3df& target, geo::core::vector3df& up)
{
	if(m_pCamera)
	{
...
void QGlobe_SceneManager::setViewFrustum(geo::f64 nearValue, geo::f64 farValue, geo::f64 fov, geo::f64 aspect)
{
	if(m_pCamera)
	{
...
void QGlobe_SceneManager::onBeforeRender()
{
	...
	qglobe_PrepareLinkLenAreaName();//for custom

	if(qglobe_GetGISDoc()->IsModified())
		qglobe_GetGISTextureIDCache()->Clear();
}

void QGlobe_SceneManager::onAfterRender()
{
...
void QGlobe_SceneManager::addGISRenderNode()
{
	m_pGeoGISNode = new CGeoGISSceneNode(m_pSceneMgr, m_pSceneMgr, -1);
	m_pGeoGISNode->drop();
}

void QGlobe_SceneManager::setFont(QString szKorFileName, QString szEngFileName, unsigned int flag)
{
	if(m_pGeoNameNode)
...
void QGlobe_SceneManager::setSimMgr(CQGlobe_SimMgr *pMgr)
{
	m_pSimMgr = pMgr;
	if(m_pGeoSimNode)
...
unsigned long qglobe_GetTickTimer()
{
	return geo::os::Timer::getRealTime();
}

bool QGlobe_SceneManager::renderScene()
{
...
void QGlobe_SceneManager::setGlobalLightPosition(double longi,double lat,double height)
{
	QGlobe_LOCATION loc;

	loc.m_dLongitude=-qglobe_DEGTORAD*longi;
	loc.m_dLatitude=qglobe_DEGTORAD*lat;
	loc.m_dDist=height*1000;

	CQGlobe_Point3DF tmp;

	qglobe_GetPosition(&loc,&tmp);

	m_pLight->SetLightPosition(vector3df(tmp.m_tX,tmp.m_tY,tmp.m_tZ));
}

void QGlobe_SceneManager::setGlobalLightColor(QColor c)
{
...
void QGlobe_SceneManager::setAmbientColor(QColor c)
{
...
void QGlobe_SceneManager::setLightAttenuation(float c,float l,float q)
{
	m_pLight->SetLightAttenuation(c,l,q);
}

void QGlobe_SceneManager::enableLighting(bool enable/* =true */)
{
