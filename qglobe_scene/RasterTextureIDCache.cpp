...
RasterTextureIDCache * qglobe_GetRasterTextureIDCache(){return g_pRasterTextureIDCache;}
...
long RasterTextureIDCache::GetTextureID(QGlobe_TILE *tile)
{
	long textureID;
	int key1, key2;
	...
	// generate texture id
	textureID = qglobe_GetRasterTextureMgr()->AddTexture(tile->m_sInfo.nX, tile->m_sInfo.nY, tile->m_sInfo.nLevel, (void*)&im);
...
void RasterTextureIDCache::FreeData(void *pData)
{
	...
	qglobe_GetRasterTextureMgr()->DelTexture(textureID);
}
