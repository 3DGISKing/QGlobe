...
double CGeoGISSceneNode::GetOffsetRadius(CGIS_Node *pNode)
{
	...
	double offset = 0;
	// get offset
	offset = qglobe_GetDataMgr()->m_pCamera->m_location.m_dDist / 200.0;
...
void CGeoGISSceneNode::DrawTrackPts()
{
	...
	for(id = 0; id < count; id++)
	{
		...
		loc.m_dDist = pt.m_tZ;
		qglobe_GetPosition(&loc, &pos);
		if(qglobe_IsBackFace(pos.m_tX, pos.m_tY, pos.m_tZ))
			continue;
		qglobe_WorldToScreen(&pos);
...
void CGeoGISSceneNode::DrawPolyNode(CGIS_Polygon *pPolyNode)
{
	...
	pPolyNode->GetDrawSegPoints(locArr, qglobe_GetDataMgr()->m_pCamera);

	count = locArr.size();

	for(index = 0; index < count; index++)
	{
		qglobe_get3DBodyPoint(locArr[index].m_tX, locArr[index].m_tY, pos, locArr[index].m_tZ + m_fOffsetRadius);
		pt.X = pos.m_X;
		pt.Y = - pos.m_Y;
		pt.Z = pos.m_Z;
	...
	CQGlobe_MeshArr *pMeshArr = pPolyNode->GetDrawCoverMesh();
	if(pMeshArr->size() > 0)
...
void CGeoGISSceneNode::DrawCoverPolygon(CQGlobe_MeshArr &meshs, video::SColor clr)
{
	...
	for(i = 0; i < count; i++)
	{
		qglobe_get3DBodyPoint(meshs[i].pt1.m_tX, meshs[i].pt1.m_tY, pos, meshs[i].pt1.m_tZ + m_fOffsetRadius);
		pVetex[i * 3].Pos.X = pos.m_X;
		pVetex[i * 3].Pos.Y = - pos.m_Y;
		pVetex[i * 3].Pos.Z = pos.m_Z;
		pVetex[i * 3].Color = clr;
		qglobe_get3DBodyPoint(meshs[i].pt2.m_tX, meshs[i].pt2.m_tY, pos, meshs[i].pt2.m_tZ + m_fOffsetRadius);
		pVetex[i * 3 + 1].Pos.X = pos.m_X;
		pVetex[i * 3 + 1].Pos.Y = - pos.m_Y;
		pVetex[i * 3 + 1].Pos.Z = pos.m_Z;
		pVetex[i * 3 + 1].Color = clr;
		qglobe_get3DBodyPoint(meshs[i].pt3.m_tX, meshs[i].pt3.m_tY, pos, meshs[i].pt3.m_tZ + m_fOffsetRadius);
