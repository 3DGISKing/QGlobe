...
#define MAX_RESOLUTION qglobe_DEGTORAD*10 /* degress */

#define EQUATOR					0*qglobe_DEGTORAD
#define TROPIC_OF_CANCER		23.5*qglobe_DEGTORAD
#define TROPIC_OF_CAPRICORN		-23.5*qglobe_DEGTORAD
#define ARCTIC_CIRCLE			66.5*qglobe_DEGTORAD
#define ANTARCTIC_CIRCLE		-66.5*qglobe_DEGTORAD
#define FIRST_MERIDIAN			0*qglobe_DEGTORAD
#define MERIDIAN				180*qglobe_DEGTORAD

#define MIN_RADIUS 				qglobe_DEGTORAD*0.0036
#define MAX_RADIUS				qglobe_DEGTORAD*10.0
...
QGlobe_GRID_VIEW_INFO g_gridViewInfo = {
		MAX_RADIUS,
		MAX_RADIUS,
...
static void AddDrawingText(double longi, double lati, const unsigned short* str)
{
	...
	qglobe_GetDataMgr()->m_pTextMgr->AddText(E_QGlobe_SUBDATA_TYPE_NONE,loc, RTT_GRID, str);		
}
...
int grid_CanDiviable(double* pdLongiInterval, double* pdLatiInterval)
{
	...
	double dCameraLongitude = qglobe_GetDataMgr()->m_pCamera->m_location.m_dLongitude;
	double dCameraLatitude = qglobe_GetDataMgr()->m_pCamera->m_location.m_dLatitude;
...
double grid_IsBackVtx(QGlobe_POINT3D& cameraPos, QGlobe_POINT3D& gridVtx)
{
	QGlobe_POINT3D vt;
...
double grid_FindFitAngle(double dAngle)
{
	...
	dCurAngle = dAngle * qglobe_RADTODEG;
	...
	dCurAngle = (d + m / 60.0 + s / 3600.0) * qglobe_DEGTORAD;
...
{	
	...
	QGlobe_POINT3D pt1;	
	qglobe_get3DWorldPoint( dCenterLongitude, dCenterLatitude, pt1, 0);
	QGlobe_POINT3D pt2;	
	qglobe_get3DWorldPoint( dCenterLongitude, dTmpLatitude, pt2, 0);
...
bool grid_UpdateGeoInfo(void)
{
	...
	CQGlobe_Camera *pCamera = qglobe_GetDataMgr()->m_pCamera;
...
//	gGRID_OFFSET_RADIUS = grid_CalcOffsetRadius(dLongiResolution, dLatiResolution);
	gGRID_OFFSET_RADIUS = qglobe_GetDataMgr()->m_pCamera->m_location.m_dDist / 10.0;
...
void grid_getGeoLongiVtx(geo::core::array<geo::scene::GRID_ONE_LINE>* pGeoLongiArray)
{
	...
	QGlobe_POINT3D ptCamera;
	ptCamera.m_X = qglobe_GetDataMgr()->m_pCamera->m_coord.m_org.m_tX;
	ptCamera.m_Y = qglobe_GetDataMgr()->m_pCamera->m_coord.m_org.m_tY;
	ptCamera.m_Z = qglobe_GetDataMgr()->m_pCamera->m_coord.m_org.m_tZ;
	...
	if( g_gridViewInfo.dMidMinLongi != -GRID_UNKNOWN)
	{
		for( double dLongi = dRealLongiStart ; dLongi <= g_gridViewInfo.dRealMidMinLongi; dLongi+= dLongiResoultion )
		{
			...
			{
				qglobe_get3DBodyPoint( dLongi, dLati, pt, gGRID_OFFSET_RADIUS, NULL, true);			
				vertex.X = pt.m_X;
				vertex.Y = -pt.m_Y;
				vertex.Z = pt.m_Z;
	...
	{		
		for(dLongi = dRealLongiStart ; dLongi <= dRealLongiEnd; dLongi+=dLongiResoultion)
		{
			...
			for(dLati = dRealLatiStart; dLati <= dRealLatiEnd; dLati+=dLatiResoultion)
			{
				...
				if(dScalar < 0)				
					gridLine.gridLineArray.push_back(vertex);
				if( qglobe_iszero(g_gridViewInfo.dMaxLati - dLati) || dLati > g_gridViewInfo.dMaxLati)
					break;
			}
			pGeoLongiArray->push_back(gridLine);
			if( qglobe_iszero(g_gridViewInfo.dMaxLongi - dLongi) || dLongi > g_gridViewInfo.dMaxLongi)
				break;
	       }	   	
	}
}
...
void grid_getGeoParallelVtx(geo::core::array<geo::scene::GRID_ONE_LINE>* pGeoTropicArray)
{
	...
	if( grid_IsInLongiRange(MERIDIAN))
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( MERIDIAN, dCameraLatitude, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
	...
	if( grid_IsInLongiRange(FIRST_MERIDIAN))
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( FIRST_MERIDIAN, dCameraLatitude, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
	...
	if(grid_IsInLatiRange(EQUATOR))
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( dCameraLongitude, EQUATOR, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
	...
	if(grid_IsInLatiRange(TROPIC_OF_CANCER))
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( dCameraLongitude, TROPIC_OF_CANCER, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
	...
	if(grid_IsInLatiRange(TROPIC_OF_CAPRICORN))
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( dCameraLongitude, TROPIC_OF_CAPRICORN, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
	...
	if(grid_IsInLatiRange(ARCTIC_CIRCLE))
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( dCameraLongitude, ARCTIC_CIRCLE, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
	...
	if(grid_IsInLatiRange(ANTARCTIC_CIRCLE))
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( dCameraLongitude, ANTARCTIC_CIRCLE, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
...
void	grid_getGeoTextVtx(void)
{
	...
	while(dLongi <= dRealLongiEnd)
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( dLongi, dCameraLatitude, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
		...
		if( fabs(dLongi) > 180.0*qglobe_DEGTORAD)
			continue;	

		QString longi;
		switch( qglobe_GetDataMgr()->m_sOption.gridViewInfo.gridMode)
		{
			case OPTION_LL_10NOTATION:
				longi = grid_Degree2Degree(dLongi*qglobe_RADTODEG, GEO_TYPE_LONGITUDE);
				break;
			case OPTION_LL_DMS:
				longi = grid_Degree2DMS(dLongi*qglobe_RADTODEG, GEO_TYPE_LONGITUDE);
				break;
			case OPTION_LL_10NOTATION_DM:
				longi = grid_Degree2DM(dLongi*qglobe_RADTODEG, GEO_TYPE_LONGITUDE);
				break;
		}
	...
	while(dLati <= dRealLatiEnd)
	{
		...
		geo::core::vector3df vertex;		
		qglobe_get3DBodyPoint( dCameraLongitude, dLati, pt, gGRID_OFFSET_RADIUS, NULL, true);			
		vertex.X = pt.m_X;
		vertex.Y = -pt.m_Y;
		vertex.Z = pt.m_Z;
		...
		if( fabs(dLati) > 90.0*qglobe_DEGTORAD)
			continue;
		...
		{
			case OPTION_LL_10NOTATION:
				lati = grid_Degree2Degree(dLati*qglobe_RADTODEG, GEO_TYPE_LATITUDE);
				break;
			case OPTION_LL_DMS:
				lati = grid_Degree2DMS(dLati*qglobe_RADTODEG, GEO_TYPE_LATITUDE);
				break;
			case OPTION_LL_10NOTATION_DM:
				lati = grid_Degree2DM(dLati*qglobe_RADTODEG, GEO_TYPE_LATITUDE);
				break;
		}		
...
void qglobe_GetGeoGridVtx(geo::core::array<geo::scene::GRID_ONE_LINE>* pGridArray,
							geo::core::array<geo::scene::GRID_ONE_LINE>* pGridTropicArray )
{
	if( !qglobe_GetDataMgr()->m_sOption.gridViewInfo.gridView)
		return;
...
int	qglobe_CallBackGetGridVtx(void* pUserData, void *param1, void *param2, void *param3)
{
	int ret = 1;
