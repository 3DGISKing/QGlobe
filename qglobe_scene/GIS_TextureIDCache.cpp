...
CGIS_TextureIDCache * qglobe_GetGISTextureIDCache(){return g_pGisTextureIDCache;}
...
long CGIS_TextureIDCache::GetTextureID(int level,int x,int y)
{
	...
	double u = tileWidth * x - qglobe_PI;
	double v = tileWidth * y - qglobe_PI;
	...
	if(!pImage)
		...
		return qglobe_GetGISTextureMgr()->getTransparentTextureID();
#else
		return 0;
#endif

	// generate texture id
	textureID = qglobe_GetGISTextureMgr()->AddTexture(x, y, level, (void*)pImage);
...
long CGIS_TextureIDCache::GetTextureID(QGlobe_TILE_INFO *pInfo)
{
	long textureID;
	int key1, key2;
	...
	double tileWidth = qglobe_TwoPI / (1 << pInfo->nLevel);
	double u = tileWidth * pInfo->nX - qglobe_PI;
	double v = tileWidth * pInfo->nY - qglobe_PI;
	...
	// generate texture id
	textureID = qglobe_GetGISTextureMgr()->AddTexture(pInfo->nX, pInfo->nY, pInfo->nLevel, (void*)pImage);
...
void CGIS_TextureIDCache::FreeData(void *pData)
{
	...
	qglobe_GetGISTextureMgr()->DelTexture(textureID);
}

void CGIS_TextureIDCache::AddTileEntry(QGlobe_TILE_INFO Info, void *buf)
{
	int key1, key2;
