...
void ResultTreeView::SaveStartPosition()
{
	...
	CGIS_Node *pNode = qglobe_GetGISDoc()->GetNodeFromKey(pmd.key);
...
void ResultTreeView::SendShowMarkItem(TreeItem *item, int type )
{
	...
	if (childcount == 0)
	{
		pNode = qglobe_GetGISDoc()->GetNodeFromKey(m_sendData.key);
			...
			m_sendData.itemtype == IT_PATH || m_sendData.itemtype == IT_POLYAREA)
		{
			...
			case MT_Remove:
				...
				qglobe_GetGISDoc()->DestroyNodeFromKey(m_sendData.key);
				break;
			case MT_Add:
			case MT_Check:
				if (m_sendData.start)
				{
					if (!pNode)
					{
						...
						qglobe_GetGISDoc()->AddNode(startNode);
					}
				}
				...
				if(pNode)
				{
					...
					qglobe_GetGISDoc()->SetModifyFlag(true);
				}
				break;
			case MT_Modify:
...
void ResultTreeView::ViewMark(QModelIndex index)
{
	...
	CGIS_Node * node=qglobe_GetGISDoc()->GetNodeFromKey(data.key);

	qglobe_GetGISDoc()->SetSelectNode(node);
