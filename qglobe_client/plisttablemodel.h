...
public:
	...
	void setData(CQGlobe_LocationArr &cPtArr);
	void getData(CQGlobe_LocationArr &cPtArr);
...
public:
    ...
    bool			insertRows(int position, int rows, const QModelIndex &index=QModelIndex());
    bool			removeRows(int position, int rows, const QModelIndex &index=QModelIndex());
	void			setDataPoint(CQGlobe_LocationArr *pPtArr) {m_pPtArr = pPtArr;}

private:
	CQGlobe_LocationArr	*m_pPtArr;
};
