...
void PListTableModel::setData(CQGlobe_LocationArr &cPtArr)
{
	m_pointList.clear();

	for(int i = 0; i < cPtArr.size(); i++)
	{
		CQGlobe_Location3D &pt = cPtArr[i];
		PointData p;
...
void PListTableModel::getData(CQGlobe_LocationArr &cPtArr)
{
	cPtArr.clear();
...
QVariant CPointListTableModel::data(const QModelIndex &index, int role) const
{
	...
	if (role == Qt::DisplayRole || role == Qt::EditRole) 
	{
		CQGlobe_Location3D pt = (*m_pPtArr)[index.row()];
...
bool CPointListTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
	...
	if (index.isValid() && role == Qt::EditRole) 
	{
		...
		CQGlobe_Location3D &pt = (*m_pPtArr)[row];
