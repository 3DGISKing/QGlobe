...
{
	CQGlobe_PathSim *pathSim = (CQGlobe_PathSim*)g_app->GetRenderProxy()->GetRenderSrv()->m_pSimMgr->GetCurSimulator();
	if (!pathSim)
		return false;

	QGlobe_SIM_STATUS status;
	memset(&status, 0, sizeof(QGlobe_SIM_STATUS));
...
void TourPlayBar::SetSimulatorStatus(QGlobe_SIM_CONTROL type)
{
	CQGlobe_PathSim *pathSim = (CQGlobe_PathSim*)g_app->GetRenderProxy()->GetRenderSrv()->m_pSimMgr->GetCurSimulator();
	if (!pathSim)
		return;
