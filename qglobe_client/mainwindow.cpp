...
void MainWindow::UpdateTourPlayLine()
{
	...
	else
	{
		...
		{			
			...
			CQGlobe_SimMgr* pSimMgr = g_app->GetRenderProxy()->GetRenderSrv()->m_pSimMgr;
			if (!pSimMgr)
				return;
			...
			memset(&simStatus, 0, sizeof(QGlobe_SIM_STATUS));
			...
			//SetGISPath(IN CGIS_Path* a_pGISPath , OUT QGlobe_CAMERA_INFO* a_pCameraInfo)
...
void MainWindow::UpdateCarSim()
{
	...
	if (!config->m_showCar)
	{
		...
		cameraInfo.m_orgDist = qglobe_getExactHeight(cameraInfo.m_loc.m_dLongitude, cameraInfo.m_loc.m_dLatitude);
		cameraInfo.m_loc.m_dDist = cameraInfo.m_orgDist + 3;
 		//cameraInfo.m_roll = 0.05;
 		cameraInfo.m_pitch = qglobe_PI / 2;
		...
		QGlobe_CAMERA_INFO cameraInfo;	
		qglobe_CameraGetLocation( g_pRender, &location );
		location.m_dDist = qglobe_getExactHeight(location.m_dLongitude, location.m_dLatitude) + 5;
