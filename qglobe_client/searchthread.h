...
class CQGlobe_SearchNode
{
public:
	CQGlobe_SearchNode(void);
	~CQGlobe_SearchNode(void);
	CQGlobe_SearchNode(const CQGlobe_SearchNode& a_cOther)
	{
...
public:
	bool operator<(const CQGlobe_SearchNode &a_cOther) const		{return Compare(a_cOther) < 0;}
	bool operator>(const CQGlobe_SearchNode &a_cOther) const		{return Compare(a_cOther) > 0;}
	bool operator==(const CQGlobe_SearchNode &a_cOther) const	{return Compare(a_cOther) == 0;}
	bool operator!=(const CQGlobe_SearchNode &a_cOther) const	{return Compare(a_cOther) != 0;}
	CQGlobe_SearchNode& operator=(const CQGlobe_SearchNode& a_cOther)
	{
		itemkey = a_cOther.itemkey;
		idCnt = a_cOther.idCnt;
		memcpy(idList, a_cOther.idList, sizeof(ST_RECORD_ID) * MAX_SEARCH_RESULT_CNT);
	...
	int		Compare(const CQGlobe_SearchNode &a_cOther) const;
public:
	int				itemkey;
	int				idCnt;
...
typedef qglobe_array<CQGlobe_SearchNode>	CQGlobe_SearchNodeArr;	
...
protected:
	...
	CQGlobe_SearchNodeArr	m_searchArray;
