...
void ObjectPlaceEditDlg::showEvent(QShowEvent *event)
{
	...
	m_pathCommand = (CQGlobe_DrawObjectCmd*)pCmdMgr->CreateCmd(eQGlobe_CMD_DRAW_OBJECT, this);
...
void ObjectPlaceEditDlg::setModifyData(int tb_type, int id)
{
	...
	if(qry.first()) {
		...
		if(tb_type == E_QGlobe_SUBDATA_TYPE_NAME_POLUTANT) {
...
{
	if(tb_type == E_QGlobe_SUBDATA_TYPE_NAME_ACCIDENT) {
		QSqlQuery qry;
		QString strSql;
		QString strVal;
		double dblVal;
...
void ObjectPlaceEditDlg::cmbObjectPlace_change(int idx)
{
	...
	if(m_shapeType > OBJECT_POINT) {
		...
		if(tbType == E_QGlobe_SUBDATA_TYPE_NAME_POLUTANT)
...
void ObjectPlaceEditDlg::GetNewPointData(PointData &pd, int curRow)
{
	...
	CQGlobe_Location3D prePos, nextPos, newPos;
...
void ObjectPlaceEditDlg::SetCenterMark()
{
	...
	if(clong > 0 && clat > 0) {
		...
		cameraInfo.m_loc.m_dLongitude =  clong * qglobe_DEGTORAD;
		cameraInfo.m_loc.m_dLatitude = clat * qglobe_DEGTORAD;
