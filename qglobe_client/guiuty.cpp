...
int gu_MessageBox(QWidget *parent, const QString &title, const QString &text, unsigned int flag)
{
	...
	if (id & MSGBOX_IDOK)
		okButton = msgBox.addButton(QApplication::translate("QGlobeClient","Ok"), QMessageBox::AcceptRole);
	if (id & MSGBOX_IDYES)
		yesButton = msgBox.addButton(QApplication::translate("QGlobeClient","Yes"), QMessageBox::YesRole);
	if (id & MSGBOX_IDNO)
		noButton = msgBox.addButton(QApplication::translate("QGlobeClient","No"), QMessageBox::NoRole);
	if (id & MSGBOX_IDCANCEL)
		cancelButton = msgBox.addButton(QApplication::translate("QGlobeClient","Cancel"), QMessageBox::RejectRole);
...
bool GetLookAtInfo(CGIS_Polygon* pNode, QGlobe_CAMERA_INFO *pLookAtInfo)
{
	if(pNode)
	{
		...
		{
			ltBaseHeigt = qglobe_getHeight(pNode->m_cBound.m_cMinPt.m_tX, pNode->m_cBound.m_cMinPt.m_tY);
			rbBaseHeight = qglobe_getHeight(pNode->m_cBound.m_cMaxPt.m_tX, pNode->m_cBound.m_cMaxPt.m_tY);
		}
		...
		qglobe_GetCoordInfo(&coord, pLookAtInfo);
		return true;
	}
	return false;
}

void GetValidPos(CQGlobe_Location3D* pFirstPos, CQGlobe_Location3D* pSecondPos, CQGlobe_Location3D* pDstPos, bool blClosed)
{
	if(pDstPos == NULL)
		return;
	double offset = 10.0;
	...
	if(pFirstPos == NULL && pSecondPos == NULL)
	{
		...
		vt = coord.m_z * (-1);
		if(QGlobe_FAIL == qglobe_HitPoint(coord.m_org,vt, &pt))
		{
			pt = coord.m_org;
		}
