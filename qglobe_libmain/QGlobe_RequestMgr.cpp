...
// for fps feedback
#define	QGlobe_FPS_MIN_CHECK			8
#define	QGlobe_FPS_MAX_CHECK			30	
#define REQUEST_START_OFFSET_LEVEL	0
#define REQUEST_VALID_TILE_COUNT	10

#define QGlobe_REQUEST_DEM_OFFSET_LEVEL	2
#define QGlobe_REQUEST_IMG_OFFSET_LEVEL	0
#define QGlobe_REQUEST_NAME_OFFSET_LEVEL	1
#define QGlobe_REQUEST_SHAPE_OFFSET_LEVEL	0
// end

CQGlobe_RequestMgr::CQGlobe_RequestMgr(void)
{
	...
	for(int i = 0; i < QGlobe_MAX_REQUEST_LIST_COUNT; i++)
		memset(m_pTileReqList[i], 0, sizeof(QGlobe_TILE_REQUEST));
...
void CQGlobe_RequestMgr::InitSpec()
{
	// dem tile
	m_sTileReq_Dem.spec.eType		= E_QGlobe_MAINDATA_TYPE_DEM;
	m_sTileReq_Dem.spec.maxLevel	= 13;
	m_sTileReq_Dem.spec.minLevel	= 0;
	m_sTileReq_Dem.spec.offsetLevel	= QGlobe_REQUEST_DEM_OFFSET_LEVEL;
	m_sTileReq_Dem.spec.maxRequestCount	= 10;
	
	// img tile
	m_sTileReq_Img.spec.eType		= E_QGlobe_MAINDATA_TYPE_IMG;
	m_sTileReq_Img.spec.maxLevel	= 19;
	m_sTileReq_Img.spec.minLevel	= 0;
	m_sTileReq_Img.spec.offsetLevel	= QGlobe_REQUEST_IMG_OFFSET_LEVEL;
	m_sTileReq_Img.spec.maxRequestCount	= 16;
	
	// name
	m_sTileReq_Name.spec.eType			= E_QGlobe_MAINDATA_TYPE_NAME;
	m_sTileReq_Name.spec.maxLevel		= 16;
	m_sTileReq_Name.spec.minLevel		= 3;
	m_sTileReq_Name.spec.offsetLevel	= QGlobe_REQUEST_NAME_OFFSET_LEVEL;
	m_sTileReq_Name.spec.maxRequestCount	= 6;

	
	// shape
	m_sTileReq_Shape.spec.eType			= E_QGlobe_MAINDATA_TYPE_SHAPE;
	m_sTileReq_Shape.spec.maxLevel		= 17;
	m_sTileReq_Shape.spec.minLevel		= 4;
	m_sTileReq_Shape.spec.offsetLevel	= QGlobe_REQUEST_SHAPE_OFFSET_LEVEL;
	m_sTileReq_Shape.spec.maxRequestCount	= 6;

	// 3d
	m_sTileReq_3D.spec.eType			= E_QGlobe_MAINDATA_TYPE_3D;
...
QGlobe_TILE_REQUEST* CQGlobe_RequestMgr::GetTileRequest(E_QGlobe_MAINDATA_TYPE eType)
{
	QGlobe_TILE_REQUEST *pReqList = NULL;
...
const QGlobe_TILE_SPEC* CQGlobe_RequestMgr::GetSpec(E_QGlobe_MAINDATA_TYPE eType)
{
	QGlobe_TILE_REQUEST *pReq = GetTileRequest(eType);
...
void CQGlobe_RequestMgr::SetStatusFlag(QGlobe_TILE_INFO *pInfo, E_TILE_STATUS status)
{
	QGlobe_TILE_REQUEST *pRequest = NULL;
	QGlobe_TILE_INFO_ARR *pReqList = NULL;
...
bool CQGlobe_RequestMgr::GetHighRequestTile(QGlobe_TILE_INFO *tile, E_QGlobe_MAINDATA_TYPE eType)
{
	QGlobe_TILE_REQUEST *pRequest = NULL;
	QGlobe_TILE_INFO_ARR *pReqList = NULL;
	QGlobe_TILE_INFO *pTile = NULL;
	float prio = -1;

	memset(tile, 0, sizeof(QGlobe_TILE_INFO));
	...
	memcpy(tile, pTile, sizeof(QGlobe_TILE_INFO));
...
// add one tile
void addOnTile(int nX, int nY, int nLevel, int prio, QGlobe_TILE_INFO_ARR *tileArr)
{
	if(tileArr->count >= QGlobe_MAX_TILE)
		return;
...
void CQGlobe_RequestMgr::InitDefaultRequest()
{
	QGlobe_TILE_INFO_ARR tileArr;

	tileArr.count = 0;
	memset(&tileArr, 0, sizeof(QGlobe_TILE_INFO_ARR));
...
void CQGlobe_RequestMgr:: UpdateRequest()
{
	QGlobe_TILE_INFO_ARR tileArr;
	...
	if(tileArr.count > 0)
	{
		if(tileArrforEstimate.count+tileArr.count>QGlobe_MAX_TILE)
		{
			qFatal("Program encountered error in CQGlobe_RequestMgr::UpdateRequest! file:%s line:%d",__FILE__,__LINE__);
		}
		memcpy(&tileArrforEstimate.data[tileArrforEstimate.count], tileArr.data, sizeof(QGlobe_TILE_INFO) * tileArr.count);
	...
	for(int i = 0; i < QGlobe_MAX_REQUEST_LIST_COUNT; i++)
	{
		if(m_pTileReqList[i]->spec.eType == E_QGlobe_MAINDATA_TYPE_DEM)
...
void CQGlobe_RequestMgr::UpdateRequestTiles(QGlobe_TILE_REQUEST *pRequest, QGlobe_TILE_INFO_ARR *pPyramidList)
{
	QGlobe_TILE_INFO cTile;
	QGlobe_TILE_INFO_ARR newReqList;
	...
	memset(&newReqList, 0, sizeof(QGlobe_TILE_INFO_ARR));

	for(i = 0; i < pPyramidList->count; i++)
	{
		memcpy(&cTile, pPyramidList->data + i, sizeof(QGlobe_TILE_INFO));

		if(i >= maxRequestCount * 3)
			break;

		if(pRequest->spec.eType == E_QGlobe_MAINDATA_TYPE_IMG)
			offsetLevel = m_nOffsetLevel - m_nOffsetBySpeed;
		else
			offsetLevel = m_nOffsetLevel;

		if(pRequest->spec.eType != E_QGlobe_MAINDATA_TYPE_IMG && cTile.offsetLevel > 0)
		...
		while(cTile.nLevel < eLevel)
		{
			...
			{
				if(pRequest->reqList.data[j].nStatus == E_TILE_STATUS_REQ && qglobe_IsSameTile(&cTile, pRequest->reqList.data + j))
				{
					isReq =0 ;
					break;
				}
			...
			// check list
			if(qglobe_IsContained(cTile.nX, cTile.nY, cTile.nLevel, &newReqList))
				continue;
	...
	memcpy(&(pRequest->reqList), &newReqList, sizeof(QGlobe_TILE_INFO_ARR));
	pRequest->isEmpty = false;
	pRequest->isLock = false;
}

int CQGlobe_RequestMgr::IsExistItem(QGlobe_TILE_INFO_ARR *pReqList)
{
	int i = 0;
...
void CQGlobe_RequestMgr::CheckRate(void)
{
	QGlobe_TILE_REQUEST *pRequest;
	...
	long curFrame = qglobe_GetFPSTester()->m_curFrame;

	if(m_blHighSpeed && curFrame < QGlobe_FPS_MIN_CHECK)
	{
		m_nOffsetBySpeed =1;
		m_blHighSpeed = false;
	}

	if(!m_blHighSpeed && curFrame > QGlobe_FPS_MAX_CHECK)
	{
		m_nOffsetBySpeed = 0;
		m_blHighSpeed = true;
	}
...
void CQGlobe_RequestMgr::ClearRequest(E_QGlobe_MAINDATA_TYPE eType)
{
	for(int i = 0; i < QGlobe_MAX_REQUEST_LIST_COUNT; i++)
	{
...
void CQGlobe_RequestMgr::UpdateRequest(E_QGlobe_MAINDATA_TYPE eType)
{
	QGlobe_TILE_INFO_ARR tileArr;
	QGlobe_TILE_INFO_ARR tileArrforEstimate;
