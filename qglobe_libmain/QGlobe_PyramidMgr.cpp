...
const double		g_latLimit = 78 * qglobe_DEGTORAD;

// level of root tiles
#define  QGlobe_ROOT_LEVEL		2

// Constants to determine level
#define	QGlobe_TILE_SPREAD			1.5
#define QGlobe_TILE_VIEW_RANGE		0.5
#define QGlobe_PYRAMID_MAX_RATE	2
#define QGlobe_PYRAMID_STANDARD_TILE_WIDTH	256

//	Update pyramid from camera
//	Generate pyramid quad tree and check them
//	It is called for pyramid calculation

void CQGlobe_PyramidMgr::Update()
{
	if (m_onWorking)
		return;
...
void CQGlobe_PyramidMgr::GetPyramidTiles(QGlobe_TILE_INFO_ARR *psTileInfoArr)
{
	Q_ASSERT(psTileInfoArr != NULL);
		
	memcpy(psTileInfoArr, &m_sTileInfoArrOrg, sizeof(QGlobe_TILE_INFO_ARR));
}

//	Constructor
CQGlobe_PyramidMgr::CQGlobe_PyramidMgr(void)
{
	m_levelInfos = NULL;
	m_sTileInfoArrOrg.count = 0;
	...
	memset(&m_viewInfo, 0, sizeof(QGlobe_VIEW_INFO));
}
...
TilePool::TilePool()
{
	for(int i=0;i<100;i++)
	{
		QGlobe_QUAD_TILE* quadTile = (QGlobe_QUAD_TILE*) malloc(sizeof(QGlobe_QUAD_TILE));

		memset(quadTile,0,sizeof(QGlobe_QUAD_TILE));
...
QGlobe_QUAD_TILE* TilePool::GetOneTile()
{
...
void CQGlobe_PyramidMgr::InitializeTileSetInfo()
{
	m_levelNum = MAX_LEVEL+1;
	m_levelInfos = (QGlobe_LEVEL_INFO*)malloc(m_levelNum * sizeof(QGlobe_LEVEL_INFO));
	...
	for (int i = 0; i < m_levelNum; i++)
	{
		...
		dividNum = 1 << i;
		m_levelInfos[i].tileSize = qglobe_TwoPI/ dividNum;
	}
}

//	Construct root quad tiles
//	Now, we start 8 tiles of 2 level because that they are valid in our case
void CQGlobe_PyramidMgr::InitRootQuadTiles()
{
	m_rootNum = 8;
	m_arrRoots = (QGlobe_QUAD_TILE**)malloc(sizeof(QGlobe_QUAD_TILE*) * m_rootNum);
	if (m_arrRoots == NULL)
		return;
	m_arrRoots[0] = MakeNewQuadTile(-qglobe_PI, -qglobe_HALF_PI, -qglobe_HALF_PI, 0, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[0])
	{
		m_arrRoots[0]->row = 1;
		m_arrRoots[0]->col = 0;
	}
	m_arrRoots[1] = MakeNewQuadTile(-qglobe_HALF_PI, 0, -qglobe_HALF_PI, 0, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[1])
	{
		m_arrRoots[1]->row = 1;
		m_arrRoots[1]->col = 1;
	}
	m_arrRoots[2] = MakeNewQuadTile(0, qglobe_HALF_PI, -qglobe_HALF_PI, 0, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[2])
	{
		m_arrRoots[2]->row = 1;
		m_arrRoots[2]->col = 2;
	}
	m_arrRoots[3] = MakeNewQuadTile(qglobe_HALF_PI, qglobe_PI, -qglobe_HALF_PI, 00, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[3])
	{
		m_arrRoots[3]->row = 1;
		m_arrRoots[3]->col = 3;
	}
	m_arrRoots[4] = MakeNewQuadTile(-qglobe_PI, -qglobe_HALF_PI, 0, qglobe_HALF_PI, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[4])
	{
		m_arrRoots[4]->row = 2;
		m_arrRoots[4]->col = 0;
	}
	m_arrRoots[5] = MakeNewQuadTile(-qglobe_HALF_PI, 0, 0, qglobe_HALF_PI, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[5])
	{
		m_arrRoots[5]->row = 2;
		m_arrRoots[5]->col = 1;
	}
	m_arrRoots[6] = MakeNewQuadTile(0, qglobe_HALF_PI, 0, qglobe_HALF_PI, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[6])
	{
		m_arrRoots[6]->row = 2;
		m_arrRoots[6]->col = 2;
	}
	m_arrRoots[7] = MakeNewQuadTile(qglobe_HALF_PI, qglobe_PI, 0, qglobe_HALF_PI, QGlobe_ROOT_LEVEL);
	if (m_arrRoots[7])
	{
		m_arrRoots[7]->row = 2;
		m_arrRoots[7]->col = 3;
	...
	m_MaxLat=-qglobe_HALF_PI;
	m_MinLat=qglobe_HALF_PI;
	m_Maxlon=-qglobe_PI;
	m_MinLon=qglobe_PI;
}

// Start the root quad tile and recursively divide the quad tree
void CQGlobe_PyramidMgr::GenerateVisibleQuadTree()
{
	QGlobe_QUAD_TILE* root;
...
QGlobe_QUAD_TILE* CQGlobe_PyramidMgr::MakeNewQuadTile(IN double a_west, IN double a_east, IN double a_south, IN double a_north, IN char a_level)
{
	QGlobe_QUAD_TILE* quadTile = m_TilePool->GetOneTile();
...
void CQGlobe_PyramidMgr::CreateChildQuadTiles(IN OUT QGlobe_QUAD_TILE* a_parent)
{
...
void CQGlobe_PyramidMgr::SphericalToCartesian(IN double a_latitude, IN double a_longitude, 
										   IN double a_radius, OUT CQGlobe_Vector3DF& a_xyz)
{
...
void CQGlobe_PyramidMgr::CalcBoundBox(IN double a_west, IN double a_east, 
								   IN double a_south, IN double a_north, 
								   IN double a_radius1, IN double a_radius2, 
								   OUT QGlobe_BOUND_BOX* a_boundBox)
{
	CQGlobe_Vector3DF* ver;
...
bool CQGlobe_PyramidMgr::IsCoverCameraPos(IN QGlobe_QUAD_TILE* a_quadTile)
{
...
void CQGlobe_PyramidMgr::CheckQuadTile(IN QGlobe_QUAD_TILE* a_quadTile)
{
	...
	QGlobe_LEVEL_INFO* levelInfo = &m_levelInfos[a_quadTile->level];

	if ( levelInfo->level > QGlobe_ROOT_LEVEL + 1)
	{
		...
		if (isdivieded)
			isdivieded = m_viewInfo.cameraViewRange < levelInfo->tileSize * QGlobe_TILE_VIEW_RANGE * (1 + m_viewInfo.quality);
		...
		if (isdivieded)
		{
			...
			isdivieded = dist < levelInfo->tileSize * QGlobe_TILE_SPREAD;
		}
...
bool CQGlobe_PyramidMgr::IntersectBoundingBox(IN QGlobe_BOUND_BOX* a_boxA, IN QGlobe_BOUND_BOX* a_boxB)
{
...
bool CQGlobe_PyramidMgr::IntersectWithFrustum(IN QGlobe_QUAD_TILE* a_quadTile)
{
	QGlobe_BOUND_BOX* box = &a_quadTile->bonndBox;
...
double CQGlobe_PyramidMgr::SphericalDistance(IN double a_latitude1, IN double a_longitude1, 
...
void CQGlobe_PyramidMgr::AddTileInfo(IN QGlobe_QUAD_TILE* a_quadTile)
{
	if(m_sTileInfoArrOrg.count >= QGlobe_MAX_TILE)
		return;
...
void PerspectiveMatrix (double fovy, double aspect, double nearPlane, double farPlane, CQGlobe_Matrix4& m)
{
...
void CQGlobe_PyramidMgr::UpdateViewInfo()
{
	...
	if (rate > 1)
		m_viewInfo.cameraViewRange = qglobe_PI;
	else
	...
	QGlobe_Coord3D cameraCoord;
	a_pCamera->GetCameraCoord(&cameraCoord);

	m_viewInfo.quality		   = QGlobe_PYRAMID_MAX_RATE * a_pCamera->m_fScalarOfDir* a_pCamera->m_fScalarOfDir;	

	// Calculate six plane equations of view frustum
	// Camera view matrix

	CQGlobe_Matrix4 viewMat, projMat, combMat;

	qglobe_MatrixSCStoTCS(&cameraCoord, &viewMat);
...
void CQGlobe_PyramidMgr::CalcVFBoundBox(IN QGlobe_Coord3D* a_coord, IN QGlobe_ViewFrustum* a_frustum, OUT QGlobe_BOUND_BOX* a_boundBox)
{
	...
	double coff[5][3] = {{ xmax,  ymax, -zmax},
						 ...
						 {-xmax,  ymax, -zmax},
						 {0, 0, 0}};
	CQGlobe_Vector3DF minV, maxV, tmp;
...
/* ���� ���� ����� nLevel�� �ش��ϴ� 4�� tile�� ��´�.*/
void CQGlobe_PyramidMgr::GetNear4Tiles(int nLevel, double dLongitude, double	dLatitude, QGlobe_TILE_INFO_ARR *psTileInfoArr)
{
	...
	memset(psTileInfoArr, 0, sizeof(QGlobe_TILE_INFO_ARR));
	...
	nNewY = nY;
	if(!qglobe_IsContained(nNewX, nNewY, psTileInfoArr))
	{
...
void CQGlobe_PyramidMgr::CheckAndCompleteTiles()
{
	QGlobe_TILE_INFO *pTile, *pOther;
	...
	for(int i = 0; i <m_sTileInfoArrOrg.count; i++)
	{
		...
		pTile->tileWidth = qglobe_TwoPI / (1 << pTile->nLevel);
		pTile->minLongitude = pTile->tileWidth * pTile->nX - qglobe_PI;
		...
		if(m_sTileInfoArrOrg.count > (QGlobe_MAX_TILE - 4) || (i >= count))
			continue;
...
void CQGlobe_PyramidMgr::SortTiles()
{
	Q_ASSERT(m_sTileInfoArrOrg.count <= QGlobe_MAX_TILE);

	QGlobe_TILE_INFO *pTile1, *pTile2, sTileTmp;
	...
	for(int i = 0; i <m_sTileInfoArrOrg.count - 1; i++)
	{
		...
		for(int j = i + 1; j < m_sTileInfoArrOrg.count; j++)
		{
			...
			{
				memcpy(&sTileTmp, pTile1, sizeof(QGlobe_TILE_INFO));
				memcpy(pTile1, pTile2, sizeof(QGlobe_TILE_INFO));
				memcpy(pTile2, &sTileTmp, sizeof(QGlobe_TILE_INFO));
			}
		}
...
bool CQGlobe_PyramidMgr::Intersect(CQGlobe_Aabbox3DF box)
{
