...
static void _generate_key (QGlobe_TILE_INFO* pInfo, int *key1, int* key2)
{
...
CQGlobe_CacheIndexTree::CQGlobe_CacheIndexTree()
{
...
int CQGlobe_CacheIndexTree::AddIndex(CACHE_FILE_INDEX* entry)
{
	int ret;
...
int CQGlobe_CacheIndexTree::GetIndex(CACHE_FILE_INDEX* entry)
{
...
void CQGlobe_CacheIndexTree::RemoveAll()
{
...
CQGlobe_FileCacheMgr::CQGlobe_FileCacheMgr()
{
	QString tmpPath;
	...
	if (m_pIndices != NULL)
	{
		for (int i = 0; i < E_QGlobe_MAINDATA_TYPE_MAX; i++)
			m_pIndices[i] = new CQGlobe_CacheIndexTree();
	}
...
CQGlobe_FileCacheMgr::~CQGlobe_FileCacheMgr()
{
	CQGlobe_CacheIndexTree *pIndex = NULL;
...
void CQGlobe_FileCacheMgr::OpenCacheDataFile(const char *szPathname)
{
	if (szPathname == NULL)
		return;
...
void CQGlobe_FileCacheMgr::Initialize()
{
	if (m_fpCacheData)
	{
		fclose(m_fpCacheData);
...
void CQGlobe_FileCacheMgr::LoadStoredData()
{
	CACHE_FILE_INDEX* entry;
	E_QGlobe_MAINDATA_TYPE  eMainType;
	int	pos;
	...
	while (!feof(m_fpCacheData))
	{
		...
		eMainType = (E_QGlobe_MAINDATA_TYPE) (entry->key1 >> MAX_MAIN_TYPE_OFFSET);
		if (pos == entry->offset)
		{
...
int CQGlobe_FileCacheMgr::IsValidCacheFile()
{
...
void CQGlobe_FileCacheMgr::Clear()
{
	CQGlobe_CacheIndexTree *pIndex = NULL;
...
void CQGlobe_FileCacheMgr::WriteTileBuffer(QGlobe_TILE_INFO *pInfo, void *buffer)
{
	CACHE_FILE_INDEX	entry;
	...
	if (pInfo->eMainType >= E_QGlobe_MAINDATA_TYPE_MAX || pInfo->eMainType < 0)
		return; /* data invalid */
 ...
 * OUT         : void* buffer
 */
int CQGlobe_FileCacheMgr::ReadData(int offset, int size, void* buffer)
{
	int nRet;
...
int CQGlobe_FileCacheMgr::WriteData(CACHE_FILE_INDEX* key, void* buffer, int size)
{
	int nRet;
