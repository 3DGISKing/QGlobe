...
#define QGlobe_FONSIZE_STEP1	3000000
#define QGlobe_FONSIZE_STEP2	5000000
#define QGlobe_FONSIZE_STEP3	7000000

int CQGlobe_TextNode::Compare(const CQGlobe_TextNode &a_cOther) const
{
	int ret = 0;
...
CQGlobe_TextNode::CQGlobe_TextNode(void)
{
...
void CQGlobe_TextNode::IncreaseScale(bool blFocus)
{
	if(!blFocus)
	{
...
void CQGlobe_TextNode::DecreaseScale(bool blFocus)
{
	if(!blFocus)
	{
...
bool CQGlobe_TextNode::IsFullView()
{
	return m_data.drawScale > 0.99f;
}

bool CQGlobe_TextNode::IsEnableFocusScale(RENTER_TEXT_TYPE	type)
{
	return true;
	switch(type)
	{
...
CQGlobe_TextMgr::CQGlobe_TextMgr(void)
{
...
void CQGlobe_TextMgr::Init(int width, int height)
{
	screenWidth = width;
	screenHeight= height;
...
void CQGlobe_TextMgr::Init(void)
{
	m_cAddList.clear();
	nFocus_ID = -1;
}

void CQGlobe_TextMgr::AddText(E_QGlobe_SUBDATA_TYPE datatype,CQGlobe_Location3D &loc, RENTER_TEXT_TYPE type, const unsigned short* str, int key, bool blKorean)
{
	QString strName;
...
void CQGlobe_TextMgr::ResetScreenPt(CQGlobe_TextNode *pNode)
{
	QGlobe_ERROR_NO ret;
...
void CQGlobe_TextMgr::AddText(E_QGlobe_SUBDATA_TYPE datatype,CQGlobe_Location3D &loc, RENTER_TEXT_TYPE type, QString &str, int key, bool blKorean)
{
	int len;

	CQGlobe_TextNode textNode;
...
void CQGlobe_TextMgr::SetInfo(QGlobe_TextCell* pTextCell , double	screenX, double screenY, RENTER_TEXT_TYPE type,double scale)
{
	QString strTmp;
	...
	case RTT_PYONGYANG:
	{
			if(dist > QGlobe_FONSIZE_STEP3)
			{
				fontID += 3;
				scale = 0.7;
			}
			else if(dist > QGlobe_FONSIZE_STEP2)
			{
				fontID += 2;
				scale = 0.8;
			}
			else if(dist > QGlobe_FONSIZE_STEP1)
			{
				fontID += 1;
				scale = 0.9;
			}
...
int CQGlobe_TextMgr::GetFontSize(int fontID)
{
	int nOffset = 0, size;
	switch(fontID)
	{
...
bool CQGlobe_TextMgr::isIntersects(QGlobe_TextCell* pCell1, QGlobe_TextCell* pCell2)
{
...
void CQGlobe_TextMgr::GetDrawingText(CQGlobe_TextNodeArr &cArr)
{
	SortAndArrange();
	SetFocusState();
	cArr = m_cOldList;
...
void CQGlobe_TextMgr::SortAndArrange()
{
	...
	{
		CQGlobe_TextNode &node = m_cAddList[i];
	...
	{
		CQGlobe_TextNode &node = m_cOldList[i];
...
void CQGlobe_TextMgr::SetFocusState()
{
...
//by RSH 2013.7.25
void CQGlobe_TextMgr::AddText(CQGlobe_Location3D &loc, RENTER_TEXT_TYPE type, QString &str, int key, bool blKorean, QString iconName,double iconscale)
{
	int len;
