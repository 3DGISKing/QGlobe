...
#ifdef	DEBUG_TERRAIN
	#include "QGlobe_LibMain.h"
#endif

void CQGlobe_Ruler::GetMeasureSceneNode(QGlobe_SceneManager* sceneMgr)
{
...
void CQGlobe_Ruler::Clear ()
{
	if (m_pRulerNode)
		m_pRulerNode->Clear ();
}

void CQGlobe_Ruler::StepPoint(QGlobe_LOCATION *a_pLocation, int nStep, bool blLine)
{
...
void CQGlobe_Ruler::MovePoint(QGlobe_LOCATION *a_pLocation, int nStep, bool blLine)
{
	if (blLine)
	{
		if (nStep == 1)
...
void CQGlobe_Ruler::Release (int nStep, bool blLine)
{
	if (blLine)
	{
		if (nStep == 1)
...
bool CQGlobe_Ruler::GetPointFromLocation (QGlobe_LOCATION *a_pLocation, CQGlobe_Point3DF	*a_pPoint, double dHeight)
{
	QGlobe_POINT3D		ptWorld;

	qglobe_get3DBodyPoint (a_pLocation->m_dLongitude, a_pLocation->m_dLatitude, ptWorld, dHeight, NULL, true);
...
double CQGlobe_Ruler::CountAllLength (bool blLine)
{
	double	dblLength = 0;
	...
	{
		double	dblValue = 0;
		if (qglobe_GetLength (NULL, &m_pRulerNode->m_locStart, &m_pRulerNode->m_locEnd, &dblValue) == QGlobe_SUCCESS)
			dblLength += dblValue;
	}
	else
	{
		...
		{
			QGlobe_LOCATION	*pStart = &m_pRulerNode->m_aryLocation[i - 1];
			QGlobe_LOCATION	*pEnd	= &m_pRulerNode->m_aryLocation[i];

			double	dblValue = 0;
			if (qglobe_GetLength (NULL, pStart, pEnd, &dblValue) == QGlobe_SUCCESS)
				dblLength += dblValue;
		}
	}
...
double CQGlobe_Ruler::CountAngle (bool blLine)
{
	double	dblAngle = 0;
	...
	/*
		...
		qglobe_GetPosition (&m_pRulerNode->m_locStart,	&ptStart);
		qglobe_GetPosition (&m_pRulerNode->m_locEnd,	&ptEnd);
		rotAxis = ptStart.crossProduct (ptEnd);

		qglobe_GetAngleBetweenTwoVector (ptStart, ptEnd, rotAxis, dblAngle);
...
void CQGlobe_Ruler::AddPoint (geo::core::array<geo::core::vector3df> *a_pPointAry, QGlobe_LOCATION *a_pLocation, double a_dHeight, double a_fRatio)
{
	CQGlobe_Point3DF	ptWorld;

#ifndef	DEBUG_TERRAIN
	GetPointFromLocation (a_pLocation, &ptWorld, a_dHeight);
#else
	{
		QGlobe_POINT3D	ptWork;
		bool		bHeight		= true;
		...
		if(bHeight && g_pQGlobeDataMgr->m_sOption.is_LayerTerrain)
		{
			...
			if(pTile == NULL)
				dValue = qglobe_getHeight(dLongitude, dLatitude);
			else
				dValue = qglobe_getHeightInTile(dLongitude, dLatitude, pTile);
...
int	CQGlobe_Ruler::GetRulerSegPoints(void *pPoint, QGlobe_LOCATION *pStart, QGlobe_LOCATION *pEnd)
{
	...
	for (int i=1; i<nPts; i++, dAngNode += dAngStep)
	{
		...
		AddPoint (pPointAry, &locNode, dHeight, sin (qglobe_HALF_PI * i / nPts));
	}
	...
	for (int i=1; i<nPts; i++, dAngNode += dAngStep)
	{
		...
		qglobe_RotateAroundVector (&point, dAngNode, rotAxis);
		qglobe_GetLocation (&point, &locNode);
		AddPoint (pPointAry, &locNode, dHeight, sin (qglobe_HALF_PI * (nPts - i) / nPts));
	}
...
int	CQGlobe_Ruler::GetValidDivid(QGlobe_LOCATION *pStart, QGlobe_LOCATION *pEnd, double	&dHeight)
{
	#define		MIN_DIVIDE		2
