...
#include "../qglobe_base/QGlobe_Aabbox3D.h"
#include "../qglobe_base/QGlobe_Triangle3d.h"
#include "QGlobe_Interface.h"

#define QGlobe_MAX_CHILD_TYPE_COUNT	10
#define QGlobe_MAX_NAME_CHILD_TYPE_COUNT	27

#define MAX_VERTEX_BUFFER_SIZE  QGLOBE_MAX_DEM_PT_COUNT*QGLOBE_MAX_DEM_PT_COUNT*30
#define MAX_INDEX_BUFFER_SIZE  QGLOBE_MAX_DEM_PT_COUNT*QGLOBE_MAX_DEM_PT_COUNT*100

typedef unsigned char BOUND_COND[QGLOBE_MAX_DEM_DIVISION4+1];
typedef QGlobe_POINT3D   BOUNDARY[QGLOBE_MAX_DEM_DIVISION4+1];
...
typedef struct _gdmLogicTile
{
	...
	QGlobe_POINT3D			edge[4];					// the edge point
	CQGlobe_Vector3DF		bndCenter;

	QGlobe_DEM_TILE*		pDemTile;					// covered Dem tile
	QGlobe_DEM_TILE*		pContourDemTile;			// covered Dem tile for contour
	QGlobe_TEXTURE_TILE*	pImgTile;					// covered Image tile
	QGlobe_TILE*	prasterTile;               


	QGlobe_SHAPE_TILE*		sArrShapeTile [QGlobe_MAX_CHILD_TYPE_COUNT];	// shape objects tile
	QGlobe_TILE*           sArr3DMeshTile[QGlobe_MAX_CHILD_TYPE_COUNT];
	QGlobe_TILE*           sArrPolygonTile[QGlobe_MAX_CHILD_TYPE_COUNT];
	QGlobe_TILE*			sArrNameTile  [QGlobe_MAX_NAME_CHILD_TYPE_COUNT];	// shape objects tile
	int					nShapeTileCnt;
	int					nNameTileCnt;
	int                 n3dMeshTileCnt;
	int                 nPoygonTileCnt;
...
typedef struct _gdmMeshTile
{
	...
	QGlobe_POINT3D*		buf_dem;		// dem buffer
	short*				buf_height;		// dem buffer
	QGlobe_DIRECTION_INFO* buf_dirInfo;	// dem normal vector info
	...
	int					nSubY;
	int					nSubs;
	QGlobe_DEM_TILE*		pDemTile;	// for height in small tile		
}QGlobe_MESH_TILE;
...
typedef struct _gdmMeshTileArr
{
	int	nLevel;
	int nCount;
	QGlobe_MESH_TILE pData[QGlobe_MAX_TILE];
}QGlobe_MESH_TILE_ARR;
...
{
	QuadTree quadTrees[QGlobe_MAX_TILE];
	...
	QGlobe_LOCATION	m_CameraLocation;	
	int  m_nUsed;
...
extern TreeListSelector	   *qglobe_TileListSelector;	
