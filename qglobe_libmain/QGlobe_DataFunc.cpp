...
#define QGlobe_MAX_SHAPE_TILES	15
#define QGlobe_MAX_NAME_TILES	15
#define QGlobe_MAX_3DMESH_TILES 15

#define QGlobe_MAX_DISPLAY_SHAPE_LEVEL		15

QGlobe_LOGIC_TILE_ARR g_qglobe_tile_list;

CQGlobe_Vector3DF g_OffsetBodyPt;

void qglobe_ClearPrepareRenderData()
{
	g_qglobe_tile_list.nCount = 0;
}

QGlobe_TILE*	qglobe_GetCoverTileAndUpdate(int nX, int nY, int nLevel, E_QGlobe_SUBDATA_TYPE eType, int nOffset)
{
	...
	{
		if(mainType == E_QGlobe_MAINDATA_TYPE_NAME)
			return NULL;
...
const QGlobe_LOGIC_TILE_ARR*		qglobe_GetRenderTiles()
{
	return &g_qglobe_tile_list;
}

void qglobe_MakePrepareRenderData()
{
	QGlobe_TILE_INFO_ARR sTileInfoArr;
	QGlobe_LOGIC_TILE *pTile;
	...
	{
		pTile = &g_qglobe_tile_list.pData[i];
		...
		QGlobe_TILE		*pNameTile;
		QGlobe_SHAPE_TILE	*pShapeTile;
		QGlobe_TILE	    *p3dMeshTile;
		...
		*******************************************/
		if(i < maxCount)
		{
			for(int k = 0; k < arrTypes.size(); k++)
			{
				...
				pNameTile = (QGlobe_TILE*)qglobe_GetCoverTileAndUpdate(nX, nY, nLevel, type, nOffsetLevel);
				if(pNameTile)
				{
		...
		*******************************************/
		if(i < maxCount)
		{
			for(int k = 0; k < arrTypes.size(); k++)
			{
				...
				pShapeTile = (QGlobe_SHAPE_TILE*)qglobe_GetCoverTileAndUpdate(nX, nY, nLevel, type, nOffsetLevel);
				if(pShapeTile)
				{
		...
		*******************************************/
		...
		if(i < maxCount)
		{
			for(int k = 0; k < arrTypes.size(); k++)
			{
				...
				{
					if(pTile->n3dMeshTileCnt >= QGlobe_MAX_CHILD_TYPE_COUNT)
						continue;

					p3dMeshTile = qglobe_GetCoverTileAndUpdate(nX, nY, nLevel, type, nOffsetLevel);
					if(p3dMeshTile)
					{
				...
				else if(type==E_QGlobe_SUBDATA_TYPE_BUILDING_SIMPLEBUILDING)
				{
					if(pTile->nPoygonTileCnt >= QGlobe_MAX_CHILD_TYPE_COUNT)
						continue;
		...
		qglobe_get3DWorldPoint(pTile->minLongitude, pTile->minLatitude, pTile->edge[0], 0, pTile->pDemTile, true);
		qglobe_get3DWorldPoint(pTile->maxLongitude, pTile->minLatitude, pTile->edge[1], 0, pTile->pDemTile, true);
		qglobe_get3DWorldPoint(pTile->minLongitude, pTile->maxLatitude, pTile->edge[3], 0, pTile->pDemTile, true);
		qglobe_get3DWorldPoint(pTile->maxLongitude, pTile->maxLatitude, pTile->edge[2], 0, pTile->pDemTile, true);
...
void   qglobe_get3DWorldPoint(double dLongitude, double dLatitude, QGlobe_POINT3D &pos, double offsetRadius, QGlobe_DEM_TILE* pTile, bool bHeight)
{
	...
	if(bHeight && g_pGDMDataMgr->IsVisible(E_QGlobe_SUBDATA_TYPE_DEM))
	{
		if(pTile == NULL)
			...
			dRadius += qglobe_getHeightInTile(dLongitude, dLatitude, pTile);
...
void   qglobe_get3DWorldPoint(double dLongitude, double dLatitude, CQGlobe_Point3DF &pos, double offsetRadius, QGlobe_DEM_TILE* pTile, bool bHeight)
{
	double dRadius, dXRadius, d;
...
void   qglobe_get3DBodyPoint(double dLongitude, double dLatitude, QGlobe_POINT3D &pos, double offsetRadius, QGlobe_DEM_TILE* pTile, bool bHeight)
{
	qglobe_get3DWorldPoint(dLongitude, dLatitude, pos, offsetRadius, pTile, bHeight);
...
short  qglobe_getHeightInTile(double dLongitude, double dLatitude, QGlobe_DEM_TILE* pTile)
{
	short height;
...
QGlobe_DIRECTION_INFO	qglobe_getRateInfoInTile(double dLongitude, double dLatitude, QGlobe_DEM_TILE* pTile)
{
	QGlobe_DIRECTION_INFO info;
	QGlobe_DIRECTION_INFO *pInfos;
	...
	info = pInfos[iY * QGLOBE_MAX_DEM_PT_COUNT + iX];
	return info;
}

short  qglobe_getMaxHeightInTile(double dLongitude, double dLatitude, QGlobe_DEM_TILE* pTile)
{
	short height[4];
	short high = 0;
	...
	height[0] = pTile->m_pData[iY * QGLOBE_MAX_DEM_PT_COUNT + iX];
	if(iX < QGLOBE_MAX_DEM_PT_COUNT)
		height[1] = pTile->m_pData[iY * QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
	if(iX < QGLOBE_MAX_DEM_PT_COUNT && iY < QGLOBE_MAX_DEM_PT_COUNT)
		height[2] = pTile->m_pData[(iY + 1) * QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
	if(iY < QGLOBE_MAX_DEM_PT_COUNT)
		height[3] = pTile->m_pData[(iY + 1) * QGLOBE_MAX_DEM_PT_COUNT + iX];
...
inline void	qglobe_getSpherePoint( double dLongitude, double dLatitude, double dRadius, CQGlobe_Point3DF* point )
{
	double dXRadius;
	dXRadius = dRadius * cos(dLatitude);
...
double  qglobe_getExactHeightInTile(double dLongitude, double dLatitude, QGlobe_DEM_TILE* pTile)
{		
	Q_ASSERT(pTile!=NULL);
	...
	Q_ASSERT(iX < QGLOBE_MAX_DEM_PT_COUNT);
	Q_ASSERT(iY < QGLOBE_MAX_DEM_PT_COUNT);
	...
	QGlobe_LOCATION		triV[3];
	CQGlobe_Triangle3DF	tri;
	CQGlobe_Point3DF		triP[3];

	triV[0].m_dLongitude	= pTile->minLongitude;
	triV[0].m_dLatitude		= pTile->minLatitude;
	triV[0].m_dDist			= pTile->m_pData[iY * QGLOBE_MAX_DEM_PT_COUNT + iX];
	if ( bTri012 )
	{
		triV[1].m_dLongitude	= pTile->maxLongitude;
		triV[1].m_dLatitude		= pTile->minLatitude;
		triV[1].m_dDist			= pTile->m_pData[iY * QGLOBE_MAX_DEM_PT_COUNT + iX + 1];

		triV[2].m_dLongitude	= pTile->maxLongitude;
		triV[2].m_dLatitude		= pTile->maxLatitude;
		triV[2].m_dDist			= pTile->m_pData[(iY + 1) * QGLOBE_MAX_DEM_PT_COUNT + iX + 1];
	}
	else
	{
		triV[1].m_dLongitude	= pTile->maxLongitude;
		triV[1].m_dLatitude		= pTile->maxLatitude;
		triV[1].m_dDist			= pTile->m_pData[(iY + 1) * QGLOBE_MAX_DEM_PT_COUNT + iX + 1];

		triV[2].m_dLongitude	= pTile->minLongitude;
		triV[2].m_dLatitude		= pTile->maxLatitude;
		triV[2].m_dDist			= pTile->m_pData[(iY + 1) * QGLOBE_MAX_DEM_PT_COUNT + iX];
	}
	...
	//
	for ( int i = 0; i < 3; i++ )
		qglobe_getSpherePoint( triV[i].m_dLongitude, triV[i].m_dLatitude, EARTH_RADIUS + triV[i].m_dDist , &triP[i] );
	tri.set( triP[0], triP[1], triP[2] );

	//
	//	Make the position object and its normal vector
	//
	CQGlobe_Point3DF pos, normVect, realPos;
	qglobe_getSpherePoint( dLongitude, dLatitude, EARTH_RADIUS, &pos );
...
void   qglobe_get3DWorldHighPoint(double dLongitude, double dLatitude, QGlobe_POINT3D &pos)
{
	int height = 0;
	...
	qglobe_get3DWorldPoint(dLongitude, dLatitude, pos, height);
}

short qglobe_getMaxHeight(double dLongitude, double dLatitude)
{
	QGlobe_DEM_TILE *pTile = qglobe_getDemTile(dLongitude, dLatitude);
	...
	return qglobe_getMaxHeightInTile(dLongitude, dLatitude, pTile);
}

double qglobe_getExactHeight(double dLongitude, double dLatitude)
{
	QGlobe_DEM_TILE *pTile = qglobe_getDemTile(dLongitude, dLatitude);
	if(pTile == NULL)
		return 0;
	return qglobe_getExactHeightInTile(dLongitude, dLatitude, pTile);
}

short qglobe_getHeight(double dLongitude, double dLatitude)
{
	QGlobe_DEM_TILE *pTile = qglobe_getDemTile(dLongitude, dLatitude);
	...
	return qglobe_getHeightInTile(dLongitude, dLatitude, pTile);
}

QGlobe_DIRECTION_INFO	qglobe_getRateInfo(double dLongitude, double dLatitude)
{
	QGlobe_DIRECTION_INFO info;
	...
	return qglobe_getRateInfoInTile(dLongitude, dLatitude, pTile);
}

QGlobe_DEM_TILE* qglobe_getDemTile(double dLongitude, double dLatitude)
{
	const QGlobe_LOGIC_TILE *pTile = NULL;
	const QGlobe_LOGIC_TILE_ARR* p_tile_list = qglobe_GetRenderTiles();
...
#define QGlobe_PATCH_SIZE_IN_TILE		2
#define QGlobe_TILE_PATCH_DIVISION		QGLOBE_MAX_DEM_DIVISION / QGlobe_PATCH_SIZE_IN_TILE

#define QGlobe_XY_TO_INDEX(x, y, stride)	(y) * (stride) + (x) 
#define QGlobe_PATCH_XY_TO_INDEX(px, py)	QGlobe_XY_TO_INDEX((px) * (QGlobe_PATCH_SIZE_IN_TILE), (py) * (QGlobe_PATCH_SIZE_IN_TILE), QGLOBE_MAX_DEM_PT_COUNT)  

inline
void getPatchQuadVertIdx(IN int px, IN int py, OUT int pVertIdx[4])
{
	pVertIdx[0] = QGlobe_PATCH_XY_TO_INDEX( px, py );
	pVertIdx[1] = QGlobe_PATCH_XY_TO_INDEX( px + 1, py );
	pVertIdx[2] = QGlobe_PATCH_XY_TO_INDEX( px + 1, py + 1 );
	pVertIdx[3] = QGlobe_PATCH_XY_TO_INDEX( px, py + 1 );
...
inline
int qglobe_intersectRayTriangle(IN CQGlobe_Point3DF& rayOrg , IN CQGlobe_Vector3DF& rayDir , 
							 IN const QGlobe_POINT3D& triV0, IN const QGlobe_POINT3D& triV1, IN const QGlobe_POINT3D& triV2,
							 OUT CQGlobe_Point3DF* pOutPt)
{
	CQGlobe_Vector3DF    u, v, n;             // triangle vectors
	CQGlobe_Vector3DF    dir, w0, w;          // ray vectors
	double			  r, a, b;             // params to calc ray-plane intersect
// 	CQGlobe_Vector3DF v0, v1, v2;
// 
...
// 	if ( r < 1 ) return -1;
	...
	if ( n.equals( CQGlobe_Vector3DF(0)) )     // triangle is degenerate
	...
	if (qglobe_equals(b, double(0) )) {     // ray is parallel to triangle plane
...
inline void
intersectRayTriangle(IN CQGlobe_Point3DF& rayOrg , IN CQGlobe_Vector3DF& rayDir , 
					 IN const QGlobe_POINT3D& triV0, IN const QGlobe_POINT3D& triV1, IN const QGlobe_POINT3D& triV2,
					 OUT CQGlobe_Point3DF* pOutPt, IN OUT double* minDist, OUT QGlobe_ERROR_NO* err)
{
	int ret = 0;
	double dist;
	CQGlobe_Point3DF pt;
	ret = qglobe_intersectRayTriangle( rayOrg, rayDir, triV0, triV1, triV2, &pt);
	if ( ret == 1 )
	{
...
QGlobe_ERROR_NO qglobe_intersectRayDEM(CQGlobe_Point3DF& rayOrg , CQGlobe_Vector3DF& rayDir , CQGlobe_Point3DF* pOutPt)
{
#ifdef DEBUG
	int nTriNum = 0;
	int nTick = qglobe_GetTickTimer();
	int nTile = 0;
#endif
	const QGlobe_LOGIC_TILE_ARR* pTileList = qglobe_GetRenderTiles();
	const QGlobe_LOGIC_TILE *pTile = NULL;
	QGlobe_DEM_TILE *pDEMTile = NULL;
	QGlobe_POINT3D* vertBuf = NULL;
	QGlobe_ERROR_NO err = QGlobe_FAIL;
	...
	for ( i = 0; i < pTileList->nCount; i++ )
	{
		...
		nDivision = QGLOBE_MAX_DEM_DIVISION / ( 1<<nDelta );		
...
#ifdef DEBUG
	nTick = qglobe_GetTickTimer() - nTick;
...
2 = are in the same plane
************************************************************************/
bool isInTriangle(CQGlobe_Point3DF p, Triangle3DF T)
{
	CQGlobe_Point3DF    u, v, w;             // triangle vectors
...
int
intersect_RayTriangle(CQGlobe_Point3DF rayOrg, CQGlobe_Point3DF rayDir, Triangle3DF T, CQGlobe_Point3DF *destPt, double *distance, bool flag)
{
	CQGlobe_Point3DF    u, v, n;             // triangle vectors
	CQGlobe_Point3DF    w0, w;          // ray vectors
	double     r, a, b;             // params to calc ray-plane intersect
	...
	v = T.pointC - T.pointA;	
	n = u.crossProduct(v);
	if (n == CQGlobe_Point3DF(0))            // triangle is degenerate
		return -1;                 // do not deal with this case
...
bool QuadTree::intersectsBoxWithLine(int index, CQGlobe_Point3DF rayOrg, CQGlobe_Point3DF rayDir, int *idx)
{	
	CQGlobe_Point3DF mid;	
	CQGlobe_Point3DF closest_pos;
	double diag, radius;
	double d[4];
	int n = 0;
...
int QuadTree::getIntersectionAll(CQGlobe_Point3DF rayOrg, CQGlobe_Point3DF rayDir, double* lamda)
{			
	Triangle3DF tri;			
	double  dis, lamda2;//, d;
	lamda2 = (*lamda)*(*lamda);

	CQGlobe_Point3DF outPt;
...
int QuadTree::getIntersection(CQGlobe_Point3DF rayOrg, CQGlobe_Point3DF rayDir, double* lamda, int index)
{	
	int chi = index<<2;	
	int flag = 0;	
...
int QuadTree::getHeight(double dLongitude, double dLatitude, double &a_Height)
{
	...
	int a,b,c,n,i;
	CQGlobe_Point3DF pa,pb,pc,pt,ps;
	a = (int)(row)*ngridSize;
	b = (int)col;
	...
	CQGlobe_Point3DF rayOrg, rayDir,despt;
	pt = CQGlobe_Point3DF(col,row,0);
...
/************************************************************************
qglobe_intersectRayDEM2(): intersection point test with terrain
Input:  a ray R 
Output: intersection point *cp(if exist), success/fail
************************************************************************/

QGlobe_ERROR_NO qglobe_intersectRayDEM2(CQGlobe_Point3DF& rayOrg, CQGlobe_Point3DF& rayDir, CQGlobe_Point3DF *cp)
{	
	QuadTree *qtree;
	int quicktree = -1;		
	...
	CQGlobe_Point3DF closest_pos = rayOrg+rayDir*(rayDir.dotProduct(-rayOrg));	
	...
	CQGlobe_Point3DF nearpos = closest_pos+(-rayDir)*t;
	...
	CQGlobe_Point3DF rayorg(rayOrg.m_tX,rayOrg.m_tY,rayOrg.m_tZ);
	CQGlobe_Point3DF raydir(rayDir.m_tX,rayDir.m_tY,rayDir.m_tZ);

	if(!qglobe_TileListSelector)
		return QGlobe_FAIL;

	TerrainQuadTreeList *tileQuadList = qglobe_TileListSelector->GetBufForRead();
	if (tileQuadList){
...
double qglobe_IntersectHeight(double dLongitude, double dLatitude)
{	
	QuadTree *qtree;
	double a_Height;
	TerrainQuadTreeList *tileQuadList = qglobe_TileListSelector->GetBufForRead();
	...
	return qglobe_getExactHeight(dLongitude, dLatitude);
}